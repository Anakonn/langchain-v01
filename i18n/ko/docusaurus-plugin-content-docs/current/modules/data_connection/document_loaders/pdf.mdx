...
>[Portable Document Format (PDF)](https://en.wikipedia.org/wiki/PDF), 표준화된 ISO 32000, 는 Adobe가 1992년에 개발한 파일 형식으로, 응용 프로그램 소프트웨어, 하드웨어, 운영 체제와 무관하게 문서, 텍스트 서식 및 이미지를 표현할 수 있습니다.

이 문서는 PDF 문서를 우리가 사용하는 문서 형식으로 로드하는 방법을 다룹니다.

## PyPDF 사용하기

`pypdf`를 사용하여 PDF를 로드하고 각 문서에 `page` 번호가 포함된 메타데이터와 페이지 내용을 배열로 가져옵니다.
...
`rapidocr-onnxruntime` 패키지를 사용하면 이미지를 텍스트로 추출할 수 있습니다:
...
이것은 PDF 구문 분석 옵션 중 가장 빠르며, PDF와 해당 페이지에 대한 자세한 메타데이터를 포함하고 페이지당 하나의 문서를 반환합니다.
...
또한 `load` 호출에서 [PyMuPDF 문서](https://pymupdf.readthedocs.io/en/latest/app1.html#plain-text/)의 모든 옵션을 키워드 인수로 전달할 수 있으며, 이는 `get_text()` 호출에 전달됩니다.

## MathPix 사용하기

Daniel Gross의 [https://gist.github.com/danielgross/3ab4104e14faccc12b49200843adab21](https://gist.github.com/danielgross/3ab4104e14faccc12b49200843adab21)에서 영감을 받았습니다.
...
`unstructured[all-docs]` 패키지는 현재 텍스트 파일, 파워포인트, HTML, PDF, 이미지 등을 로드할 수 있습니다.
...
Unstructured는 내부적으로 텍스트의 다른 부분에 대해 다른 "요소"를 만듭니다. 기본적으로 이를 결합하지만 `mode="elements"`를 지정하여 이 구분을 쉽게 유지할 수 있습니다.
...
이 섹션에서는 온라인 PDF를 문서 형식으로 로드하는 방법을 다룹니다. 이는 https://open.umn.edu/opentextbooks/textbooks/ 및 https://arxiv.org/archive/와 같은 다양한 온라인 PDF 사이트에 사용할 수 있습니다.

참고: 다른 모든 PDF 로더도 원격 PDF를 가져오는 데 사용할 수 있지만 `OnlinePDFLoader`는 레거시 함수이며 `UnstructuredPDFLoader`와 함께 작동합니다.
...
PDFMiner는 PDF 문서에서 정보를 추출하고 데이터를 분석하는 데 도움이 되는 도구입니다.
...
### PDFMiner를 사용하여 HTML 텍스트 생성하기

이는 출력 HTML 콘텐츠를 `BeautifulSoup`를 통해 구문 분석하여 글꼴 크기, 페이지 번호, PDF 헤더/푸터 등에 대한 더 구조화되고 풍부한 정보를 얻을 수 있어 텍스트를 의미론적으로 섹션으로 나누는 데 도움이 될 수 있습니다.
...
PyMuPDF와 마찬가지로, 출력 문서에는 PDF와 해당 페이지에 대한 자세한 메타데이터가 포함되어 있으며 페이지당 하나의 문서를 반환합니다.
...
## AmazonTextractPDFParser 사용하기

AmazonTextractPDFLoader는 [Amazon Textract Service](https://aws.amazon.com/textract/)를 호출하여 PDF를 문서 구조로 변환합니다. 현재 로더는 순수 OCR을 수행하며, 레이아웃 지원 등 추가 기능은 수요에 따라 계획되어 있습니다. 단일 및 다중 페이지 문서가 최대 3000페이지와 512MB 크기까지 지원됩니다.

AWS 계정이 필요하며, [AWS CLI](https://docs.aws.amazon.com/cli/latest/userguide/cli-chap-configure.html) 요구 사항과 유사합니다.

AWS 구성 외에도 다른 PDF 로더와 매우 유사하지만, JPEG, PNG, TIFF 및 네이티브 PDF 형식을 지원합니다.
...
## AzureAIDocumentIntelligenceLoader 사용하기
...
`Document Intelligence`를 사용하는 이 [현재 구현](https://aka.ms/di-langchain)은 콘텐츠를 페이지 단위로 통합하고 LangChain 문서로 변환할 수 있습니다. 기본 출력 형식은 markdown이며, `MarkdownHeaderTextSplitter`와 쉽게 연결할 수 있습니다. `mode="single"` 또는 `mode="page"`를 사용하여 단일 페이지 또는 페이지별로 분할된 순수 텍스트를 반환할 수도 있습니다.

### 전제 조건

**East US**, **West US2**, **West Europe** 중 하나의 3개 미리 보기 지역에 Azure AI Document Intelligence 리소스가 필요합니다. [이 문서](https://learn.microsoft.com/azure/ai-services/document-intelligence/create-document-intelligence-resource?view=doc-intel-4.0.0)를 따라 리소스를 만들 수 있습니다. `<endpoint>` 및 `<key>`를 매개변수로 전달해야 합니다.
...
## UpstageLayoutAnalysisLoader 사용하기

UpstageLayoutAnalysisLoader는 [Upstage Layout Analysis API](https://developers.upstage.ai/docs/apis/layout-analyzer)를 호출하여 다양한 문서 형식에서 테이블 및 그림을 포함한 문서 요소를 감지합니다. 이 로더는 순수 OCR을 사용하여 텍스트 정보를 추출하고 `JPEG`, `PNG`, `BMP`, `PDF`, `TIFF` 및 `HEIC` 파일 내의 요소를 감지합니다. 디지털 PDF 문서의 경우 use_ocr=False로 설정하여 파일 내 텍스트 정보를 활용할 수 있습니다. 이것이 기본값입니다. 단일 및 다중 페이지 문서를 지원하며, use_ocr=True인 경우 100페이지 및 50MB 파일 크기 제한이 있지만, use_ocr=False(PDF 파일에만 해당)인 경우 제한이 없습니다.

### 전제 조건

Upstage Layout Analysis API에 액세스하려면 API 액세스 토큰이 필요합니다. [빠른 시작 가이드](https://developers.upstage.ai/docs/getting-started/quick-start)를 참조하여 액세스 토큰을 얻고 Upstage Layout Analysis API 사용을 시작하세요.
