...
`자연어 API` 툴킷(`NLAToolkits`)은 LangChain 에이전트가 엔드포인트 간에 효율적으로 계획하고 조합할 수 있게 해줍니다.

이 노트북은 `Speak`, `Klarna`, `Spoonacular` API의 샘플 구성을 보여줍니다.

### 먼저, 종속성을 가져오고 LLM을 로드합니다.
...
### 인증 사용 및 더 많은 엔드포인트 추가

일부 엔드포인트에는 액세스 토큰과 같은 사용자 인증이 필요할 수 있습니다. 여기서는 `Requests` 래퍼 객체를 통해 인증 정보를 전달하는 방법을 보여줍니다.

각 NLATool이 래핑된 API에 대한 간결한 자연어 인터페이스를 노출하므로, 최상위 대화형 에이전트는 사용자 요청을 충족하기 위해 각 엔드포인트를 더 쉽게 통합할 수 있습니다.

**Spoonacular 엔드포인트 추가.**

1. [Spoonacular API 콘솔](https://spoonacular.com/food-api/console#Profile)에 가서 무료 계정을 만드세요.
2. `Profile`을 클릭하고 API 키를 아래에 복사하세요.
