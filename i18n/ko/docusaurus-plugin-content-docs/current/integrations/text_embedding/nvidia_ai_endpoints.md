...
> [NVIDIA AI Foundation Endpoints](https://www.nvidia.com/en-us/ai-data-science/foundation-models/)는 사용자에게 NVIDIA Mixtral 8x7B, Llama 2, Stable Diffusion 등의 NVIDIA AI Foundation 모델에 대한 NVIDIA 호스팅 API 엔드포인트에 쉽게 액세스할 수 있는 기능을 제공합니다. [NVIDIA API 카탈로그](https://build.nvidia.com/)에 호스팅되는 이러한 모델은 최적화되고 테스트되어 NVIDIA AI 플랫폼에서 호스팅되므로 가속화된 스택에서 빠르고 쉽게 평가하고 추가로 사용자 정의하며 원활하게 최고의 성능으로 실행할 수 있습니다.
>
> [NVIDIA AI Foundation Endpoints](https://www.nvidia.com/en-us/ai-data-science/foundation-models/)를 통해 [NVIDIA DGX Cloud](https://www.nvidia.com/en-us/data-center/dgx-cloud/)에서 실행되는 완전히 가속화된 스택에서 빠른 결과를 얻을 수 있습니다. 사용자 정의 후에는 [NVIDIA AI Enterprise](https://www.nvidia.com/en-us/data-center/products/ai-enterprise/)를 사용하여 엔터프라이즈 수준의 보안, 안정성 및 지원으로 이러한 모델을 어디서나 배포할 수 있습니다.
>
> 이러한 모델은 [`langchain-nvidia-ai-endpoints`](https://pypi.org/project/langchain-nvidia-ai-endpoints/) 패키지를 통해 쉽게 액세스할 수 있습니다.

이 예제에서는 LangChain을 사용하여 [NVIDIA Retrieval QA Embedding Model](https://catalog.ngc.nvidia.com/orgs/nvidia/teams/ai-foundation/models/nvolve-40k)과 상호 작용하는 방법을 설명합니다. 이 모델은 [retrieval-augmented generation](https://developer.nvidia.com/blog/build-enterprise-retrieval-augmented-generation-apps-with-nvidia-retrieval-qa-embedding-model/)을 위해 `NVIDIAEmbeddings` 클래스를 통해 사용할 수 있습니다.

채팅 모델에 이 API를 통해 액세스하는 방법에 대한 자세한 내용은 [ChatNVIDIA](https://python.langchain.com/docs/integrations/chat/nvidia_ai_endpoints/) 문서를 참조하세요.
...
3. `Input`에서 `Python` 탭을 선택하고 `Get API Key`를 클릭한 다음 `Generate Key`를 클릭하세요.

4. 생성된 키를 `NVIDIA_API_KEY`로 복사하여 저장하세요. 그러면 엔드포인트에 액세스할 수 있습니다.
...
목록에서 임베딩 모델을 찾을 수 있으며, 이 모델을 LLM과 함께 사용하여 효과적인 RAG 솔루션을 구현할 수 있습니다. `NVIDIAEmbeddings` 모델을 사용하면 이 모델과 쉽게 상호 작용할 수 있습니다.

## 초기화

임베딩 모델을 초기화할 때 모델을 전달하거나(예: `ai-embed-qa-4`) 인수를 전달하지 않으면 기본값을 사용할 수 있습니다.
...
이 모델은 fine-tuned E5-large 모델로, 다음과 같은 `Embeddings` 메서드를 지원합니다:

- `embed_query`: 쿼리 샘플에 대한 쿼리 임베딩을 생성합니다.

- `embed_documents`: 검색할 문서 목록에 대한 passage 임베딩을 생성합니다.

- `aembed_quey`/`embed_documents`: 위의 비동기 버전입니다.

### **유사성**

다음은 사용 사례에 따른 메서드 사용, 형식 및 속도를 빠르게 테스트한 것입니다:

**쿼리:**

- Komchatka의 날씨는 어떤가요?

- 이탈리아에서 유명한 음식은 무엇인가요?

- 제 이름이 뭔지 기억하시나요? 기억하지 못할 거예요...

- 삶의 목적이 뭔가요?

- 삶의 목적은 즐겁게 사는 것이에요 :D

**문서:**

- Komchatka의 날씨는 추운 편이며 겨울이 길고 혹독합니다.

- 이탈리아는 파스타, 피자, 젤라토, 에스프레소로 유명합니다.

- 개인 이름은 기억할 수 없지만 정보는 제공할 수 있습니다.

- 삶의 목적은 개인에 따라 다르며, 보통 자아실현으로 여겨집니다.

- 삶의 순간을 즐기는 것은 훌륭한 접근법입니다.
...
이제 임베딩을 생성했으므로 결과에 대한 간단한 유사성 검사를 수행하여 검색 작업에서 합리적인 답변을 제공할 수 있는 문서를 확인할 수 있습니다:
...
다시 말씀드리면, 시스템에 전송된 쿼리와 문서는 다음과 같습니다:
...
다음은 [LangChain Expression Language Retrieval Cookbook 항목](https://python.langchain.com/docs/expression_language/cookbook/retrieval)의 초기 예제를 재사용한 것이지만, AI Foundation Models의 [Mixtral 8x7B Instruct](https://catalog.ngc.nvidia.com/orgs/nvidia/teams/ai-foundation/models/mixtral-8x7b) 및 [NVIDIA Retrieval QA Embedding](https://catalog.ngc.nvidia.com/orgs/nvidia/teams/ai-foundation/models/nvolve-40k) 모델을 사용하여 실행한 것입니다. 이 Cookbook의 후속 예제들도 예상대로 실행되며, 이러한 옵션을 탐색해 보시기 바랍니다.

**팁:** Mixtral을 내부 추론(예: 데이터 추출, 도구 선택 등을 위한 지침 따르기)에 사용하고, Llama-Chat를 단일 최종 "사용자의 기록과 맥락을 기반으로 작동하는 간단한 응답으로 마무리하기" 응답에 사용하는 것이 좋습니다.
