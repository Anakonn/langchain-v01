...
[Aphrodite](https://github.com/PygmalionAI/aphrodite-engine)는 [PygmalionAI](https://pygmalion.chat) 웹사이트에서 수천 명의 사용자를 지원하도록 설계된 오픈 소스 대규모 추론 엔진입니다.

* vLLM의 주의 메커니즘을 통한 빠른 처리량과 낮은 지연 시간
* 다양한 SOTA 샘플링 방법 지원
* 더 낮은 배치 크기에서 더 나은 처리량을 위한 Exllamav2 GPTQ 커널

이 노트북은 langchain과 Aphrodite를 사용하여 LLM을 사용하는 방법을 설명합니다.

사용하려면 `aphrodite-engine` Python 패키지가 설치되어 있어야 합니다.
...
LLM 클래스로 다중 GPU 추론을 실행하려면 `tensor_parallel_size` 인수를 사용할 GPU 수로 설정하십시오. 예를 들어 4개의 GPU에서 추론을 실행하려면
