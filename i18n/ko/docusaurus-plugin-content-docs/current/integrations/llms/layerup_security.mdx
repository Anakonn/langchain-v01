...
[Layerup 보안](https://uselayerup.com) 통합을 통해 LangChain LLM, LLM 체인 또는 LLM 에이전트에 대한 호출을 보안할 수 있습니다. LLM 객체는 기존 LLM 객체를 감싸서 사용자와 LLM 사이에 보안 계층을 제공합니다.

Layerup 보안 객체는 LLM으로 설계되었지만 실제로는 LLM이 아니며, 단순히 LLM을 감싸서 기본 LLM과 동일한 기능을 수행할 수 있도록 합니다.

## 설정

먼저 Layerup 보안 계정이 필요합니다. Layerup [웹사이트](https://uselayerup.com)에서 계정을 만드세요.

다음으로 [대시보드](https://dashboard.uselayerup.com)에서 프로젝트를 생성하고 API 키를 복사하세요. API 키를 프로젝트의 환경 변수에 저장하는 것이 좋습니다.

Layerup 보안 SDK를 설치하세요:
