...
[ExLlamav2](https://github.com/turboderp/exllamav2)는 현대 소비자급 GPU에서 LLM을 로컬로 실행할 수 있는 빠른 추론 라이브러리입니다.

GPTQ & EXL2 양자화 모델에 대한 추론을 지원하며, [Hugging Face](https://huggingface.co/TheBloke)에서 액세스할 수 있습니다.

이 노트북에서는 LangChain 내에서 `exllamav2`를 실행하는 방법을 설명합니다.
...
원하는 기계에서 사용할 수 있는 모델을 이해하는 것이 중요합니다.

[TheBloke's](https://huggingface.co/TheBloke) Hugging Face 모델에는 다양한 양자화 크기와 방법(예: [Mistral-7B-Instruct-v0.2-GPTQ](https://huggingface.co/TheBloke/Mistral-7B-Instruct-v0.2-GPTQ))에 대한 RAM 요구사항이 포함된 `Provided files` 섹션이 있습니다.
