...
이 노트북은 [Cohere의 재순위화 엔드포인트](https://docs.cohere.com/docs/reranking)를 리트리버에서 사용하는 방법을 보여줍니다. 이는 [ContextualCompressionRetriever](/docs/modules/data_connection/retrievers/contextual_compression/)의 아이디어를 기반으로 합니다.
...
먼저 간단한 벡터 저장소 리트리버를 초기화하고 2023년 국정연설문(chunk 단위)을 저장해 보겠습니다. 리트리버를 설정하여 많은 수(20개)의 문서를 검색할 수 있습니다.
...
## CohereRerank를 사용한 재순위화

이제 기본 리트리버를 `ContextualCompressionRetriever`로 감싸 보겠습니다. `CohereRerank`를 추가하여 Cohere 재순위화 엔드포인트를 사용하여 반환된 결과를 재순위화합니다.
