...
>[하이브리드 검색](https://weaviate.io/blog/hybrid-search-explained)은 검색 결과의 정확성과 관련성을 높이기 위해 여러 검색 알고리즘을 결합하는 기술입니다. 키워드 기반 검색 알고리즘과 벡터 검색 기술의 장점을 활용합니다.

>`Weaviate의 하이브리드 검색`은 희소 벡터와 밀집 벡터를 사용하여 검색 쿼리와 문서의 의미와 맥락을 나타냅니다.

이 노트북은 `Weaviate 하이브리드 검색`을 LangChain 리트리버로 사용하는 방법을 보여줍니다.
