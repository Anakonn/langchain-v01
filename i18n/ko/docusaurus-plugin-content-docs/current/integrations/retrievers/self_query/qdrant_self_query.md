...
>[Qdrant](https://qdrant.tech/documentation/) (읽기: 쿼드란트)은 벡터 유사성 검색 엔진입니다. 편리한 API를 제공하여 포인트(벡터와 추가 페이로드)를 저장, 검색 및 관리할 수 있는 프로덕션 준비 서비스를 제공합니다. `Qdrant`는 확장된 필터링 지원에 맞춰져 있습니다.

이 노트북에서는 `Qdrant` 벡터 스토어를 래핑한 `SelfQueryRetriever`를 데모할 것입니다.

## Qdrant 벡터 스토어 생성하기

먼저 Qdrant 벡터 스토어를 생성하고 일부 데이터로 채워 보겠습니다. 영화 요약이 포함된 작은 데모 세트를 만들었습니다.

**참고:** self-query retriever를 사용하려면 `lark`를 설치해야 합니다(`pip install lark`). `qdrant-client` 패키지도 필요합니다.
