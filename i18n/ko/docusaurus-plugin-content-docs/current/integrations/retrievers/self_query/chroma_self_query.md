...
이 노트북에서는 `Chroma` 벡터 저장소를 둘러싼 `SelfQueryRetriever`를 데모할 것입니다.

## Chroma 벡터 저장소 만들기

먼저 Chroma 벡터 저장소를 만들고 일부 데이터로 채워야 합니다. 영화 요약이 포함된 작은 데모 세트를 만들었습니다.

**참고:** 자체 쿼리 리트리버를 사용하려면 `lark`를 설치해야 합니다(`pip install lark`). `langchain-chroma` 패키지도 필요합니다.
...
생성자에 `enable_limit=True`를 전달하면 이 작업을 수행할 수 있습니다.
