...
>[Pinecone](https://docs.pinecone.io/docs/overview)는 광범위한 기능을 가진 벡터 데이터베이스입니다.

이 연습에서는 `Pinecone` 벡터 저장소와 함께 `SelfQueryRetriever`를 시연할 것입니다.

## Pinecone 인덱스 생성하기

먼저 `Pinecone` 벡터 저장소를 만들고 일부 데이터로 채워야 합니다. 영화 요약이 포함된 작은 데모 세트를 만들었습니다.

Pinecone을 사용하려면 `pinecone` 패키지를 설치해야 하며 API 키와 환경이 있어야 합니다. [설치 지침](https://docs.pinecone.io/docs/quickstart)을 참조하세요.

**참고:** self-query retriever를 사용하려면 `lark` 패키지를 설치해야 합니다.
