...
Ollama는 모델 가중치, 구성 및 데이터를 Modelfile로 정의된 단일 패키지로 번들화합니다.

이 도구는 GPU 사용을 포함한 설정 및 구성 세부 사항을 최적화합니다.

지원되는 모델 및 모델 변형의 전체 목록은 [Ollama 모델 라이브러리](https://github.com/jmorganca/ollama#model-library)를 참조하십시오.

## 설정

먼저, 로컬 Ollama 인스턴스를 설정하고 실행하려면 [이 지침](https://github.com/jmorganca/ollama)을 따르십시오:

- [다운로드](https://ollama.ai/download) 및 지원되는 플랫폼에 Ollama를 설치하십시오(Windows Subsystem for Linux 포함)
- `ollama pull <name-of-model>` 명령을 사용하여 사용할 수 있는 LLM 모델을 가져옵니다.
  - [모델 라이브러리](https://ollama.ai/library)에서 사용 가능한 모델 목록을 확인하십시오.
  - 예: `ollama pull llama3`
- 이 명령은 기본 태그가 지정된 모델 버전을 다운로드합니다. 일반적으로 기본 설정은 최신의 가장 작은 크기의 매개변수 모델을 가리킵니다.

> Mac에서는 모델이 `~/.ollama/models`에 다운로드됩니다.
>
> Linux(또는 WSL)에서는 모델이 `/usr/share/ollama/.ollama/models`에 저장됩니다.

- 관심 있는 모델의 정확한 버전을 지정하려면 `ollama pull vicuna:13b-v1.5-16k-q4_0`과 같이 합니다(이 경우 [Vicuna 모델의 다양한 태그](https://ollama.ai/library/vicuna/tags)를 참조하십시오).
- 모든 가져온 모델을 보려면 `ollama list`를 사용하십시오.
- 명령줄에서 모델과 직접 대화하려면 `ollama run <name-of-model>`을 사용하십시오.
- 더 많은 명령어를 보려면 [Ollama 문서](https://github.com/jmorganca/ollama)를 참조하십시오. 터미널에서 `ollama help`를 실행하여 사용 가능한 명령어를 확인할 수도 있습니다.

## 사용법

지원되는 매개변수의 전체 목록은 [API 참조 페이지](https://api.python.langchain.com/en/latest/llms/langchain.llms.ollama.Ollama.html)에서 확인할 수 있습니다.

LLaMA `chat` 모델(e.g., `ollama pull llama3`)을 사용하는 경우 `ChatOllama` 인터페이스를 사용할 수 있습니다.

여기에는 시스템 메시지 및 사용자 입력에 대한 [특별 토큰](https://huggingface.co/blog/llama2#how-to-prompt-llama-2)이 포함됩니다.

## 모델과의 상호작용

가져온 로컬 모델과 상호작용하는 몇 가지 방법이 있습니다.

### 터미널에서 직접:

- 모든 로컬 모델은 자동으로 `localhost:11434`에서 제공됩니다.
- `ollama run <name-of-model>`을 실행하여 명령줄에서 직접 상호작용을 시작합니다.

### API를 통해

API 엔드포인트에 `application/json` 요청을 보내서 상호작용합니다.
...
모든 엔드포인트에 대한 정보는 Ollama [API 문서](https://github.com/jmorganca/ollama/blob/main/docs/api.md)를 참조하십시오.

### LangChain을 통해

LangChain 애플리케이션에서 `ChatOllama` 채팅 모델을 사용하는 일반적인 기본 예제를 참조하십시오.

```python
# LangChain은 많은 다른 채팅 모델을 지원합니다. 여기서는 Ollama를 사용합니다.

from langchain_community.chat_models import ChatOllama
from langchain_core.output_parsers import StrOutputParser
from langchain_core.prompts import ChatPromptTemplate

# 더 많은 선택적 매개변수를 지원합니다. 최신 지원 매개변수를 보려면

# `ChatOllama(...)` 클래스에 마우스를 올리십시오.

llm = ChatOllama(model="llama3")
prompt = ChatPromptTemplate.from_template("Tell me a short joke about {topic}")

# LangChain Expressive Language 체인 문법을 사용합니다.

# LCEL에 대해 자세히 알아보려면

# /docs/expression_language/why를 참조하십시오.

chain = prompt | llm | StrOutputParser()

# 간결성을 위해 응답은 터미널에 출력됩니다.

# 애플리케이션을 배포하려면 LangServe를 사용하십시오.
...
비동기 지원 스트리밍 예제는 다음과 같습니다. 위에서 생성한 체인을 통해 가능합니다.
...
체인이 생성될 때 사용할 수 있는 다른 인터페이스에 대해서는 [LangChain Expressive Language (LCEL) Interface](/docs/expression_language/interface)를 참조하십시오.

## 소스에서 빌드하기

소스에서 빌드하는 최신 지침은 [소스에서 빌드하기](https://github.com/jmorganca/ollama?tab=readme-ov-file#building)에 관한 Ollama 문서를 참조하십시오.

## 추출

최신 버전의 Ollama를 사용하고 [`format`](https://github.com/jmorganca/ollama/blob/main/docs/api.md#json-mode) 플래그를 제공합니다. `format` 플래그는 모델이 응답을 JSON 형식으로 생성하도록 강제합니다.

> **참고:** 편리함을 위해 실험적인 [OllamaFunctions](/docs/integrations/chat/ollama_functions) 래퍼도 시도해 볼 수 있습니다.
...
Ollama는 [bakllava](https://ollama.ai/library/bakllava) 및 [llava](https://ollama.ai/library/llava)와 같은 멀티모달 LLM을 지원합니다.

`tags`가 있는 모델의 전체 버전을 확인하려면 [Llava](https://ollama.ai/library/llava/tags)를 참조하십시오.

원하는 LLM을 `ollama pull bakllava`를 통해 다운로드하십시오.

멀티모달을 지원하려면 최신 버전의 Ollama로 업데이트하십시오.

아래는 ChatOllama 멀티모달 지원을 사용하는 전형적인 예입니다:
...
def convert_to_base64(pil_image):
    ...
    """
    buffered = BytesIO()
    pil_image.save(buffered, format="JPEG")  # 필요에 따라 형식을 변경할 수 있습니다.
