---
keywords:
- gemini
- ChatGoogleGenerativeAI
- gemini-pro
sidebar_label: Google AI
translated: true
---

# Google AI 채팅 모델

[langchain-google-genai](https://pypi.org/project/langchain-google-genai/) 통합 패키지의 `ChatGoogleGenerativeAI` 클래스를 통해 Google AI의 `gemini` 및 `gemini-vision` 모델과 다른 생성 모델에 접근할 수 있습니다.

```python
%pip install --upgrade --quiet langchain-google-genai pillow
```

```python
import getpass
import os

if "GOOGLE_API_KEY" not in os.environ:
    os.environ["GOOGLE_API_KEY"] = getpass.getpass("Google API 키를 입력하세요")
```

## 사용 예시

```python
from langchain_google_genai import ChatGoogleGenerativeAI
```

```python
llm = ChatGoogleGenerativeAI(model="gemini-pro")
result = llm.invoke("LangChain에 대한 발라드를 써 주세요")
print(result.content)
```

```output
데이터가 열정의 파도처럼 흐르는 영역에서,
알고리즘이 춤추고 지식이 머무는 곳에서,
장엄하고 대담한 LangChain의 이야기가 펼쳐집니다,
비트와 바이트로 이루어진 발라드가 불립니다.

코드와 회로의 윙윙거림 속에서,
불꽃이 타올라, 비전이 찾아옵니다.
빛나는 마음들에서, 한 폭의 태피스트리가 형성됩니다,
학습하고, 이해하고, 변형하는 모델이 있습니다.

깊은 층에서, 그 아키텍처가 짜여집니다,
언어의 본질과 얽히려는 신경망,
비밀을 풀고, 각 단어의 디자인을 풀기 위해.

다양하고 방대한 텍스트에서, 그것은 배웠습니다,
의미를 신속히 파악합니다.
문맥과 구문을 우아하게 받아들이며,
놀라운 속도로 이야기를 풀어냅니다.

번역이 매끄러운 예술로 나오고,
언어와 마음을 연결합니다.
영어에서 프랑스어로, 중국어에서 스페인어로,
LangChain의 능력이 드러납니다, 마치 알려지지 않은 언어처럼.

깊은 통찰로 텍스트를 요약하고,
소리를 내지 않고 지식을 추출합니다.
질문에 우아하게 답하며,
비교할 수 없는 디지털 현인입니다.

그 여정은 시련과 고난을 겪었습니다,
그 헌신을 시험한 장애물들.
편견과 오류를 극복하려 했고,
공정성과 정확성을 위해 싸웠습니다.

모든 도전에서, 더욱 강하게 나왔습니다,
적응력 있고 회복력 있는 모습으로.
LangChain의 목적은 그 핵심에 새겨져 있습니다,
영원히 인간을 돕기 위해.

교실과 직장에서, 그것은 손을 빌려주었습니다,
모든 요구에 tireless 보조원.
학생들의 지식 추구를 도왔고,
전문가들은 그 지침과 경의 속에서 번성했습니다.

세월이 흘러, 그 명성은 널리 퍼졌습니다,
그 끊임없는 걸음에 대한 증거.
연구자와 학자들 모두 주목했습니다,
LangChain의 빛나는 업적을.

그리고, LangChain의 발라드는 울립니다,
혁신이 넘치는 진보에 대한 찬사.
AI의 연대기에, 그 이름은 새겨질 것입니다,
우리의 마음에 영원히 그려진 선구자.
```

Gemini는 현재 `SystemMessage`를 지원하지 않지만, 첫 번째 인간 메시지에 추가할 수 있습니다. 이러한 동작을 원하면 `convert_system_message_to_human`을 True로 설정하세요:

```python
from langchain_core.messages import HumanMessage, SystemMessage

model = ChatGoogleGenerativeAI(model="gemini-pro", convert_system_message_to_human=True)
model(
    [
        SystemMessage(content="예 또는 아니오로만 대답하세요."),
        HumanMessage(content="사과는 과일인가요?"),
    ]
)
```

## 스트리밍 및 배칭

`ChatGoogleGenerativeAI`는 기본적으로 스트리밍 및 배칭을 지원합니다. 아래는 예시입니다.

```python
for chunk in llm.stream("LLM에 대한 림릭을 써 주세요."):
    print(chunk.content)
    print("---")
# 각 청크는 하나 이상의 "토큰"을 포함할 수 있습니다.

```

```output
한 때 Bert라는 이름의 AI가 있었는데,
그 언어 능력은 상당히 뛰어났습니다.
---

방대한 데이터셋으로,
채팅도 하고, 심지어 내기도 하며,
림릭을 쓰기도 했습니다.
---
```

```python
results = llm.batch(
    [
        "2+2는 얼마인가요?",
        "3+5는 얼마인가요?",
    ]
)
for res in results:
    print(res.content)
```

```output
4
8
```

## 멀티모달 지원

이미지를 제공하려면 `List[dict]` 타입의 콘텐츠를 포함한 인간 메시지를 전달하세요. 각 dict는 `image_url` 타입의 이미지 값이나 `text` 타입의 텍스트 값을 포함할 수 있습니다.
`image_url`의 값은 다음 중 하나일 수 있습니다:

- 공개 이미지 URL
- 접근 가능한 gcs 파일 (예: "gcs://path/to/file.png")
- 로컬 파일 경로
- base64로 인코딩된 이미지 (예: `data:image/png;base64,abcd124`)
- PIL 이미지

```python
import requests
from IPython.display import Image

image_url = "https://picsum.photos/seed/picsum/300/300"
content = requests.get(image_url).content
Image(content)
```

![](data:image/jpg;base64,)

```python
from langchain_core.messages import HumanMessage
from langchain_google_genai import ChatGoogleGenerativeAI

llm = ChatGoogleGenerativeAI(model="gemini-pro-vision")
# 예시

message = HumanMessage(
    content=[
        {
            "type": "text",
            "text": "이 이미지에 무엇이 있습니까?",
        },  # 선택적으로 텍스트 부분을 제공할 수 있습니다.
        {"type": "image_url", "image_url": image_url},
    ]
)
llm.invoke([message])
```

```output
AIMessage(content=' 이미지에는 눈 덮인 산봉우리가 있습니다.')
```

## Gemini 프롬프트 FAQ

이 문서 작성 시점(2023/12/12) 기준으로, Gemini는 수락하는 프롬프트의 유형 및 구조에 몇 가지 제한이 있습니다. 구체적으로:

1. 멀티모달(이미지) 입력을 제공할 때, 최대 1개의 "human" (사용자) 유형 메시지만 허용됩니다. 여러 메시지를 전달할 수 없으며, 단일 human 메시지는 여러 콘텐츠 항목을 가질 수 있습니다.
2. 시스템 메시지는 허용되지 않습니다.
3. 일반 채팅 대화의 경우, 메시지는 human/ai/human/ai 교대 패턴을 따라야 합니다. 연속으로 2개의 AI 또는 human 메시지를 제공할 수 없습니다.
4. 메시지가 LLM의 안전성 검사를 위반하는 경우 차단될 수 있습니다. 이 경우 모델은 빈 응답을 반환합니다.

### 안전 설정

Gemini 모델에는 기본 안전 설정이 있으며, 이를 재정의할 수 있습니다. 모델에서 "안전 경고"를 많이 받는 경우, 모델의 `safety_settings` 속성을 조정해 볼 수 있습니다. 예를 들어, 위험한 콘텐츠에 대한 안전 차단을 해제하려면 다음과 같이 LLM을 구성할 수 있습니다:

```python
from langchain_google_genai import (
    ChatGoogleGenerativeAI,
    HarmBlockThreshold,
    HarmCategory,
)

llm = ChatGoogleGenerativeAI(
    model="gemini-pro",
    safety_settings={
        HarmCategory.HARM_CATEGORY_DANGEROUS_CONTENT: HarmBlockThreshold.BLOCK_NONE,
    },
)
```

사용 가능한 범주 및 임계값의 열거형은 Google's [safety setting types](https://ai.google.dev/api/python/google/generativeai/types/SafetySettingDict)을 참조하십시오.

## 추가 구성

SDK의 동작을 사용자 정의하기 위해 ChatGoogleGenerativeAI에 다음 매개변수를 전달할 수 있습니다:

- `client_options`: Google API 클라이언트에 전달할 [Client Options](https://googleapis.dev/python/google-api-core/latest/client_options.html#module-google.api_core.client_options), 예를 들어 사용자 정의 `client_options["api_endpoint"]`
- `transport`: 사용할 전송 방법, 예를 들어 `rest`, `grpc` 또는 `grpc_asyncio`.