...
이 노트북은 [Cohere 채팅 모델](https://cohere.com/chat)을 사용하는 방법을 다룹니다.

모든 속성과 메서드에 대한 자세한 문서는 [API 참조](https://api.python.langchain.com/en/latest/chat_models/langchain_community.chat_models.cohere.ChatCohere.html)를 참조하십시오.

## 설정

통합은 `langchain-cohere` 패키지에 있습니다. 다음 명령어로 설치할 수 있습니다:

```bash
pip install -U langchain-cohere
```

[Cohere API 키](https://cohere.com/)를 얻어 `COHERE_API_KEY` 환경 변수에 설정해야 합니다:
...
최상의 관측 가능성을 위해 [LangSmith](https://smith.langchain.com/)를 설정하는 것도 도움이 됩니다 (필수는 아님).
...
ChatCohere는 모든 [ChatModel](/docs/modules/model_io/chat/) 기능을 지원합니다:
...
사용자 입력을 쉽게 구조화하기 위해 프롬프트 템플릿과 쉽게 결합할 수 있습니다. 이를 위해 [LCEL](/docs/expression_language)을 사용할 수 있습니다.
