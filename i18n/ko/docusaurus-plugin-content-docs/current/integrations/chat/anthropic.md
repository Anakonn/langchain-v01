...
이 노트북은 Anthropic 채팅 모델을 시작하는 방법을 다룹니다.

## 설정

설정 지침은 [Anthropic 플랫폼 페이지](/ko/docs/integrations/platforms/anthropic)의 설치 및 환경 설정 섹션을 참조하십시오.
...
[Anthropic](https://console.anthropic.com/settings/keys) API 키를 얻고 `ANTHROPIC_API_KEY` 환경 변수를 설정해야 합니다:
...
제공된 코드는 ANTHROPIC_API_KEY가 환경 변수에 설정되어 있음을 가정합니다. API 키를 수동으로 지정하고 다른 모델을 선택하려면 다음 코드를 사용할 수 있습니다:

```python
chat = ChatAnthropic(temperature=0, api_key="YOUR_API_KEY", model_name="claude-3-opus-20240229")
```

이 데모에서는 Claude 3 Opus 모델을 사용할 것이며, `claude-3-sonnet-20240229`을 사용하여 Sonnet 모델의 출시 버전을 사용할 수도 있습니다.

모델 비교 문서는 [여기](https://docs.anthropic.com/claude/docs/models-overview#model-comparison)에서 확인할 수 있습니다.
...
## `ChatAnthropic`는 비동기 및 스트리밍 기능도 지원합니다:
...
Anthropic의 [도구 호출, 또는 도구 사용, API](https://docs.anthropic.com/claude/docs/functions-external-tools)를 사용하면 모델이 호출할 도구를 정의할 수 있습니다. 이는 도구 사용 체인 및 에이전트를 구축하거나 모델에서 구조화된 출력을 얻는 데 매우 유용합니다.

:::note

Anthropic의 도구 호출 기능은 아직 베타입니다.

:::

### bind_tools()

`ChatAnthropic.bind_tools`를 사용하면 Pydantic 클래스, dict 스키마, LangChain 도구 또는 함수까지 쉽게 모델에 도구로 전달할 수 있습니다. 내부적으로 이것들은 다음과 같은 Anthropic 도구 스키마로 변환되어 모든 모델 호출 시 전달됩니다:
...
출력 메시지 내용은 텍스트 블록과 도구 사용 블록을 포함한 목록입니다:
...
도구 호출도 `tool_calls`에 표준화된 모델 무관 형식으로 추출됩니다:
...
ChatAnthropic 모델 출력은 항상 단일 AI 메시지로, 단일 문자열 또는 콘텐츠 블록 목록이 포함될 수 있습니다. 콘텐츠 블록은 텍스트 블록 또는 도구 사용 블록이 될 수 있으며, 여러 개가 있을 수 있고 섞여 있을 수 있습니다.

:::

### 도구 호출 구문 분석

`langchain_anthropic.output_parsers.ToolsOutputParser`를 사용하면 Anthropic AI 메시지의 도구 호출을 Pydantic 객체로 쉽게 구문 분석할 수 있습니다:
...
[BaseChatModel.with_structured_output 인터페이스](/docs/modules/model_io/chat/structured_output)를 사용하면 채팅 모델에서 구조화된 출력을 쉽게 얻을 수 있습니다. `ChatAnthropic.with_structured_output`을 사용하여 모델이 특정 형식으로 출력을 보다 신뢰성 있게 반환하도록 할 수 있습니다:
...
첫 번째는 단일 GetWeather 호출만 반환하는 반면, 두 번째 접근 방식은 목록을 반환합니다.

### 도구 결과를 모델에 전달

적절한 `tool_call_id`s로 `ToolMessage`s를 사용하여 도구 결과를 모델에 전달할 수 있습니다:
...
Anthropic은 현재 스트리밍 도구 호출을 지원하지 않습니다. 스트리밍을 시도하면 단일 최종 메시지가 생성됩니다.
...
Anthropic의 Claude-3 모델은 이미지 및 텍스트 입력과 호환됩니다. 다음과 같이 사용할 수 있습니다:
