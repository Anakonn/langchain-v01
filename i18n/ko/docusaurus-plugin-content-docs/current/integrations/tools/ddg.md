...
이 노트북은 duck-duck-go 검색 구성 요소 사용 방법을 다룹니다.
...
추가 정보(예: 링크, 소스)를 얻으려면 `DuckDuckGoSearchResults()`를 사용하세요.
...
사용자 정의 ``DuckDuckGoSearchAPIWrapper``를 ``DuckDuckGoSearchResults``에 직접 전달할 수도 있습니다. 따라서 검색 결과에 대한 더 많은 제어가 가능합니다.
