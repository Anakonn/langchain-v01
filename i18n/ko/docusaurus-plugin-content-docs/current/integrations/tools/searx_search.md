...
이 노트북은 자체 호스팅된 `SearxNG` 검색 API를 사용하여 웹을 검색하는 방법을 설명합니다.

`Searx API` 매개변수에 대한 자세한 내용은 [이 링크](https://docs.searxng.org/dev/search_api.html)를 확인할 수 있습니다.
...
일부 엔진의 경우 직접 `answer`를 사용할 수 있는 경우 래퍼가 전체 검색 결과 목록 대신 답변을 출력합니다. 모든 결과를 얻으려면 래퍼의 `results` 메서드를 사용할 수 있습니다.
...
SearxNG는 [135개의 검색 엔진](https://docs.searxng.org/user/configured_engines.html)을 지원합니다. Searx 래퍼를 임의의 이름 지정 매개변수로 사용자 정의할 수도 있으며, 이 매개변수는 Searx 검색 API에 전달됩니다. 아래 예에서는 Searx 검색 API의 사용자 정의 검색 매개변수를 더 흥미롭게 사용할 것입니다.

이 예에서는 `engines` 매개변수를 사용하여 Wikipedia를 검색할 것입니다.
...
`language` 매개변수와 같은 다른 Searx 매개변수 전달하기
...
이 예에서는 `categories` 매개변수를 사용하여 과학 논문을 찾고 `time_range`(모든 엔진이 시간 범위 옵션을 지원하는 것은 아님)로 결과를 제한할 것입니다.

또한 메타데이터를 포함하는 구조화된 방식으로 결과를 얻고 싶습니다. 이를 위해 래퍼의 `results` 메서드를 사용할 것입니다.
...
이 예에서는 `it` 카테고리에서 `large language models`를 검색합니다. 그런 다음 결과에서 github 출처를 필터링합니다.
...
또한 직접 `github` 및 기타 소스 포지를 검색할 수 있습니다.
