...
Python용 ArcGIS API `arcgis`를 설치해야 하며, 선택적으로 `bs4.BeautifulSoup`도 설치할 수 있습니다.

인증된 데이터 로드를 위해 `arcgis.gis.GIS` 객체를 사용할 수 있으며, 공개 데이터를 접근하려면 빈 상태로 둘 수 있습니다.
...
피처 지오메트리를 가져오고 싶다면 `return_geometry` 키워드를 사용할 수 있습니다.

각 문서의 지오메트리는 해당 메타데이터 사전에 저장됩니다.
