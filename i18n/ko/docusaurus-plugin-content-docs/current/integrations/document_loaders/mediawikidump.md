...
>[MediaWiki XML 덤프](https://www.mediawiki.org/wiki/Manual:Importing_XML_dumps)에는 위키의 내용(모든 개정판의 위키 페이지)이 포함되어 있지만 사이트 관련 데이터는 포함되어 있지 않습니다. XML 덤프에는 사용자 계정, 이미지, 편집 로그 등 위키 데이터베이스의 전체 백업이 포함되어 있지 않습니다.

이 문서에서는 MediaWiki XML 덤프 파일을 우리가 사용할 수 있는 문서 형식으로 로드하는 방법을 다룹니다.

`mediawiki-utilities`의 `mwxml`과 `earwig`의 `mwparserfromhell`을 사용합니다.
