...
>[Etherscan](https://docs.etherscan.io/)는 이더리움, 분산형 스마트 계약 플랫폼의 선도적인 블록체인 탐색기, 검색, API 및 분석 플랫폼입니다.

## 개요

`Etherscan` 로더는 `Ethereum Mainnet`의 특정 계정에 대한 거래 내역을 로드하기 위해 `Etherscan API`를 사용합니다.

`Etherscan api key`가 필요합니다. 무료 api 키는 초당 5회 호출 할당량이 있습니다.

로더는 다음 6가지 기능을 지원합니다:

* Ethereum Mainet의 특정 계정에 대한 일반 거래 내역 검색
* Ethereum Mainet의 특정 계정에 대한 내부 거래 내역 검색
* Ethereum Mainet의 특정 계정에 대한 erc20 거래 내역 검색
* Ethereum Mainet의 특정 계정에 대한 erc721 거래 내역 검색
* Ethereum Mainet의 특정 계정에 대한 erc1155 거래 내역 검색
* Ethereum Mainet의 특정 계정에 대한 이더리움 잔액 검색

계정에 해당 거래 내역이 없는 경우, 로더는 하나의 문서로 구성된 목록을 반환합니다. 문서의 내용은 ''입니다.

다음과 같은 필터를 로더에 전달하여 위에서 언급한 다양한 기능에 액세스할 수 있습니다:
...
* "erc721_transaction"
* "erc1155_transaction"
필터의 기본값은 normal_transaction입니다.

질문이 있으시면 [Etherscan API 문서](https://etherscan.io/tx/0x0ffa32c787b1398f44303f731cb06678e086e4f82ce07cebf75e99bb7c079c77)를 참조하거나 i@inevitable.tech로 연락주시기 바랍니다.

거래 내역과 관련된 모든 기능은 Etherscan의 제한으로 인해 최대 1000개의 내역으로 제한됩니다. 필요한 거래 내역을 찾기 위해 다음 매개변수를 사용할 수 있습니다:

* offset: 기본값 20. 한 번에 20개의 거래 내역을 표시합니다.
* page: 기본값 1. 이것은 페이지 매김을 제어합니다.
* start_block: 기본값 0. 거래 내역은 0 블록부터 시작합니다.
* end_block: 기본값 99999999. 거래 내역은 99999999 블록까지 포함됩니다.
* sort: "desc" 또는 "asc". 기본값은 "desc"로 최신 거래 내역을 가져옵니다.
