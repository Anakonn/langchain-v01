...
>[GitBook](https://docs.gitbook.com/)은 제품, 내부 지식 베이스 및 API 등 모든 것을 문서화할 수 있는 현대적인 문서 플랫폼입니다.

이 노트북은 `GitBook`의 모든 페이지 데이터를 가져오는 방법을 보여줍니다.
...
이 작업을 수행하려면 GitbookLoader를 루트 경로(`https://docs.gitbook.com`의 예)로 초기화하고 `load_all_paths`를 `True`로 설정해야 합니다.
