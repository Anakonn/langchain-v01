...
>[LLM용 MLflow 배포](https://www.mlflow.org/docs/latest/llms/deployments/index.html)는 OpenAI와 Anthropic과 같은 다양한 대규모 언어 모델(LLM) 공급업체를 조직 내에서 사용하고 관리하는 것을 간소화하는 강력한 도구입니다. 이는 특정 LLM 관련 요청을 처리하기 위한 통합 엔드포인트를 제공하여 이러한 서비스와의 상호 작용을 단순화합니다.

## 설치 및 설정

MLflow 배포 종속성을 사용하여 `mlflow`를 설치합니다:
...
> 이 모듈은 langchain 맛의 다변량 LangChain 모델과 pyfunc 맛의 단변량 LangChain
> 모델을 내보냅니다.

자세한 내용은 [API 문서와 예제](https://www.mlflow.org/docs/latest/llms/langchain/index.html)를 참조하세요.
