...
이 페이지에서는 LangChain 내에서 Prediction Guard 생태계를 사용하는 방법을 다룹니다.
설치 및 설정, 그리고 특정 Prediction Guard 래퍼에 대한 참조로 구성되어 있습니다.

## 설치 및 설정

- `pip install predictionguard`로 Python SDK를 설치하세요.
- Prediction Guard 액세스 토큰을 얻고(여기에 설명되어 있음: https://docs.predictionguard.com/) 환경 변수 `PREDICTIONGUARD_TOKEN`으로 설정하세요.

## LLM 래퍼

Prediction Guard LLM 래퍼가 존재하며, 다음과 같이 액세스할 수 있습니다:
...
LLM을 초기화할 때 Prediction Guard 모델 이름을 인수로 제공할 수 있습니다:
...
마지막으로 "output" 인수를 제공하여 LLM의 출력을 구조화하거나 제어할 수 있습니다:
...
Prediction Guard 래퍼를 사용한 기본 LLM 체인:
