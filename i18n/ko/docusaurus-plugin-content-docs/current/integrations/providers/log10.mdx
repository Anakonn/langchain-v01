...
Log10은 [오픈 소스](https://github.com/log10-io/log10) 프록시리스 LLM 데이터 관리 및 애플리케이션 개발 플랫폼으로, Langchain 호출을 로깅, 디버깅 및 태깅할 수 있습니다.

## 빠른 시작

1. [log10.io](https://log10.io)에서 무료 계정을 만드세요.
2. 설정 및 조직 탭에서 `LOG10_TOKEN`과 `LOG10_ORG_ID`를 환경 변수로 추가하세요.
3. `LOG10_URL=https://log10.io`와 `OPENAI_API_KEY` 또는 `ANTHROPIC_API_KEY`와 같은 기존 LLM API 키를 환경 변수로 추가하세요.

## Langchain에 Log10 데이터 관리 활성화하기

Log10과의 통합은 아래와 같이 간단한 `log10_callback` 통합으로 이루어집니다:
...
[자세한 내용 + 스크린샷](https://log10.io/docs/observability/logs) 자체 호스팅 로그에 대한 지침 포함
...
OpenAI 호출과 Langchain LLM 호출을 혼합할 수도 있습니다:
