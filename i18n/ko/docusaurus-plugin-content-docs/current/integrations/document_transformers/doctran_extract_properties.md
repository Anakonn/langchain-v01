...
[Doctran](https://github.com/psychic-api/doctran) 라이브러리를 사용하면 OpenAI의 함수 호출 기능을 이용하여 문서의 특정 메타데이터를 추출할 수 있습니다.

문서에서 메타데이터를 추출하면 다음과 같은 다양한 작업에 도움이 됩니다:

* **분류:** 문서를 다양한 범주로 분류
* **데이터 마이닝:** 데이터 분석에 사용할 수 있는 구조화된 데이터 추출
* **스타일 전환:** 사용자 입력에 더 잘 맞추기 위해 텍스트 작성 방식 변경, 벡터 검색 결과 개선
...
문서에서 속성을 추출하면 메타데이터가 포함된 새 문서로 결과가 반환됩니다.
