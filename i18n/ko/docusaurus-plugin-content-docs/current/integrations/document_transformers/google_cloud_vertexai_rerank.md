...
> [Vertex Search Ranking API](https://cloud.google.com/generative-ai-app-builder/docs/ranking)는 [Vertex AI Agent Builder](https://cloud.google.com/generative-ai-app-builder/docs/builder-apis)의 독립형 API 중 하나입니다. 문서 목록을 받아 문서의 관련성에 따라 다시 순위를 매깁니다. 임베딩과 달리 문서와 쿼리의 의미적 유사성만 보는 것이 아니라, 순위 API를 통해 문서가 주어진 쿼리에 얼마나 잘 답변하는지에 대한 정확한 점수를 얻을 수 있습니다. 순위 API는 초기 후보 문서 집합을 검색한 후 검색 결과의 품질을 향상시키는 데 사용할 수 있습니다.

>순위 API는 상태가 없기 때문에 API를 호출하기 전에 문서를 인덱싱할 필요가 없습니다. 쿼리와 문서만 전달하면 됩니다. 이를 통해 모든 문서 검색기에서 문서를 다시 순위 매기는 데 적합합니다.

>자세한 내용은 [문서 순위 매기기 및 다시 순위 매기기](https://cloud.google.com/generative-ai-app-builder/docs/ranking)를 참조하세요.
...
이 예제에서는 [Google 위키 페이지](https://en.wikipedia.org/wiki/Google)를 사용하여 Vertex Ranking API의 작동 방식을 보여줍니다.

`load -> split -> embed data`의 표준 파이프라인을 사용합니다.

임베딩은 [Vertex Embeddings API](https://cloud.google.com/vertex-ai/generative-ai/docs/embeddings/get-text-embeddings#supported_models) 모델 - `textembedding-gecko@003`을 사용하여 생성됩니다.
...
`basic_retriever`와 `retriever_with_reranker`에 동일한 쿼리를 실행하고 검색된 문서를 비교해 보겠습니다.

Ranking API는 `basic_retriever`의 출력을 입력으로 받아 Ranking API에 전달합니다.

Ranking API는 순위의 품질을 향상시키고 각 레코드의 쿼리 관련성을 나타내는 점수를 결정하는 데 사용됩니다.

순위 없음과 순위 있음 문서의 차이를 확인할 수 있습니다. Ranking API는 의미적으로 가장 관련성 있는 문서를 LLM의 컨텍스트 창 맨 위로 이동시켜 더 나은 답변과 추론을 할 수 있게 합니다.
...
몇 개의 재순위 문서를 살펴보겠습니다. 검색기가 여전히 관련 Langchain 유형 [문서](https://api.python.langchain.com/en/latest/documents/langchain_core.documents.base.Document.html)를 반환하지만, 메타데이터 필드에서 Ranking API의 `relevance_score`도 받을 수 있습니다.
...
이것은 Vertex Ranking API를 사용하여 재순위를 수행하는 방법을 보여주는 완전한 RAG 체인의 예입니다.
