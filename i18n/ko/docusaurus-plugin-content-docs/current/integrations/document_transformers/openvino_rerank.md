...
[OpenVINO™](https://github.com/openvinotoolkit/openvino)는 AI 추론을 최적화하고 배포하기 위한 오픈 소스 툴킷입니다. OpenVINO™ 런타임은 x86 및 ARM CPU, Intel GPU를 포함한 다양한 하드웨어 [장치](https://github.com/openvinotoolkit/openvino?tab=readme-ov-file#supported-hardware-matrix)를 지원합니다. 컴퓨터 비전, 자동 음성 인식, 자연어 처리 및 기타 일반적인 작업에서 딥 러닝 성능을 높일 수 있습니다.

Hugging Face 재순위화 모델은 ``OpenVINOReranker`` 클래스를 통해 OpenVINO로 지원될 수 있습니다. Intel GPU가 있는 경우 `model_kwargs={"device": "GPU"}`를 지정하여 추론을 실행할 수 있습니다.
...
2023년 국정 연설(chunks)을 초기화하고 저장하는 간단한 벡터 저장소 검색기를 시작해 보겠습니다. 검색기를 설정하여 많은 수(20)의 문서를 검색할 수 있습니다.
...
이제 `OpenVINOReranker`를 압축기로 사용하여 기본 검색기를 `ContextualCompressionRetriever`로 래핑해 보겠습니다.
...
재순위화 후, 상위 4개 문서는 기본 검색기가 검색한 상위 4개 문서와 다릅니다.
...
``OVModelForSequenceClassification``을 사용하여 재순위화 모델을 OpenVINO IR 형식으로 내보내고 로컬 폴더에서 모델을 로드할 수 있습니다.
