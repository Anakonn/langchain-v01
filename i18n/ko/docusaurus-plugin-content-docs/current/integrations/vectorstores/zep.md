...
> 대화 기록을 회상하고 이해하며 데이터를 추출할 수 있습니다. 맞춤형 AI 경험을 제공합니다.

> [Zep](https://www.getzep.com)은 AI 어시스턴트 앱을 위한 장기 메모리 서비스입니다.
> Zep을 통해 AI 어시스턴트가 과거 대화를 기억할 수 있게 하고, 환각, 지연, 비용을 줄일 수 있습니다.

> Zep Cloud에 관심이 있으신가요? [Zep Cloud 설치 가이드](https://help.getzep.com/sdks)와 [Zep Cloud Vector Store 예제](https://help.getzep.com/langchain/examples/vectorstore-example)를 참고하세요.
...
아래 예제에서는 Zep의 자동 임베딩 기능을 사용하여 문서를 Zep 서버에 자동으로 임베딩합니다.

## 참고사항

- 이 예제에서는 Zep의 비동기 인터페이스를 사용합니다. 동기 인터페이스를 사용하려면 메서드 이름에서 `a`를 제거하세요.
- `Embeddings` 인스턴스를 전달하면 Zep이 자동 임베딩 대신 이를 사용합니다. 문서 컬렉션의 `isAutoEmbedded`를 `false`로 설정해야 합니다.
- `isAutoEmbedded`를 `false`로 설정한 경우 `Embeddings` 인스턴스를 전달해야 합니다.
...
Zep은 하드웨어 가속 MMR 재정렬 기능을 제공합니다.
...
메타데이터 필터를 사용하여 결과를 좁힙니다. 먼저 "셜록 홈즈의 모험"이라는 다른 책을 로드합니다.
...
두 책의 결과가 모두 표시됩니다. `source` 메타데이터를 확인하세요.
