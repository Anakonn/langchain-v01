...
# Google Vertex AI 벡터 검색

이 노트북은 `Google Cloud Vertex AI 벡터 검색` 벡터 데이터베이스와 관련된 기능을 사용하는 방법을 보여줍니다.

> [Google Vertex AI 벡터 검색](https://cloud.google.com/vertex-ai/docs/vector-search/overview), 이전에는 Vertex AI Matching Engine으로 알려져 있었던 서비스는 업계 최고 수준의 고확장성 저지연 벡터 데이터베이스를 제공합니다. 이러한 벡터 데이터베이스는 일반적으로 벡터 유사성 매칭 또는 근사 최근접 이웃(ANN) 서비스로 알려져 있습니다.

**참고**: Langchain API에는 이미 생성된 엔드포인트와 배포된 인덱스가 필요합니다. 인덱스 생성 시간은 최대 1시간이 걸릴 수 있습니다.

> 인덱스를 생성하는 방법은 [인덱스 생성 및 엔드포인트에 배포하기](#create-index-and-deploy-it-to-an-endpoint) 섹션을 참조하세요.
이미 배포된 인덱스가 있다면 [텍스트에서 벡터 저장소 만들기](#create-vector-store-from-texts)로 건너뛰세요.

## 인덱스 생성 및 엔드포인트에 배포하기

- 이 섹션에서는 새 인덱스를 생성하고 엔드포인트에 배포하는 방법을 보여줍니다.
...
### [VertexAIEmbeddings](https://python.langchain.com/docs/integrations/text_embedding/google_vertex_ai_palm/)를 임베딩 모델로 사용하기
...
**참고:** 인덱스를 생성할 때 "BATCH_UPDATE" 또는 "STREAM_UPDATE" 중 하나의 "index_update_method"를 지정해야 합니다.
> 배치 인덱스는 일정 기간 동안 저장된 데이터를 일괄적으로 업데이트할 때 사용합니다. 예를 들어 주간 또는 월간 처리 시스템입니다. 스트리밍 인덱스는 새 데이터가 데이터 저장소에 추가될 때마다 인덱스 데이터를 업데이트하고 싶을 때 사용합니다. 예를 들어 새로운 재고를 온라인에 신속하게 표시하고 싶은 서점 시스템입니다. 어떤 유형을 선택할지는 중요하며, 설정 및 요구 사항이 다릅니다.

자세한 내용은 [공식 문서](https://cloud.google.com/vertex-ai/docs/vector-search/create-manage-index#create-index-batch)를 참조하세요.
...
### 선택 사항: 데이터 저장소에 청크를 저장하는 벡터 저장소를 만들 수도 있습니다.
