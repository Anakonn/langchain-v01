...
> [Annoy](https://github.com/spotify/annoy) (`Approximate Nearest Neighbors Oh Yeah`)는 주어진 쿼리 포인트에 가까운 공간 내 포인트를 검색하는 Python 바인딩이 있는 C++ 라이브러리입니다. 또한 많은 프로세스가 동일한 데이터를 공유할 수 있도록 메모리에 mmapped되는 대용량 읽기 전용 파일 기반 데이터 구조를 생성합니다.

이 노트북은 `Annoy` 벡터 데이터베이스와 관련된 기능 사용 방법을 보여줍니다.

:::note
주의: Annoy는 읽기 전용입니다 - 인덱스가 한 번 구축되면 더 이상 임베딩을 추가할 수 없습니다!
새로운 항목을 점진적으로 VectorStore에 추가하려면 대안을 선택하는 것이 좋습니다!

```python
%pip install --upgrade --quiet  annoy
```

## 텍스트에서 VectorStore 생성하기
