...
# Intel의 Visual Data Management System (VDMS)

>Intel의 [VDMS](https://github.com/IntelLabs/vdms)는 관련 시각 데이터를 그래프로 저장된 시각 메타데이터를 통해 검색하고, 시각 데이터에 대한 기계 친화적인 향상을 통해 클라우드 규모를 달성하는 것을 목표로 하는 대용량 "시각" 데이터에 대한 효율적인 액세스를 위한 저장 솔루션입니다. VDMS는 MIT 라이선스 하에 제공됩니다.

VDMS는 다음을 지원합니다:
* K 최근접 이웃 검색
* 유클리드 거리(L2) 및 내적(IP)
* 인덱싱 및 거리 계산을 위한 라이브러리: TileDBDense, TileDBSparse, FaissFlat(기본값), FaissIVFFlat
* 벡터 및 메타데이터 검색

VDMS에는 서버와 클라이언트 구성 요소가 있습니다. 서버를 설정하려면 [설치 지침](https://github.com/IntelLabs/vdms/blob/master/INSTALL.md)을 참조하거나 [Docker 이미지](https://hub.docker.com/r/intellabs/vdms)를 사용하세요.

이 노트북은 Docker 이미지를 사용하여 VDMS를 벡터 저장소로 사용하는 방법을 보여줍니다.

시작하려면 VDMS 클라이언트와 Sentence Transformers를 위한 Python 패키지를 설치하세요:
...
이 기본 예제에서는 문서를 VDMS에 추가하고 벡터 데이터베이스로 사용하는 방법을 보여줍니다.

VDMS 서버를 별도의 Docker 컨테이너에서 실행하여 VDMS Python 클라이언트를 통해 LangChain과 연결할 수 있습니다.

VDMS는 여러 문서 컬렉션을 처리할 수 있지만, LangChain 인터페이스에서는 하나의 컬렉션만 예상하므로 컬렉션 이름을 지정해야 합니다. LangChain에서 사용하는 기본 컬렉션 이름은 "langchain"입니다.
...
여기서는 가장 최근의 국정연설문을 로드하고 문서를 청크로 분할합니다.

LangChain 벡터 저장소는 문서 관리를 위해 문자열/키워드 `id`를 사용합니다. 기본적으로 `id`는 uuid이지만 여기서는 정수를 문자열로 정의했습니다. 추가 메타데이터도 문서와 함께 제공되며, 이 예에서는 HuggingFaceEmbeddings가 임베딩 함수로 사용됩니다.
...
### Faiss Flat 및 유클리드 거리(기본값)를 사용한 유사도 검색

이 섹션에서는 FAISS IndexFlat 인덱싱(기본값)과 유클리드 거리(기본값)를 사용하여 문서를 VDMS에 추가합니다. `"What did the president say about Ketanji Brown Jackson"`에 대해 3개의 문서(`k=3`)를 검색합니다.
...
### TileDBDense 및 유클리드 거리를 사용한 유사도 검색

이 섹션에서는 TileDB Dense 인덱싱과 L2를 거리 메트릭으로 사용하여 문서를 VDMS에 추가합니다. `"What did the president say about Ketanji Brown Jackson"`에 대해 3개의 문서(`k=3`)를 검색하고 점수도 함께 반환합니다.
...
### Faiss IVFFlat 및 유클리드 거리를 사용한 유사도 검색

이 섹션에서는 Faiss IndexIVFFlat 인덱싱과 L2를 거리 메트릭으로 사용하여 문서를 VDMS에 추가합니다. `"What did the president say about Ketanji Brown Jackson"`에 대해 3개의 문서(`k=3`)를 검색하고 점수도 함께 반환합니다.
...
실제 애플리케이션을 구축하는 과정에서는 데이터를 추가하는 것 외에도 업데이트 및 삭제가 필요합니다.

다음은 이를 수행하는 기본 예제입니다. 먼저 쿼리와 가장 관련성이 높은 문서의 메타데이터를 업데이트합니다.
...
VDMS는 다양한 유형의 시각 데이터와 작업을 지원합니다. 일부 기능은 LangChain 인터페이스에 통합되어 있지만, VDMS가 계속 개발되면서 추가적인 워크플로 개선이 이루어질 것입니다.

LangChain에 통합된 추가 기능은 다음과 같습니다.

### 벡터에 의한 유사도 검색

문자열 쿼리 대신 임베딩/벡터로 검색할 수 있습니다.
...
예를 들어, 메타데이터를 사용하여 컬렉션을 필터링할 수 있습니다. 여기서는 `id = 2`인 문서를 검색하고 벡터 저장소에서 제거합니다.
...
이 섹션에서는 VDMS를 리트리버로 사용하는 다양한 옵션을 살펴봅니다.

#### 유사도 검색

여기서는 리트리버 객체에서 유사도 검색을 사용합니다.
...
리트리버 객체에서 유사성 검색을 사용하는 것 외에도 `mmr`을 사용할 수 있습니다.
...
이전에는 `id`를 기반으로 문서를 제거했습니다. 여기서는 ID가 제공되지 않았기 때문에 모든 문서가 제거됩니다.
