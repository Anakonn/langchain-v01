...
Divise le texte en fonction de la similarité sémantique.

Tiré du merveilleux cahier de Greg Kamradt :
...
Au niveau le plus élevé, cela se divise en phrases, puis en groupes de 3
phrases, et fusionne ensuite ceux qui sont similaires dans l'espace d'intégration.
...
Ce fractionnement fonctionne en déterminant quand "casser" les phrases. Cela se fait en recherchant les différences d'intégration entre deux phrases. Lorsque cette différence dépasse un certain seuil, elles sont alors divisées.

Il existe plusieurs façons de déterminer quel est ce seuil.

### Percentile

La méthode par défaut de division est basée sur le percentile. Dans cette méthode, toutes les différences entre les phrases sont calculées, et toute différence supérieure au Xème percentile est divisée.
...
Dans cette méthode, toute différence supérieure à X écarts types est divisée.
...
Dans cette méthode, la distance interquartile est utilisée pour diviser les chunks.
