...
>[Format de document portable (PDF)](https://en.wikipedia.org/wiki/PDF), normalisé en tant que norme ISO 32000, est un format de fichier développé par Adobe en 1992 pour présenter des documents, y compris la mise en forme du texte et les images, d'une manière indépendante des logiciels d'application, du matériel et des systèmes d'exploitation.

Cela couvre comment charger les documents `PDF` dans le format de document que nous utilisons en aval.

## Utilisation de PyPDF

Chargez le PDF à l'aide de `pypdf` dans un tableau de documents, où chaque document contient le contenu de la page et les métadonnées avec le numéro de `page`.
...
Un avantage de cette approche est que les documents peuvent être récupérés avec les numéros de page.
...
En utilisant le package `rapidocr-onnxruntime`, nous pouvons également extraire des images sous forme de texte :
...
C'est l'option de parsing PDF la plus rapide et contient des métadonnées détaillées sur le PDF et ses pages, ainsi que renvoie un document par page.
...
De plus, vous pouvez transmettre l'une des options de la [documentation PyMuPDF](https://pymupdf.readthedocs.io/en/latest/app1.html#plain-text/) en tant qu'arguments de mot-clé dans l'appel `load`, et elle sera transmise à l'appel `get_text()`.

## Utilisation de MathPix

Inspiré par le [https://gist.github.com/danielgross/3ab4104e14faccc12b49200843adab21](https://gist.github.com/danielgross/3ab4104e14faccc12b49200843adab21) de Daniel Gross
...
Le package `unstructured[all-docs]` prend en charge actuellement le chargement de fichiers texte, de présentations, de HTML, de PDF, d'images et plus encore.
...
### Récupération de PDF distants à l'aide d'Unstructured

Cela couvre comment charger des PDF en ligne dans un format de document que nous pouvons utiliser en aval. Cela peut être utilisé pour divers sites Web de PDF tels que https://open.umn.edu/opentextbooks/textbooks/ et https://arxiv.org/archive/

Remarque : tous les autres chargeurs PDF peuvent également être utilisés pour récupérer des PDF distants, mais `OnlinePDFLoader` est une fonction héritée et fonctionne spécifiquement avec `UnstructuredPDFLoader`.
...
PDFMiner est un outil qui peut aider à extraire des informations et à analyser des données à partir de documents PDF.
...
### Utilisation de PDFMiner pour générer du texte HTML

Cela peut être utile pour découper les textes de manière sémantique en sections, car le contenu HTML de sortie peut être analysé via `BeautifulSoup` pour obtenir des informations plus structurées et riches sur la taille de la police, les numéros de page, les en-têtes/pieds de page PDF, etc.
...
Charger des PDF à partir d'un répertoire
...
Comme PyMuPDF, les Documents de sortie contiennent des métadonnées détaillées sur le PDF et ses pages, et renvoient un document par page.
...
## Utilisation d'AmazonTextractPDFParser

Le chargeur AmazonTextractPDFLoader appelle le [service Amazon Textract](https://aws.amazon.com/textract/) pour convertir les PDF en une structure de document. Le chargeur effectue pour le moment un OCR pur, avec d'autres fonctionnalités comme le support de la mise en page prévues, en fonction de la demande. Les documents à page unique et à plusieurs pages sont pris en charge avec jusqu'à 3000 pages et 512 Mo de taille.

Pour que l'appel soit réussi, un compte AWS est nécessaire, similaire aux exigences de [AWS CLI](https://docs.aws.amazon.com/cli/latest/userguide/cli-chap-configure.html).

Outre la configuration AWS, c'est très similaire aux autres chargeurs PDF, tout en prenant en charge les formats JPEG, PNG et TIFF et les formats PDF non natifs.
...
## Utilisation d'AzureAIDocumentIntelligenceLoader
...
## Utilisation de UpstageLayoutAnalysisLoader

Le UpstageLayoutAnalysisLoader invoque l'[API Upstage Layout Analysis](https://developers.upstage.ai/docs/apis/layout-analyzer) pour détecter les éléments de document, y compris les tableaux et les figures, à partir de divers formats de document. Ce chargeur utilise une OCR pure pour extraire les informations textuelles et détecter les éléments dans les documents tels que `JPEG`, `PNG`, `BMP`, `PDF`, `TIFF` et `HEIC`. Dans le cas des documents PDF nés numériques, les utilisateurs ont la possibilité de se passer de l'OCR et d'utiliser les informations textuelles dans le fichier en définissant use_ocr=False, qui est la valeur par défaut. Les documents à page unique et à plusieurs pages sont pris en charge, avec une limite de 100 pages et une taille de fichier de 50 Mo lorsque use_ocr=True, tandis qu'il n'y a pas de restrictions lorsque use_ocr=False (applicable aux fichiers PDF uniquement).

### Prérequis

Pour accéder à l'API Upstage Layout Analysis, vous avez besoin d'un jeton d'accès API. Veuillez vous référer au [guide de démarrage rapide](https://developers.upstage.ai/docs/getting-started/quick-start) fourni pour obtenir le jeton d'accès et commencer à utiliser l'API Upstage Layout Analysis.
