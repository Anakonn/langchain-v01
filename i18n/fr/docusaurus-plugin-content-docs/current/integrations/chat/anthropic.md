...
Ce cahier couvre comment se lancer avec les modèles de chat Anthropic.

## Configuration

Pour les instructions de configuration, veuillez consulter les sections Installation et Configuration de l'environnement de la page [Anthropic Platform](/docs/integrations/platforms/anthropic.mdx).
...
Nous aurons besoin d'obtenir une clé d'API [Anthropic](https://console.anthropic.com/settings/keys) et de définir la variable d'environnement `ANTHROPIC_API_KEY` :
...
Le code fourni suppose que votre ANTHROPIC_API_KEY est défini dans vos variables d'environnement. Si vous souhaitez spécifier manuellement votre clé d'API et également choisir un modèle différent, vous pouvez utiliser le code suivant :
...
Dans ces démonstrations, nous utiliserons le modèle Claude 3 Opus, et vous pouvez également utiliser la version de lancement du modèle Sonnet avec `claude-3-sonnet-20240229`.

Vous pouvez consulter le document de comparaison des modèles [ici](https://docs.anthropic.com/claude/docs/models-overview#model-comparison).
...
## `ChatAnthropic` prend également en charge les fonctionnalités asynchrones et de diffusion en continu :
...
Avec l'[API d'appel d'outils, ou d'utilisation d'outils, d'Anthropic](https://docs.anthropic.com/claude/docs/functions-external-tools), vous pouvez définir des outils que le modèle peut invoquer. Cela est extrêmement utile pour construire des chaînes et des agents utilisant des outils, ainsi que pour obtenir des sorties structurées d'un modèle.

:::note

La fonctionnalité d'appel d'outils d'Anthropic est encore en bêta.
...
et passés à chaque invocation du modèle.
...
Notez que le contenu du message de sortie est une liste qui contient un bloc de texte et ensuite un bloc d'utilisation d'outils :
...
Fait crucial, les appels d'outils sont également extraits dans `tool_calls` où ils sont dans un format standardisé et indépendant du modèle :
...
Les sorties du modèle ChatAnthropic sont toujours un seul message AI qui peut avoir soit une seule chaîne, soit une liste de blocs de contenu. Les blocs de contenu peuvent être des blocs de texte ou des blocs d'utilisation d'outils. Il peut y en avoir plusieurs de chaque et ils peuvent être entremêlés.

:::

### Analyse des appels d'outils

Le `langchain_anthropic.output_parsers.ToolsOutputParser` facilite l'analyse des appels d'outils à partir d'un message AI Anthropic en objets Pydantic si nous le souhaitons :
...
L'interface [BaseChatModel.with_structured_output](/docs/modules/model_io/chat/structured_output) facilite l'obtention d'une sortie structurée des modèles de chat. Vous pouvez utiliser `ChatAnthropic.with_structured_output`, qui utilise l'appel d'outils en interne, pour demander au modèle de renvoyer une sortie dans un format spécifique de manière plus fiable :
...
est qu'elle ne renverra que le premier appel GetWeather, alors que la deuxième approche renverra une liste.

### Transmettre les résultats des outils au modèle

Nous pouvons utiliser des `ToolMessage` avec les `tool_call_id` appropriés pour transmettre les résultats des outils au modèle :
...
Anthropic ne prend actuellement pas en charge la diffusion en continu des appels d'outils. Toute tentative de diffusion en continu donnera un seul message final.
...
Les modèles Claude-3 d'Anthropic sont compatibles avec les entrées d'image et de texte. Vous pouvez l'utiliser comme suit :
