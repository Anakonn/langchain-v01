---
sidebar_label: Perplexité
translated: true
---

# ChatPerplexité

Ce notebook couvre comment se lancer avec les modèles de chat Perplexité.
...
Le code fourni suppose que votre PPLX_API_KEY est défini dans vos variables d'environnement. Si vous souhaitez spécifier manuellement votre clé API et également choisir un modèle différent, vous pouvez utiliser le code suivant :

```python
chat = ChatPerplexity(temperature=0, pplx_api_key="YOUR_API_KEY", model="pplx-70b-online")
```

Vous pouvez consulter une liste des modèles disponibles [ici](https://docs.perplexity.ai/docs/model-cards). Pour la reproductibilité, nous pouvons définir dynamiquement la clé API en la saisissant comme entrée dans ce notebook.
...
Vous pouvez formater et structurer les invites comme vous le feriez normalement. Dans l'exemple suivant, nous demandons au modèle de nous raconter une blague sur les chats.
...
## `ChatPerplexité` prend également en charge la fonctionnalité de diffusion en continu :
