...
Ce cahier de laboratoire démontre l'utilisation de `langchain.chat_models.ChatAnyscale` pour les [Anyscale Endpoints](https://endpoints.anyscale.com/).

* Définissez la variable d'environnement `ANYSCALE_API_KEY`
* ou utilisez l'argument de mot-clé `anyscale_api_key`
...
# Essayons chaque modèle proposé sur les Anyscale Endpoints
...
# Nous pouvons utiliser des méthodes asynchrones et d'autres éléments pris en charge par ChatOpenAI

De cette façon, les trois requêtes ne prendront que le temps de la requête individuelle la plus longue.
