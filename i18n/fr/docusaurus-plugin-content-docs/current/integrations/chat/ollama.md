...
[Ollama](https://ollama.ai/) vous permet d'exécuter des modèles de langage à grande échelle open-source, comme Llama 2, localement.
...
Pour une liste complète des modèles et variantes de modèles pris en charge, consultez la [bibliothèque de modèles Ollama](https://github.com/jmorganca/ollama#model-library).
...
* [Téléchargez](https://ollama.ai/download) et installez Ollama sur les plateformes prises en charge disponibles (y compris Windows Subsystem for Linux)
* Récupérez le modèle LLM disponible via `ollama pull <nom-du-modèle>`
    * Affichez la liste des modèles disponibles via la [bibliothèque de modèles](https://ollama.ai/library)
    * par exemple, `ollama pull llama3`
* Cela téléchargera la version par défaut du modèle. Généralement, le par défaut pointe vers le modèle avec le plus petit nombre de paramètres.

> Sur Mac, les modèles seront téléchargés dans `~/.ollama/models`
>
> Sur Linux (ou WSL), les modèles seront stockés dans `/usr/share/ollama/.ollama/models`

* Spécifiez la version exacte du modèle qui vous intéresse comme suit `ollama pull vicuna:13b-v1.5-16k-q4_0` (Consultez les [différentes étiquettes pour le modèle `Vicuna`](https://ollama.ai/library/vicuna/tags) dans cette instance)
* Pour afficher tous les modèles récupérés, utilisez `ollama list`
* Pour discuter directement avec un modèle depuis la ligne de commande, utilisez `ollama run <nom-du-modèle>`
* Consultez la [documentation Ollama](https://github.com/jmorganca/ollama) pour plus de commandes. Exécutez `ollama help` dans le terminal pour voir les commandes disponibles.

## Utilisation

Vous pouvez voir la liste complète des paramètres pris en charge sur la [page de référence de l'API](https://api.python.langchain.com/en/latest/llms/langchain.llms.ollama.Ollama.html).

Si vous utilisez un modèle `chat` LLaMA (par exemple, `ollama pull llama3`), vous pouvez utiliser l'interface `ChatOllama`.

Cela inclut des [jetons spéciaux](https://huggingface.co/blog/llama2#how-to-prompt-llama-2) pour les messages système et les entrées utilisateur.

## Interaction avec les modèles

Voici quelques moyens d'interagir avec les modèles locaux récupérés

#### directement dans le terminal :

* Tous vos modèles locaux sont automatiquement servis sur `localhost:11434`
* Exécutez `ollama run <nom-du-modèle>` pour commencer à interagir directement via la ligne de commande

### via une API

Envoyez une requête `application/json` au point de terminaison API d'Ollama pour interagir.
...
Consultez la [documentation de l'API Ollama](https://github.com/jmorganca/ollama/blob/main/docs/api.md) pour tous les points de terminaison.

#### via LangChain

Voici un exemple de base typique d'utilisation d'Ollama via le modèle de chat `ChatOllama` dans votre application LangChain.
...
Les chaînes LCEL, par défaut, fournissent des fonctionnalités supplémentaires, comme le streaming des réponses et la prise en charge asynchrone.
...
Pour la prise en charge du streaming asynchrone, voici un exemple - tout cela est possible via la seule chaîne créée ci-dessus.
...
Consultez l'[interface LCEL (LangChain Expressive Language)](/docs/expression_language/interface) pour les autres interfaces disponibles à utiliser lors de la création d'une chaîne.

## Construire à partir de la source

Pour obtenir les instructions les plus à jour sur la construction à partir de la source, consultez la documentation Ollama sur [Construire à partir de la source](https://github.com/jmorganca/ollama?tab=readme-ov-file#building)

## Extraction

Utilisez la dernière version d'Ollama et fournissez le drapeau [`format`](https://github.com/jmorganca/ollama/blob/main/docs/api.md#json-mode). Le drapeau `format` forcera le modèle à produire la réponse au format JSON.

> **Remarque :** Vous pouvez également essayer le wrapper [OllamaFunctions](/docs/integrations/chat/ollama_functions) expérimental pour plus de commodité.
...
Ollama prend en charge les LLM multi-modaux, comme [bakllava](https://ollama.ai/library/bakllava) et [llava](https://ollama.ai/library/llava).

Parcourez l'ensemble complet des versions des modèles avec les `tags`, comme [Llava](https://ollama.ai/library/llava/tags).

Téléchargez le LLM souhaité via `ollama pull bakllava`

Assurez-vous de mettre à jour Ollama afin d'avoir la version la plus récente pour prendre en charge le multi-modal.

Découvrez un exemple typique de la manière d'utiliser le support multi-modal de ChatOllama ci-dessous :
