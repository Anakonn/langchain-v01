...
# Doctran : traduction de langue

La comparaison de documents à travers les embeddings a l'avantage de fonctionner dans plusieurs langues. "Harrison dit bonjour" et "Harrison dice hola" occuperont des positions similaires dans l'espace vectoriel car ils ont la même signification sémantique.

Cependant, il peut encore être utile d'utiliser un LLM pour **traduire des documents dans d'autres langues** avant de les vectoriser. Cela est particulièrement utile lorsque les utilisateurs sont censés interroger la base de connaissances dans différentes langues, ou lorsque les modèles d'embedding de pointe ne sont pas disponibles pour une langue donnée.

Nous pouvons accomplir cela en utilisant la bibliothèque [Doctran](https://github.com/psychic-api/doctran), qui utilise la fonctionnalité d'appel de fonction d'OpenAI pour traduire des documents entre les langues.
