...
# Doctran : interroger les documents

Les documents utilisés dans une base de connaissances de type "vector store" sont généralement stockés dans un format narratif ou conversationnel. Cependant, la plupart des requêtes des utilisateurs sont sous forme de questions. Si nous **convertissons les documents en format questions-réponses** avant de les vectoriser, nous pouvons augmenter la probabilité de récupérer des documents pertinents et diminuer la probabilité de récupérer des documents non pertinents.

Nous pouvons y arriver en utilisant la bibliothèque [Doctran](https://github.com/psychic-api/doctran), qui utilise la fonctionnalité d'appel de fonction d'OpenAI pour "interroger" les documents.

Voir [ce notebook](https://github.com/psychic-api/doctran/blob/main/benchmark.md) pour les benchmarks sur les scores de similarité vectorielle pour différentes requêtes basées sur des documents bruts versus des documents interrogés.
...
Voici le document que nous allons interroger
...
Après avoir interrogé un document, le résultat sera renvoyé sous la forme d'un nouveau document avec les questions et les réponses fournies dans les métadonnées.
