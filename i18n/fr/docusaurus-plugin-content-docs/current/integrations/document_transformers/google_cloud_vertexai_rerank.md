...
> L'[API de classement de recherche Vertex](https://cloud.google.com/generative-ai-app-builder/docs/ranking) est l'une des API autonomes de [Vertex AI Agent Builder](https://cloud.google.com/generative-ai-app-builder/docs/builder-apis). Elle prend une liste de documents et les reclasse en fonction de leur pertinence par rapport à une requête. Comparé aux embeddings, qui ne tiennent compte que de la similarité sémantique d'un document et d'une requête, l'API de classement peut vous donner des scores précis sur la façon dont un document répond à une requête donnée. L'API de classement peut être utilisée pour améliorer la qualité des résultats de recherche après avoir récupéré un ensemble initial de documents candidats.

>L'API de classement est sans état, donc il n'est pas nécessaire d'indexer les documents avant d'appeler l'API. Tout ce que vous avez à faire est de passer la requête et les documents. Cela rend l'API bien adaptée pour reclasser les documents de n'importe quel récupérateur de documents.

>Pour plus d'informations, voir [Classer et reclasser les documents](https://cloud.google.com/generative-ai-app-builder/docs/ranking).
...
Pour cet exemple, nous utiliserons la [page Wikipédia de Google](https://en.wikipedia.org/wiki/Google) pour démontrer le fonctionnement de l'API de classement Vertex.

Nous utilisons un pipeline standard de `chargement -> division -> intégration des données`.

Les embeddings sont créés à l'aide du modèle [API d'embeddings Vertex](https://cloud.google.com/vertex-ai/generative-ai/docs/embeddings/get-text-embeddings#supported_models) - `textembedding-gecko@003`
...
### Tester l'API de classement Vertex

Interrogeons à la fois le `basic_retriever` et le `retriever_with_reranker` avec la même requête et comparons les documents récupérés.

L'API de classement prend en entrée les résultats du `basic_retriever` et les transmet à l'API de classement.

L'API de classement est utilisée pour améliorer la qualité du classement et déterminer un score indiquant la pertinence de chaque enregistrement par rapport à la requête.

Vous pouvez voir la différence entre les documents non classés et les documents classés. L'API de classement déplace les documents les plus sémantiquement pertinents en haut de la fenêtre de contexte du LLM, l'aidant ainsi à former une meilleure réponse avec un raisonnement.
...
Examinons quelques documents reclassés. Nous observons que le récupérateur renvoie toujours les [documents](https://api.python.langchain.com/en/latest/documents/langchain_core.documents.base.Document.html) de type Langchain pertinents, mais dans le champ des métadonnées, nous recevons également le `relevance_score` de l'API de classement.
...
Ceci montre un exemple de chaîne RAG complète avec un modèle de requête simple sur la façon dont vous pouvez effectuer un reclassement à l'aide de l'API de classement Vertex.
