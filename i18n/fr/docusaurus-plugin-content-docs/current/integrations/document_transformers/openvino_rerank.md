...
Le modèle de reclassement Hugging Face peut être pris en charge par OpenVINO via la classe ``OpenVINOReranker``. Si vous avez un GPU Intel, vous pouvez spécifier `model_kwargs={"device": "GPU"}` pour exécuter l'inférence sur celui-ci.
...
## Reclassement avec OpenVINO

Maintenant, enveloppons notre récupérateur de base avec un `ContextualCompressionRetriever`, en utilisant `OpenVINOReranker` comme compresseur.
...
Après le reclassement, les 4 premiers documents sont différents des 4 premiers documents récupérés par le récupérateur de base.
...
Il est possible d'exporter votre modèle de reclassement au format IR OpenVINO avec ``OVModelForSequenceClassification`` et de charger le modèle à partir d'un dossier local.
