...
>[Etherscan](https://docs.etherscan.io/) est le principal explorateur de blockchain, moteur de recherche, plateforme d'API et d'analyse pour Ethereum, une plateforme décentralisée de contrats intelligents.

## Aperçu

Le chargeur `Etherscan` utilise l'`API Etherscan` pour charger les historiques de transactions sous un compte spécifique sur le `réseau principal Ethereum`.

Vous aurez besoin d'une `clé API Etherscan` pour procéder. La clé API gratuite a un quota de 5 appels par seconde.

Le chargeur prend en charge les six fonctionnalités suivantes :

* Récupérer les transactions normales sous un compte spécifique sur le réseau principal Ethereum
* Récupérer les transactions internes sous un compte spécifique sur le réseau principal Ethereum
* Récupérer les transactions erc20 sous un compte spécifique sur le réseau principal Ethereum
* Récupérer les transactions erc721 sous un compte spécifique sur le réseau principal Ethereum
* Récupérer les transactions erc1155 sous un compte spécifique sur le réseau principal Ethereum
* Récupérer le solde en wei sous un compte spécifique sur le réseau principal Ethereum

Si le compte n'a pas de transactions correspondantes, le chargeur renverra une liste avec un document. Le contenu du document est ''.

Vous pouvez passer différents filtres au chargeur pour accéder aux différentes fonctionnalités que nous avons mentionnées ci-dessus :
...
* "erc721_transaction"
* "erc1155_transaction"
Le filtre est par défaut sur normal_transaction

Si vous avez des questions, vous pouvez accéder à la [documentation de l'API Etherscan](https://etherscan.io/tx/0x0ffa32c787b1398f44303f731cb06678e086e4f82ce07cebf75e99bb7c079c77) ou me contacter via i@inevitable.tech.

Toutes les fonctions liées aux historiques de transactions sont limitées à un maximum de 1000 historiques en raison de la limite d'Etherscan. Vous pouvez utiliser les paramètres suivants pour trouver les historiques de transactions dont vous avez besoin :

* offset : par défaut à 20. Affiche 20 transactions à la fois
* page : par défaut à 1. Cela contrôle la pagination.
* start_block : Par défaut à 0. Les historiques de transactions commencent à partir du bloc 0.
* end_block : Par défaut à 99999999. Les historiques de transactions commencent à partir du bloc 99999999
* sort : "desc" ou "asc". Défini par défaut sur "desc" pour obtenir les transactions les plus récentes.
...
## Créer un chargeur de transactions ERC20
...
## Créer un chargeur de transactions normales avec des paramètres personnalisés
