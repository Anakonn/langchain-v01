...
Ce cahier montre comment charger des fichiers de courrier électronique (`.eml`) ou `Microsoft Outlook` (`.msg`).
...
Vous pouvez traiter les pièces jointes avec `UnstructuredEmailLoader` en définissant `process_attachments=True` dans le constructeur. Par défaut, les pièces jointes seront partitionnées à l'aide de la fonction `partition` de `unstructured`. Vous pouvez utiliser une fonction de partitionnement différente en la passant à l'argument `attachment_partitioner`.
...
## Utilisation d'OutlookMessageLoader
