...
Ce notebook démontre l'utilisation de la classe `langchain_community.document_loaders.ArcGISLoader`.

Vous devrez installer l'API ArcGIS pour Python `arcgis` et, en option, `bs4.BeautifulSoup`.

Vous pouvez utiliser un objet `arcgis.gis.GIS` pour le chargement de données authentifiées, ou le laisser vide pour accéder aux données publiques.
...
Mesurons la latence du chargeur.
...
Si vous voulez récupérer les géométries des entités, vous pouvez le faire avec le mot-clé `return_geometry`.

La géométrie de chaque document sera stockée dans son dictionnaire de métadonnées.
