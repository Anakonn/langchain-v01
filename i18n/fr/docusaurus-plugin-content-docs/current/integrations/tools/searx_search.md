...
Ce cahier aborde l'utilisation d'une API de recherche `SearxNG` auto-hébergée pour rechercher sur le Web.

Vous pouvez [vérifier ce lien](https://docs.searxng.org/dev/search_api.html) pour plus d'informations sur les paramètres de l'API `Searx`.
...
Pour certains moteurs, si une `réponse` directe est disponible, le wrapper affichera la réponse au lieu de la liste complète des résultats de recherche. Vous pouvez utiliser la méthode `results` du wrapper si vous voulez obtenir tous les résultats.
...
SearxNG prend en charge [135 moteurs de recherche](https://docs.searxng.org/user/configured_engines.html). Vous pouvez également personnaliser le wrapper Searx avec des paramètres nommés arbitraires qui seront transmis à l'API de recherche Searx. Dans l'exemple ci-dessous, nous allons faire une utilisation plus intéressante des paramètres de recherche personnalisés de l'API de recherche searx.

Dans cet exemple, nous utiliserons le paramètre `engines` pour interroger Wikipédia.
...
Passer d'autres paramètres Searx comme `language`
...
## Obtenir les résultats avec les métadonnées

Dans cet exemple, nous recherchons des articles scientifiques à l'aide du paramètre `categories` et en limitant les résultats à une `time_range` (tous les moteurs ne prennent pas en charge l'option de plage de temps).

Nous voudrions également obtenir les résultats de manière structurée, y compris les métadonnées. Pour cela, nous utiliserons la méthode `results` du wrapper.
...
Dans cet exemple, nous recherchons `large language models` dans la catégorie `it`. Nous filtrons ensuite les résultats provenant de github.
...
Nous pourrions également interroger directement les résultats de `github` et d'autres forges de code source.
