...
Ce notebook explique comment utiliser le composant de recherche duck-duck-go.
...
Pour obtenir des informations supplémentaires (par exemple, lien, source), utilisez `DuckDuckGoSearchResults()`
...
Vous pouvez également rechercher des articles d'actualité. Utilisez le mot-clé ``backend="news"``
...
Vous pouvez également transmettre directement un `DuckDuckGoSearchAPIWrapper` personnalisé à `DuckDuckGoSearchResults`. Vous aurez ainsi beaucoup plus de contrôle sur les résultats de la recherche.
