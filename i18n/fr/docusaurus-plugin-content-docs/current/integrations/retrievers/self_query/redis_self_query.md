...
>[Redis](https://redis.com) est une base de données clé-valeur open-source qui peut être utilisée comme cache, courtier de messages, base de données, base de données vectorielle et plus encore.

Dans le notebook, nous allons démontrer le `SelfQueryRetriever` enveloppé autour d'un magasin vectoriel `Redis`.

## Création d'un magasin vectoriel Redis

Tout d'abord, nous voulons créer un magasin vectoriel Redis et le remplir avec quelques données. Nous avons créé un petit ensemble de documents de démonstration contenant des résumés de films.
