...
>[Chroma](https://docs.trychroma.com/getting-started) est une base de données vectorielle pour construire des applications d'IA avec des embeddings.

Dans le notebook, nous allons démontrer le `SelfQueryRetriever` enveloppé autour d'un magasin de vecteurs `Chroma`.

## Création d'un magasin de vecteurs Chroma

Tout d'abord, nous voulons créer un magasin de vecteurs Chroma et l'alimenter avec quelques données. Nous avons créé un petit ensemble de documents de démonstration contenant des résumés de films.

**Remarque :** Le récupérateur d'auto-requête nécessite que vous ayez `lark` installé (`pip install lark`). Nous avons également besoin du package `langchain-chroma`.
