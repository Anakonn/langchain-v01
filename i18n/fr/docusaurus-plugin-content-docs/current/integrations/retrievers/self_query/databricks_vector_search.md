...
>[Databricks Vector Search](https://docs.databricks.com/en/generative-ai/vector-search.html) est un moteur de recherche de similarité sans serveur qui vous permet de stocker une représentation vectorielle de vos données, y compris les métadonnées, dans une base de données vectorielle. Avec Vector Search, vous pouvez créer des index de recherche vectorielle auto-mis à jour à partir de tables Delta gérées par Unity Catalog et les interroger avec une simple API pour retourner les vecteurs les plus similaires.

Dans le tutoriel, nous allons faire une démonstration du `SelfQueryRetriever` avec une recherche vectorielle Databricks.

## créer un index de magasin vectoriel Databricks

Tout d'abord, nous allons créer un index de magasin vectoriel Databricks et l'ensemencer avec des données. Nous avons créé un petit ensemble de documents de démonstration contenant des résumés de films.

**Remarque :** Le récupérateur de requêtes auto-requêtes nécessite que vous ayez `lark` installé (`pip install lark`) ainsi que des exigences spécifiques à l'intégration.
...
## Création de notre récupérateur de requêtes auto-requêtes

Nous pouvons maintenant instancier notre récupérateur. Pour ce faire, nous devrons fournir quelques informations à l'avance sur les champs de métadonnées que nos documents prennent en charge et une courte description du contenu des documents.
...
Nous pouvons également utiliser le récupérateur de requêtes auto-requêtes pour spécifier `k`: le nombre de documents à récupérer.
