...
>[Qdrant](https://qdrant.tech/documentation/) (se lit : quadrant) est un moteur de recherche de similarité vectorielle. Il fournit un service prêt pour la production avec une API pratique pour stocker, rechercher et gérer des points - des vecteurs avec une charge utile supplémentaire. `Qdrant` est conçu pour prendre en charge un filtrage étendu.

Dans le notebook, nous démontrerons le `SelfQueryRetriever` enveloppé autour d'un magasin de vecteurs `Qdrant`.

## Création d'un magasin de vecteurs Qdrant

Tout d'abord, nous voulons créer un magasin de vecteurs Qdrant et le remplir avec quelques données. Nous avons créé un petit ensemble de démonstration de documents contenant des résumés de films.

**Remarque :** Le récupérateur d'auto-requête nécessite que vous ayez `lark` installé (`pip install lark`). Nous avons également besoin du package `qdrant-client`.
