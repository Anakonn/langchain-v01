...
>[you.com API](https://api.you.com) est une suite d'outils conçue pour aider les développeurs à ancrer la sortie des LLM dans les informations les plus récentes, les plus précises et les plus pertinentes qui peuvent ne pas avoir été incluses dans leur ensemble de données d'entraînement.

## Configuration

Le récupérateur se trouve dans le package `langchain-community`.

Vous devez également définir votre clé d'API you.com.
...
Il est également utile (mais pas nécessaire) de configurer [LangSmith](https://smith.langchain.com/) pour une observabilité de classe mondiale
