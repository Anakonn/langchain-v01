...
> [Annoy](https://github.com/spotify/annoy) (`Approximate Nearest Neighbors Oh Yeah`) est une bibliothèque C++ avec des liaisons Python pour rechercher des points dans l'espace qui sont proches d'un point de requête donné. Il crée également de grandes structures de données en lecture seule basées sur des fichiers qui sont mmapped en mémoire afin que de nombreux processus puissent partager les mêmes données.

Ce notebook montre comment utiliser les fonctionnalités liées à la base de données vectorielle `Annoy`.

:::note
REMARQUE : Annoy est en lecture seule - une fois l'index construit, vous ne pouvez plus ajouter d'autres embeddings !
Si vous voulez ajouter progressivement de nouvelles entrées à votre VectorStore, il vaut mieux choisir une alternative !
...
## Créer un VectorStore via des embeddings existants
...
## Rechercher via l'identifiant du docstore
