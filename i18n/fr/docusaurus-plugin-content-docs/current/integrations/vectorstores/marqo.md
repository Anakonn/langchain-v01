...
Ce cahier montre comment utiliser les fonctionnalités liées au Marqo vectorstore.

>[Marqo](https://www.marqo.ai/) est un moteur de recherche vectoriel open-source. Marqo vous permet de stocker et d'interroger des données multimodales telles que du texte et des images. Marqo crée les vecteurs pour vous en utilisant une vaste sélection de modèles open-source, vous pouvez également fournir vos propres modèles affinés et Marqo se chargera du chargement et de l'inférence.

Pour exécuter ce cahier avec notre image docker, veuillez d'abord exécuter les commandes suivantes pour obtenir Marqo :
...
L'une des fonctionnalités puissantes de Marqo en tant que vectorstore est que vous pouvez utiliser des index créés en externe. Par exemple :

+ Si vous aviez une base de données de paires d'images et de texte provenant d'une autre application, vous pouvez simplement l'utiliser dans langchain avec le Marqo vectorstore. Notez que l'apport de vos propres index multimédias désactivera la méthode `add_texts`.

+ Si vous aviez une base de données de documents textuels, vous pouvez l'intégrer dans le cadre langchain et ajouter d'autres textes via `add_texts`.

Les documents renvoyés sont personnalisés en passant votre propre fonction au rappel `page_content_builder` dans les méthodes de recherche.
...
Nous exposons également les requêtes pondérées de Marqo, qui sont un moyen puissant de composer des recherches sémantiques complexes.
...
Cette section montre comment utiliser Marqo dans le cadre d'une `RetrievalQAWithSourcesChain`. Marqo effectuera les recherches d'informations dans les sources.
