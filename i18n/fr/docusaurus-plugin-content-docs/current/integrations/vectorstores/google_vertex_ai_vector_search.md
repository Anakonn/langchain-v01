...
# Recherche Vectorielle Google Vertex AI

Ce notebook montre comment utiliser les fonctionnalités liées à la base de données vectorielle `Google Cloud Vertex AI Vector Search`.

> [Google Vertex AI Vector Search](https://cloud.google.com/vertex-ai/docs/vector-search/overview), anciennement connu sous le nom de Vertex AI Matching Engine, offre la base de données vectorielle à grande échelle et à faible latence leader de l'industrie. Ces bases de données vectorielles sont communément appelées correspondance de similarité vectorielle ou un service de plus proche voisin approximatif (ANN).

**Remarque** : L'API Langchain attend un point de terminaison et un index déployé déjà créé. Le temps de création de l'index peut prendre jusqu'à une heure.

> Pour voir comment créer un index, consultez la section [Créer un index et le déployer sur un point de terminaison](#create-index-and-deploy-it-to-an-endpoint)
Si vous avez déjà un index déployé, passez à [Créer une VectorStore à partir de textes](#create-vector-store-from-texts)

## Créer un index et le déployer sur un point de terminaison

- Cette section démontre la création d'un nouvel index et son déploiement sur un point de terminaison
...
### Utiliser [VertexAIEmbeddings](https://python.langchain.com/docs/integrations/text_embedding/google_vertex_ai_palm/) comme modèle d'embeddings
...
**Remarque :** Lors de la création d'un index, vous devez spécifier une "index_update_method" parmi "BATCH_UPDATE" ou "STREAM_UPDATE"
> Un index par lots est pour lorsque vous souhaitez mettre à jour votre index en lot, avec des données stockées sur une période de temps définie, comme les systèmes traités chaque semaine ou chaque mois. Un index en streaming est lorsque vous souhaitez que les données de l'index soient mises à jour à mesure que de nouvelles données sont ajoutées à votre datastore, par exemple, si vous avez une librairie et que vous souhaitez afficher les nouveaux stocks en ligne dès que possible. Le type que vous choisissez est important, car la configuration et les exigences sont différentes.

Consultez la [Documentation Officielle](https://cloud.google.com/vertex-ai/docs/vector-search/create-manage-index#create-index-batch) pour plus de détails sur la configuration des index
...
## Créer une Vector Store à partir de textes

REMARQUE : Si vous avez des index et des points de terminaison existants, vous pouvez les charger en utilisant le code ci-dessous
...
### Créer une vectorstore simple (sans filtres)
...
### OPTIONNEL : Vous pouvez également créer des vecteurs et stocker des morceaux dans un Datastore
...
### Créer une vectorstore avec des filtres de métadonnées
...
### Utiliser Vector Store comme récupérateur
...
### Utiliser des filtres avec le récupérateur dans des chaînes de questions-réponses
...
## Lire, fragmenter, vectoriser et indexer des PDFs
