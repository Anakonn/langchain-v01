...
- la recherche de plus proches voisins exacts et approximatifs
- les distances L2, le produit scalaire et la distance cosinus

Ce notebook montre comment utiliser le magasin de vecteurs Kinetica (`Kinetica`).

Cela nécessite une instance de Kinetica qui peut facilement être configurée en suivant les instructions données ici - [instructions d'installation](https://www.kinetica.com/developer-edition/).
...
## Recherche de similarité avec la distance euclidienne (par défaut)
...
La pertinence marginale maximale optimise à la fois la similarité avec la requête et la diversité parmi les documents sélectionnés.
...
## Travailler avec le magasin de vecteurs

Ci-dessus, nous avons créé un magasin de vecteurs à partir de zéro. Cependant, souvent, nous voulons travailler avec un magasin de vecteurs existant.
...
Nous pouvons ajouter des documents au magasin de vecteurs existant.
...
### Remplacer un magasin de vecteurs

Si vous avez une collection existante, vous pouvez la remplacer en utilisant `from_documents` et en définissant `pre_delete_collection` = True
...
### Utiliser un VectorStore comme Retriever
