...
>[LLMRails](https://www.llmrails.com/) est une plateforme d'API pour construire des applications GenAI. Elle fournit une API facile à utiliser pour l'indexation et l'interrogation de documents, gérée par LLMRails et optimisée pour les performances et la précision.
Consultez la [documentation de l'API LLMRails](https://docs.llmrails.com/) pour plus d'informations sur la façon d'utiliser l'API.

Ce notebook montre comment utiliser les fonctionnalités liées à l'intégration de `LLMRails` avec langchain.
Notez que, contrairement à de nombreuses autres intégrations dans cette catégorie, LLMRails fournit un service géré de bout en bout pour la génération avec augmentation de la récupération, qui comprend :
1. Un moyen d'extraire le texte des fichiers de documents et de les découper en phrases.
2. Son propre modèle d'embeddings et son propre magasin de vecteurs - chaque segment de texte est encodé dans un vecteur d'embeddings et stocké dans le magasin de vecteurs interne de LLMRails
3. Un service de requête qui encode automatiquement la requête en embeddings et récupère les segments de texte les plus pertinents (y compris la prise en charge de la [recherche hybride](https://docs.llmrails.com/datastores/search)))

Tout cela est pris en charge dans cette intégration LangChain.

# Configuration

Vous aurez besoin d'un compte LLMRails pour utiliser LLMRails avec LangChain. Pour commencer, suivez ces étapes :
1. [Inscrivez-vous](https://console.llmrails.com/signup) à un compte LLMRails si vous n'en avez pas déjà un.
2. Ensuite, vous devrez créer des clés API pour accéder à l'API. Cliquez sur l'onglet **"Clés API"** dans la vue du corpus, puis sur le bouton **"Créer une clé API"**. Donnez un nom à votre clé. Cliquez sur "Créer la clé" et vous avez maintenant une clé API active. Gardez cette clé confidentielle.

Pour utiliser LangChain avec LLMRails, vous aurez besoin de cette valeur : api_key.
Vous pouvez les fournir à LangChain de deux manières :

1. Incluez dans votre environnement ces deux variables : `LLM_RAILS_API_KEY`, `LLM_RAILS_DATASTORE_ID`.
...
1. Fournissez-les en tant qu'arguments lors de la création de l'objet LLMRails vectorstore :
...
Pour ajouter du texte à votre datastore, vous devez d'abord aller sur la page [Datastores](https://console.llmrails.com/datastores) et en créer un. Cliquez sur le bouton Créer un datastore et choisissez un nom et un modèle d'embeddings pour votre datastore. Ensuite, récupérez l'identifiant de votre datastore dans les paramètres du nouveau datastore.
...
Le scénario le plus simple pour utiliser LLMRails est d'effectuer une recherche de similarité.
...
## LLMRails en tant que Retriever

LLMRails, comme tous les autres magasins de vecteurs LangChain, est le plus souvent utilisé en tant que Retriever LangChain :
