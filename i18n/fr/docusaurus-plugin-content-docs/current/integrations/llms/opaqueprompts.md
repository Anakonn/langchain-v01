...
[OpaquePrompts](https://opaqueprompts.readthedocs.io/en/latest/) est un service qui permet aux applications de tirer parti de la puissance des modèles de langage sans compromettre la confidentialité des utilisateurs. Conçu pour être composable et facile à intégrer dans les applications et services existants, OpaquePrompts est accessible via une simple bibliothèque Python ainsi que via LangChain. Fait peut-être plus important, OpaquePrompts tire parti de la puissance du [calcul confidentiel](https://en.wikipedia.org/wiki/Confidential_computing) pour s'assurer que même le service OpaquePrompts lui-même ne peut pas accéder aux données qu'il protège.

Ce notebook explique comment utiliser LangChain pour interagir avec `OpaquePrompts`.
...
L'accès à l'API OpaquePrompts nécessite une clé API, que vous pouvez obtenir en créant un compte sur [le site Web d'OpaquePrompts](https://opaqueprompts.opaque.co/). Une fois que vous avez un compte, vous pouvez trouver votre clé API sur [la page des clés API](https:opaqueprompts.opaque.co/api-keys).
...
# Utiliser le wrapper LLM OpaquePrompts

Appliquer OpaquePrompts à votre application pourrait être aussi simple que d'envelopper votre LLM à l'aide de la classe OpaquePrompts en remplaçant `llm=OpenAI()` par `llm=OpaquePrompts(base_llm=OpenAI())`.
...
À partir de la sortie, vous pouvez voir que les données sensibles suivantes du contexte de l'entrée de l'utilisateur.
...
OpaquePrompts détectera automatiquement les données sensibles et les remplacera par un espace réservé.
...
L'espace réservé est utilisé dans la réponse du LLM.
...
La réponse est désanitisée en remplaçant l'espace réservé par les données sensibles d'origine.
...
# Utiliser OpaquePrompts dans l'expression LangChain

Il existe des fonctions qui peuvent être utilisées avec l'expression LangChain si un remplacement direct n'offre pas la flexibilité dont vous avez besoin.
