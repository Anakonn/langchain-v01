...
`Infinity` permet de créer des `Embeddings` à l'aide d'un serveur d'Embedding sous licence MIT.

Ce notebook explique comment utiliser Langchain avec les Embeddings avec le [projet Github Infinity](https://github.com/michaelfeil/infinity).
...
# Option 1 : Utiliser infinity depuis Python

#### Facultatif : installer infinity

Pour installer infinity, utilisez la commande suivante. Pour plus de détails, consultez les [Documents sur Github](https://github.com/michaelfeil/infinity).
Installez les dépendances torch et onnx.
...
# Option 2 : Exécuter le serveur et se connecter via l'API

#### Facultatif : Assurez-vous de démarrer l'instance Infinity
...
# Installer le package infinity

%pip install --upgrade --quiet  infinity_emb[all]

Démarrez le serveur - il est préférable de le faire à partir d'un terminal séparé, pas à l'intérieur du Jupyter Notebook
...
ou alternativement, utilisez docker :
...
## Incorporez vos documents à l'aide de votre instance Infinity
