---
translated: true
---

# Points d'accès de la fondation IA NVIDIA

> Les [points d'accès de la fondation IA NVIDIA](https://www.nvidia.com/en-us/ai-data-science/foundation-models/) donnent aux utilisateurs un accès facile aux points de terminaison d'API hébergés par NVIDIA pour les modèles de la fondation IA NVIDIA comme Mixtral 8x7B, Llama 2, Stable Diffusion, etc. Ces modèles, hébergés sur le [catalogue d'API NVIDIA](https://build.nvidia.com/), sont optimisés, testés et hébergés sur la plateforme IA NVIDIA, ce qui les rend rapides et faciles à évaluer, à personnaliser davantage et à exécuter de manière transparente à des performances maximales sur n'importe quelle pile accélérée.
>
> Avec les [points d'accès de la fondation IA NVIDIA](https://www.nvidia.com/en-us/ai-data-science/foundation-models/), vous pouvez obtenir des résultats rapides à partir d'une pile entièrement accélérée s'exécutant sur le [cloud DGX NVIDIA](https://www.nvidia.com/en-us/data-center/dgx-cloud/). Une fois personnalisés, ces modèles peuvent être déployés n'importe où avec une sécurité, une stabilité et un support de niveau entreprise à l'aide de [NVIDIA AI Enterprise](https://www.nvidia.com/en-us/data-center/products/ai-enterprise/).
>
> Ces modèles peuvent être facilement accessibles via le package [`langchain-nvidia-ai-endpoints`](https://pypi.org/project/langchain-nvidia-ai-endpoints/), comme indiqué ci-dessous.

Cet exemple explique comment utiliser LangChain pour interagir avec le [modèle d'intégration de questions-réponses de récupération NVIDIA](https://catalog.ngc.nvidia.com/orgs/nvidia/teams/ai-foundation/models/nvolve-40k) pris en charge pour la [génération avec augmentation de la récupération](https://developer.nvidia.com/blog/build-enterprise-retrieval-augmented-generation-apps-with-nvidia-retrieval-qa-embedding-model/) via la classe `NVIDIAEmbeddings`.

Pour plus d'informations sur l'accès aux modèles de chat via cette API, consultez la documentation [ChatNVIDIA](https://python.langchain.com/docs/integrations/chat/nvidia_ai_endpoints/).

## Installation

```python
%pip install --upgrade --quiet  langchain-nvidia-ai-endpoints
```

```output
Note: you may need to restart the kernel to use updated packages.
```

## Configuration

**Pour commencer :**

1. Créez un compte gratuit avec [NVIDIA](https://build.nvidia.com/), qui héberge les modèles de la fondation IA NVIDIA

2. Sélectionnez l'onglet `Récupération`, puis sélectionnez le modèle de votre choix

3. Dans `Entrée`, sélectionnez l'onglet `Python` et cliquez sur `Obtenir la clé API`. Cliquez ensuite sur `Générer la clé`.

4. Copiez et enregistrez la clé générée en tant que `NVIDIA_API_KEY`. À partir de là, vous devriez avoir accès aux points de terminaison.

```python
import getpass
import os

# del os.environ['NVIDIA_API_KEY']  ## delete key and reset
if os.environ.get("NVIDIA_API_KEY", "").startswith("nvapi-"):
    print("Valid NVIDIA_API_KEY already in environment. Delete to reset")
else:
    nvapi_key = getpass.getpass("NVAPI Key (starts with nvapi-): ")
    assert nvapi_key.startswith("nvapi-"), f"{nvapi_key[:5]}... is not a valid key"
    os.environ["NVIDIA_API_KEY"] = nvapi_key
```

```output
Valid NVIDIA_API_KEY already in environment. Delete to reset
```

Nous devrions pouvoir voir un modèle d'intégration parmi cette liste qui peut être utilisé conjointement avec un LLM pour des solutions RAG efficaces. Nous pouvons interagir avec ce modèle assez facilement avec l'aide du modèle `NVIDIAEmbeddings`.

## Initialisation

Lors de l'initialisation d'un modèle d'intégration, vous pouvez sélectionner un modèle en le transmettant, par exemple `ai-embed-qa-4` ci-dessous, ou utiliser le modèle par défaut en ne transmettant pas d'arguments.

```python
from langchain_nvidia_ai_endpoints import NVIDIAEmbeddings

embedder = NVIDIAEmbeddings(model="ai-embed-qa-4")
```

Ce modèle est un modèle E5-large affiné qui prend en charge les méthodes `Embeddings` attendues, notamment :

- `embed_query` : Générer une intégration de requête pour un échantillon de requête.

- `embed_documents` : Générer des intégrations de passage pour une liste de documents sur lesquels vous souhaitez effectuer une recherche.

- `aembed_quey`/`embed_documents` : Versions asynchrones des éléments ci-dessus.

### **Similarité**

Voici un test rapide des méthodes en termes d'utilisation, de format et de vitesse pour le cas d'utilisation de l'intégration des points de données suivants :

**Requêtes :**

- Quel temps fait-il à Komchatka ?

- Quels types d'aliments l'Italie est-elle connue pour ?

- Quel est mon nom ? Je parie que vous ne vous en souvenez pas...

- Quel est le sens de la vie, après tout ?

- Le sens de la vie est de s'amuser :D

**Documents :**

- Le temps à Komchatka est froid, avec de longs et rudes hivers.

- L'Italie est connue pour ses pâtes, sa pizza, son gelato et son espresso.

- Je ne peux pas me rappeler les noms personnels, je ne peux fournir que des informations.

- Le but de la vie varie, souvent vu comme l'épanouissement personnel.

- Profiter des moments de la vie est en effet une approche merveilleuse.

### Temps d'intégration

```python
import time

print("Single Query Embedding: ")
s = time.perf_counter()
q_embedding = embedder.embed_query("What's the weather like in Komchatka?")
elapsed = time.perf_counter() - s
print("\033[1m" + f"Executed in {elapsed:0.2f} seconds." + "\033[0m")
print("Shape:", (len(q_embedding),))

print("\nSequential Embedding: ")
s = time.perf_counter()
q_embeddings = [
    embedder.embed_query("What's the weather like in Komchatka?"),
    embedder.embed_query("What kinds of food is Italy known for?"),
    embedder.embed_query("What's my name? I bet you don't remember..."),
    embedder.embed_query("What's the point of life anyways?"),
    embedder.embed_query("The point of life is to have fun :D"),
]
elapsed = time.perf_counter() - s
print("\033[1m" + f"Executed in {elapsed:0.2f} seconds." + "\033[0m")
print("Shape:", (len(q_embeddings), len(q_embeddings[0])))

print("\nBatch Query Embedding: ")
s = time.perf_counter()
# To use the "query" mode, we have to add it as an instance arg
q_embeddings = NVIDIAEmbeddings(
    model="ai-embed-qa-4", model_type="query"
).embed_documents(
    [
        "What's the weather like in Komchatka?",
        "What kinds of food is Italy known for?",
        "What's my name? I bet you don't remember...",
        "What's the point of life anyways?",
        "The point of life is to have fun :D",
    ]
)
elapsed = time.perf_counter() - s
print("\033[1m" + f"Executed in {elapsed:0.2f} seconds." + "\033[0m")
print("Shape:", (len(q_embeddings), len(q_embeddings[0])))
```

```output
Single Query Embedding:
[1mExecuted in 2.19 seconds.[0m
Shape: (1024,)

Sequential Embedding:
[1mExecuted in 3.16 seconds.[0m
Shape: (5, 1024)

Batch Query Embedding:
[1mExecuted in 1.23 seconds.[0m
Shape: (5, 1024)
```

### Intégration de documents

```python
import time

print("Single Document Embedding: ")
s = time.perf_counter()
d_embeddings = embedder.embed_documents(
    [
        "Komchatka's weather is cold, with long, severe winters.",
    ]
)
elapsed = time.perf_counter() - s
print("\033[1m" + f"Executed in {elapsed:0.2f} seconds." + "\033[0m")
print("Shape:", (len(q_embedding),))

print("\nBatch Document Embedding: ")
s = time.perf_counter()
d_embeddings = embedder.embed_documents(
    [
        "Komchatka's weather is cold, with long, severe winters.",
        "Italy is famous for pasta, pizza, gelato, and espresso.",
        "I can't recall personal names, only provide information.",
        "Life's purpose varies, often seen as personal fulfillment.",
        "Enjoying life's moments is indeed a wonderful approach.",
    ]
)
elapsed = time.perf_counter() - s
print("\033[1m" + f"Executed in {elapsed:0.2f} seconds." + "\033[0m")
print("Shape:", (len(q_embeddings), len(q_embeddings[0])))
```

```output
Single Document Embedding:
[1mExecuted in 0.52 seconds.[0m
Shape: (1024,)

Batch Document Embedding:
[1mExecuted in 0.89 seconds.[0m
Shape: (5, 1024)
```

Maintenant que nous avons généré nos intégrations, nous pouvons effectuer une simple vérification de similarité sur les résultats pour voir quels documents auraient déclenché comme réponses raisonnables dans une tâche de récupération :

```python
%pip install --upgrade --quiet  matplotlib scikit-learn
```

```output
Note: you may need to restart the kernel to use updated packages.
```

```python
import matplotlib.pyplot as plt
import numpy as np
from sklearn.metrics.pairwise import cosine_similarity

# Assuming embeddings1 and embeddings2 are your two sets of vectors
# Compute the similarity matrix between embeddings1 and embeddings2
cross_similarity_matrix = cosine_similarity(
    np.array(q_embeddings),
    np.array(d_embeddings),
)

# Plotting the cross-similarity matrix
plt.figure(figsize=(8, 6))
plt.imshow(cross_similarity_matrix, cmap="Greens", interpolation="nearest")
plt.colorbar()
plt.title("Cross-Similarity Matrix")
plt.xlabel("Query Embeddings")
plt.ylabel("Document Embeddings")
plt.grid(True)
plt.show()
```

![](data:image/png;base64,)

Pour rappel, les requêtes et les documents envoyés à notre système étaient :

**Requêtes :**

- Quel temps fait-il à Komchatka ?

- Quels types d'aliments l'Italie est-elle connue pour ?

- Quel est mon nom ? Je parie que vous ne vous en souvenez pas...

- Quel est le sens de la vie, après tout ?

- Le sens de la vie est de s'amuser :D

**Documents :**

- Le temps à Komchatka est froid, avec de longs et rudes hivers.

- L'Italie est connue pour ses pâtes, sa pizza, son gelato et son espresso.

- Je ne peux pas me rappeler les noms personnels, je ne peux fournir que des informations.

- Le but de la vie varie, souvent vu comme l'épanouissement personnel.

- Profiter des moments de la vie est en effet une approche merveilleuse.

## Récupération RAG :

Voici une réutilisation de l'exemple initial de l'[entrée du livre de recettes LangChain Expression Language Retrieval](
https://python.langchain.com/docs/expression_language/cookbook/retrieval), mais exécutée avec les modèles [Mixtral 8x7B Instruct](https://catalog.ngc.nvidia.com/orgs/nvidia/teams/ai-foundation/models/mixtral-8x7b) et [NVIDIA Retrieval QA Embedding](https://catalog.ngc.nvidia.com/orgs/nvidia/teams/ai-foundation/models/nvolve-40k) des AI Foundation Models disponibles dans leurs environnements de jeu. Les exemples suivants du livre de recettes s'exécutent également comme prévu, et nous vous encourageons à explorer ces options.

**CONSEIL :** Nous vous recommandons d'utiliser Mixtral pour le raisonnement interne (c'est-à-dire le suivi d'instructions pour l'extraction de données, la sélection d'outils, etc.) et Llama-Chat pour une seule "conclusion finale en formulant une réponse simple qui convient à cet utilisateur en fonction de l'historique et du contexte".

```python
%pip install --upgrade --quiet  langchain faiss-cpu tiktoken

from operator import itemgetter

from langchain_community.vectorstores import FAISS
from langchain_core.output_parsers import StrOutputParser
from langchain_core.prompts import ChatPromptTemplate
from langchain_core.runnables import RunnablePassthrough
from langchain_nvidia_ai_endpoints import ChatNVIDIA
```

```output
Note: you may need to restart the kernel to use updated packages.
```

```python
vectorstore = FAISS.from_texts(
    ["harrison worked at kensho"],
    embedding=NVIDIAEmbeddings(model="ai-embed-qa-4"),
)
retriever = vectorstore.as_retriever()

prompt = ChatPromptTemplate.from_messages(
    [
        (
            "system",
            "Answer solely based on the following context:\n<Documents>\n{context}\n</Documents>",
        ),
        ("user", "{question}"),
    ]
)

model = ChatNVIDIA(model="ai-mixtral-8x7b-instruct")

chain = (
    {"context": retriever, "question": RunnablePassthrough()}
    | prompt
    | model
    | StrOutputParser()
)

chain.invoke("where did harrison work?")
```

```output
'Based on the document provided, Harrison worked at Kensho.'
```

```python
prompt = ChatPromptTemplate.from_messages(
    [
        (
            "system",
            "Answer using information solely based on the following context:\n<Documents>\n{context}\n</Documents>"
            "\nSpeak only in the following language: {language}",
        ),
        ("user", "{question}"),
    ]
)

chain = (
    {
        "context": itemgetter("question") | retriever,
        "question": itemgetter("question"),
        "language": itemgetter("language"),
    }
    | prompt
    | model
    | StrOutputParser()
)

chain.invoke({"question": "where did harrison work", "language": "italian"})
```

```output
'Harrison ha lavorato presso Kensho.\n\n(In English: Harrison worked at Kensho.)'
```
