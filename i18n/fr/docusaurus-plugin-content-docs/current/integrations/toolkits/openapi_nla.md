...
Les boîtes à outils (`NLAToolkits`) de l'`API de Langage Naturel` permettent aux agents LangChain de planifier et de combiner efficacement les appels entre les points de terminaison.

Ce notebook démontre une composition d'exemple des API `Speak`, `Klarna` et `Spoonacular`.

### Tout d'abord, importez les dépendances et chargez le LLM
...
### Ensuite, chargez les boîtes à outils de l'API de Langage Naturel
...
### Utilisez l'authentification et ajoutez plus de points de terminaison

Certains points de terminaison peuvent nécessiter une authentification de l'utilisateur via des jetons d'accès. Ici, nous montrons comment transmettre les informations d'authentification via l'objet wrapper `Requests`.

Comme chaque `NLATool` expose une interface de langage naturel concise pour l'API qu'il enveloppe, l'agent conversationnel de niveau supérieur a un travail plus facile pour incorporer chaque point de terminaison afin de satisfaire la demande de l'utilisateur.

**Ajout des points de terminaison Spoonacular.**

1. Allez sur la [console API Spoonacular](https://spoonacular.com/food-api/console#Profile) et créez un compte gratuit.
2. Cliquez sur `Profil` et copiez votre clé API ci-dessous.
