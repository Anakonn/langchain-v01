...
>[Infino](https://github.com/infinohq/infino) est un magasin de télémétrie évolutif conçu pour les journaux, les métriques et les traces. Infino peut fonctionner comme une solution d'observabilité autonome ou comme la couche de stockage de votre pile d'observabilité.

Cet exemple montre comment on peut suivre ce qui suit lors de l'appel des modèles OpenAI et ChatOpenAI via `LangChain` et [Infino](https://github.com/infinohq/infino) :

* entrée d'invite
* réponse de `ChatGPT` ou de tout autre modèle `LangChain`
* latence
* erreurs
...
## Démarrer le serveur Infino, initialiser le client Infino
...
## Lire le jeu de données des questions
...
## Exemple 1 : LangChain OpenAI Q&A ; Publier des métriques et des journaux sur Infino
...
## Créer des graphiques de métriques

Nous utilisons maintenant matplotlib pour créer des graphiques de latence, d'erreurs et de jetons consommés.
...
## Requête de texte complet sur l'invite ou les sorties d'invite.
...
# Exemple 2 : Résumer un morceau de texte à l'aide de ChatOpenAI
