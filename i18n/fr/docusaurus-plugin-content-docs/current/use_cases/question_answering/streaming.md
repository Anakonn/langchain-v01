...
Souvent, dans les applications de questions-réponses, il est important de montrer aux utilisateurs les sources qui ont été utilisées pour générer la réponse. La façon la plus simple de procéder est de faire en sorte que la chaîne renvoie les documents qui ont été récupérés à chaque génération.

Nous nous baserons sur l'application de questions-réponses avec sources que nous avons construite dans le billet de blog [LLM Powered Autonomous Agents](https://lilianweng.github.io/posts/2023-06-23-agent/) de Lilian Weng, dans le guide [Returning sources](/docs/use_cases/question_answering/sources).
...
Notez que LangSmith n'est pas obligatoire, mais il est utile. Si vous souhaitez utiliser LangSmith, après vous être inscrit au lien ci-dessus, assurez-vous de définir vos variables d'environnement pour commencer à enregistrer les traces :
...
Voici l'application de questions-réponses avec sources que nous avons construite à partir du billet de blog [LLM Powered Autonomous Agents](https://lilianweng.github.io/posts/2023-06-23-agent/) de Lilian Weng, dans le guide [Returning sources](/docs/use_cases/question_answering/sources) :
...
## Streaming des sorties finales

Avec LCEL, il est facile de diffuser les sorties finales :
...
Nous pouvons ajouter une logique pour compiler notre flux au fur et à mesure qu'il est renvoyé :
...
## Diffusion des étapes intermédiaires

Supposons que nous voulions diffuser non seulement les sorties finales de la chaîne, mais aussi certaines étapes intermédiaires. Prenons par exemple notre chaîne [Chat history](/docs/use_cases/question_answering/chat_history). Ici, nous reformulons la question de l'utilisateur avant de la transmettre au récupérateur. Cette question reformulée n'est pas renvoyée dans la sortie finale. Nous pourrions modifier notre chaîne pour renvoyer la nouvelle question, mais à des fins de démonstration, nous la laisserons telle quelle.
...
Pour diffuser les étapes intermédiaires, nous utiliserons la méthode `astream_log`. Il s'agit d'une méthode asynchrone qui renvoie des opérations JSONPatch qui, lorsqu'elles sont appliquées dans le même ordre que celui dans lequel elles ont été reçues, construisent l'état d'exécution :
...
Vous pouvez diffuser toutes les étapes (par défaut) ou inclure/exclure des étapes par nom, balises ou métadonnées. Dans ce cas, nous ne diffuserons que les étapes intermédiaires qui font partie de la `contextualize_q_chain` et la sortie finale. Notez que lorsque nous avons défini la `contextualize_q_chain`, nous lui avons attribué une balise correspondante, que nous pouvons maintenant filtrer.

Nous ne montrons que les 20 premiers morceaux du flux par souci de lisibilité :
...
Si nous voulions récupérer nos documents récupérés, nous pourrions filtrer sur le nom "Retriever" :
...
Pour en savoir plus sur la diffusion des étapes intermédiaires, consultez la documentation de [LCEL Interface](/docs/expression_language/interface#async-stream-intermediate-steps).
