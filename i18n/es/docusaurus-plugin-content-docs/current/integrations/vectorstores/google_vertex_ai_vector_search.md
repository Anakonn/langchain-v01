...
Este cuaderno muestra cómo usar la funcionalidad relacionada con la `Google Cloud Vertex AI Vector Search` base de datos de vectores.

> [Google Vertex AI Vector Search](https://cloud.google.com/vertex-ai/docs/vector-search/overview), anteriormente conocido como Vertex AI Matching Engine, proporciona la base de datos de vectores de alta escala y baja latencia líder en la industria. Estas bases de datos de vectores se conocen comúnmente como servicios de coincidencia de similitud de vectores o de vecino más cercano aproximado (ANN).

**Nota**: La API de Langchain espera un punto final y un índice desplegado ya creado. El tiempo de creación del índice puede tardar hasta una hora.

> Para ver cómo crear un índice, consulte la sección [Crear índice y desplegarlo en un punto final](#create-index-and-deploy-it-to-an-endpoint)
Si ya tiene un índice desplegado, salte a [Crear VectorStore a partir de textos](#create-vector-store-from-texts)

## Crear índice y desplegarlo en un punto final

- Esta sección demuestra la creación de un nuevo índice y su despliegue en un punto final
...
### Usar [VertexAIEmbeddings](https://python.langchain.com/docs/integrations/text_embedding/google_vertex_ai_palm/) como el modelo de incrustaciones
...
**Nota:** Al crear un índice, debe especificar un "index_update_method" ya sea de "BATCH_UPDATE" o "STREAM_UPDATE"
> Un índice por lotes es para cuando desea actualizar su índice en un lote, con datos que se han almacenado durante un cierto período de tiempo, como sistemas que se procesan semanalmente o mensualmente. Un índice de transmisión es cuando desea que los datos del índice se actualicen a medida que se agrega nuevo datos a su almacén de datos, por ejemplo, si tiene una librería y desea mostrar el nuevo inventario en línea tan pronto como sea posible. El tipo que elija es importante, ya que la configuración y los requisitos son diferentes.

Consulte la [Documentación oficial](https://cloud.google.com/vertex-ai/docs/vector-search/create-manage-index#create-index-batch) para obtener más detalles sobre la configuración de índices
...
### Implementar el índice en el punto final
...
## Crear Vector Store a partir de textos

NOTA: Si tiene índices y puntos finales existentes, puede cargarlos usando el código a continuación
...
### Crear un vectorstore simple (sin filtros)
...
### OPCIONAL: También puede crear vectores y almacenar fragmentos en un Datastore
...
### Crear vectorstore con filtros de metadatos
...
### Usar Vector Store como recuperador
...
### Usar filtros con el recuperador en las cadenas de preguntas y respuestas
...
## Leer, dividir en fragmentos, vectorizar e indexar PDF
