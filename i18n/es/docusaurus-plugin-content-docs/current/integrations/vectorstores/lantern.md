...
>[Linterna](https://github.com/lanterndata/lantern) es un buscador de similitud de vectores de código abierto para `Postgres`

Admite:
- Búsqueda de vecinos más cercanos exacta y aproximada
- Distancia cuadrada L2, distancia de Hamming y distancia de coseno

Este cuaderno muestra cómo usar la base de datos de vectores de Postgres (`Linterna`).

Consulta las [instrucciones de instalación](https://github.com/lanterndata/lantern#-quick-install).
...
La relevancia marginal máxima optimiza la similitud con la consulta Y la diversidad entre los documentos seleccionados.
...
### Anular un vectorstore

Si tiene una colección existente, puede anularla haciendo `from_documents` y estableciendo `pre_delete_collection` = True
Esto eliminará la colección antes de volver a rellenarla
...
### Usar un VectorStore como un Recuperador
