...
Este cuaderno muestra cómo crear tu propio cargador de chat que funciona en mensajes copiados y pegados (de DM) a una lista de mensajes de LangChain.

El proceso tiene cuatro pasos:
1. Crea el archivo de chat .txt copiando los chats de la aplicación Discord y pegándolos en un archivo en tu computadora local.
2. Copia la definición del cargador de chat desde abajo a un archivo local.
3. Inicializa el `DiscordChatLoader` con la ruta del archivo apuntando al archivo de texto.
4. Llama a `loader.load()` (o `loader.lazy_load()`) para realizar la conversión.

## 1. Crear volcado de mensajes

Actualmente (2023/08/23) este cargador solo admite archivos .txt con el formato generado al copiar mensajes en la aplicación y pegarlos en un archivo. A continuación se muestra un ejemplo.
...
Puedes usar estos mensajes como mejor te parezca, como ajustar un modelo, seleccionar ejemplos de pocos disparos o hacer predicciones directamente para el siguiente mensaje.
