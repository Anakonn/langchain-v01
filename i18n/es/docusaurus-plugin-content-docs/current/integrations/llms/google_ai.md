...
Una guía sobre el uso de los modelos [Google Generative AI](https://developers.generativeai.google/) con Langchain. Nota: Es independiente de la [integración](/docs/integrations/llms/google_vertex_ai_palm) de Google Cloud Vertex AI.

## Configuración

Para usar Google Generative AI, debe instalar el paquete de Python `langchain-google-genai` y generar una clave API. [Leer más detalles](https://developers.generativeai.google/).
...
Los modelos Gemini tienen configuraciones de seguridad predeterminadas que se pueden anular. Si está recibiendo muchas "Advertencias de seguridad" de sus modelos, puede intentar ajustar el atributo `safety_settings` del modelo. Por ejemplo, para desactivar el bloqueo de seguridad para contenido peligroso, puede construir su LLM de la siguiente manera:
...
Para una enumeración de las categorías y umbrales disponibles, consulte los [tipos de configuración de seguridad](https://ai.google.dev/api/python/google/generativeai/types/SafetySettingDict) de Google.
