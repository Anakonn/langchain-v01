...
>[Replicar](https://replicate.com/blog/machine-learning-needs-better-tools) ejecuta modelos de aprendizaje automático en la nube. Tenemos una biblioteca de modelos de código abierto que puedes ejecutar con unas pocas líneas de código. Si estás construyendo tus propios modelos de aprendizaje automático, Replicate facilita su implementación a escala.

Este ejemplo explica cómo usar LangChain para interactuar con [modelos](https://replicate.com/explore) `Replicate`
...
Para ejecutar este cuaderno, deberás crear una cuenta [replicate](https://replicate.com) e instalar el [cliente python de replicate](https://github.com/replicate/replicate-python).
...
Encuentra un modelo en la [página de exploración de replicate](https://replicate.com/explore), y luego pega el nombre del modelo y la versión en este formato: nombre_modelo/versión.

Por ejemplo, aquí está [`Meta Llama 3`](https://replicate.com/meta/meta-llama-3-8b-instruct).
...
Como otro ejemplo, para este [modelo dolly](https://replicate.com/replicate/dolly-v2-12b), haz clic en la pestaña API. El nombre/versión del modelo sería: `replicate/dolly-v2-12b:ef0e1aefc61f8e096ebe4db6b2bacc297daf2ef6899f0f7e001ec445893500e5`

Solo el parámetro `model` es obligatorio, pero podemos agregar otros parámetros del modelo al inicializarlo.
...
Podemos llamar a cualquier modelo replicate usando esta sintaxis. Por ejemplo, podemos llamar a stable diffusion.
...
El modelo escupe una URL. Vamos a renderizarla.
...
Opcionalmente, puedes transmitir la respuesta a medida que se produce, lo cual es útil para mostrar interactividad a los usuarios para generaciones que tardan mucho tiempo. Consulta la documentación detallada sobre [Streaming](/docs/modules/model_io/llms/streaming_llm) para obtener más información.
...
También puedes especificar secuencias de parada. Si tienes una secuencia de parada definitiva para la generación que de todos modos vas a analizar, es mejor (¡más barato y más rápido!) cancelar la generación una vez que se alcance una o más secuencias de parada, en lugar de dejar que el modelo divague hasta que se alcance la `max_length` especificada. Las secuencias de parada funcionan independientemente de si estás en modo de transmisión o no, y Replicate solo te cobra por la generación hasta la secuencia de parada.
...
¡El objetivo principal de langchain es... encadenar! Aquí hay un ejemplo de cómo hacer eso.

```python
from langchain.chains import SimpleSequentialChain
```

Primero, definamos el LLM para este modelo como un flan-5, y text2image como un modelo de stable diffusion.
...
Segundo prompt para obtener el logotipo para la descripción de la empresa
...
Tercer prompt, vamos a crear la imagen basada en la salida de la descripción del prompt 2
