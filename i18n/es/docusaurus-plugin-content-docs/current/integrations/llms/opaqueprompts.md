...
[OpaquePrompts](https://opaqueprompts.readthedocs.io/en/latest/) es un servicio que permite a las aplicaciones aprovechar el poder de los modelos de lenguaje sin comprometer la privacidad del usuario. Diseñado para la composabilidad y la facilidad de integración en aplicaciones y servicios existentes, OpaquePrompts se puede consumir a través de una sencilla biblioteca de Python, así como a través de LangChain. Quizás más importante aún, OpaquePrompts aprovecha el poder de la [computación confidencial](https://en.wikipedia.org/wiki/Confidential_computing) para garantizar que incluso el propio servicio de OpaquePrompts no pueda acceder a los datos que está protegiendo.

Este cuaderno explica cómo usar LangChain para interactuar con `OpaquePrompts`.
...
El acceso a la API de OpaquePrompts requiere una clave API, que puede obtener creando una cuenta en [el sitio web de OpaquePrompts](https://opaqueprompts.opaque.co/). Una vez que tenga una cuenta, puede encontrar su clave API en [la página de Claves API](https:opaqueprompts.opaque.co/api-keys).
...
# Usar el wrapper de LLM de OpaquePrompts

Aplicar OpaquePrompts a su aplicación podría ser tan sencillo como envolver su LLM usando la clase OpaquePrompts reemplazando `llm=OpenAI()` por `llm=OpaquePrompts(base_llm=OpenAI())`.
...
A partir de la salida, puede ver que el siguiente contexto de la entrada del usuario tiene datos confidenciales.
...
OpaquePrompts detectará automáticamente los datos confidenciales y los reemplazará con un marcador de posición.
...
Se utiliza un marcador de posición en la respuesta de LLM.
...
La respuesta se desanitiza reemplazando el marcador de posición con los datos confidenciales originales.
...
# Usar OpaquePrompts en la expresión de LangChain

También hay funciones que se pueden usar con la expresión de LangChain si una sustitución directa no ofrece la flexibilidad que necesita.
