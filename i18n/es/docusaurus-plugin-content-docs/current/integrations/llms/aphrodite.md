...
[Aphrodite](https://github.com/PygmalionAI/aphrodite-engine) es el motor de inferencia de código abierto a gran escala diseñado para atender a miles de usuarios en el sitio web [PygmalionAI](https://pygmalion.chat).

* Mecanismo de atención por vLLM para un alto rendimiento y bajas latencias
* Soporte para muchos métodos de muestreo SOTA
* Kernels Exllamav2 GPTQ para un mejor rendimiento con tamaños de lote más pequeños

Este cuaderno explica cómo usar un LLM con langchain y Aphrodite.

Para usar, debes tener instalado el paquete de Python `aphrodite-engine`.
...
Aphrodite admite la inferencia y el servicio en paralelo de tensores distribuidos.

Para ejecutar la inferencia de varios GPU con la clase LLM, establece el argumento `tensor_parallel_size` en el número de GPU que quieres usar. Por ejemplo, para ejecutar la inferencia en 4 GPU
