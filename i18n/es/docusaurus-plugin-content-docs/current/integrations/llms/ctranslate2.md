...
**CTranslate2** es una biblioteca de C++ y Python para una inferencia eficiente con modelos Transformer.

El proyecto implementa un tiempo de ejecución personalizado que aplica muchas técnicas de optimización de rendimiento, como la cuantificación de pesos, la fusión de capas, el reordenamiento por lotes, etc., para acelerar y reducir el uso de memoria de los modelos Transformer en CPU y GPU.

La lista completa de características y modelos compatibles se incluye en el [repositorio del proyecto](https://opennmt.net/CTranslate2/guides/transformers.html). Para comenzar, consulte la [guía de inicio rápido oficial](https://opennmt.net/CTranslate2/quickstart.html).

Para usar, debe tener instalado el paquete de Python `ctranslate2`.

```python
%pip install --upgrade --quiet  ctranslate2
```

Para usar un modelo de Hugging Face con CTranslate2, primero debe convertirse al formato CTranslate2 usando el comando `ct2-transformers-converter`. El comando toma el nombre del modelo preentrenado y la ruta al directorio del modelo convertido.
