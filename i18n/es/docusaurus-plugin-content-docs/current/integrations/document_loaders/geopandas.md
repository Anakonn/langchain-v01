...
[Geopandas](https://geopandas.org/en/stable/index.html) es un proyecto de código abierto para facilitar el trabajo con datos geoespaciales en python.

GeoPandas extiende los tipos de datos utilizados por pandas para permitir operaciones espaciales en tipos geométricos.

Las operaciones geométricas se realizan mediante shapely. Geopandas también depende de fiona para el acceso a archivos y de matplotlib para la representación gráfica.

Las aplicaciones de LLM (chat, QA) que utilizan datos geoespaciales son un área interesante para la exploración.
...
Crea un dataframe de GeoPandas a partir de [`Open City Data`](/docs/integrations/document_loaders/open_city_data) como un ejemplo de entrada.
...
Visualización de la muestra de datos de delitos en SF.
...
Carga el dataframe de GeoPandas como un `Document` para el procesamiento posterior (incrustación, chat, etc).

La `geometry` será la columna `page_content` predeterminada, y todas las demás columnas se colocarán en `metadata`.

Sin embargo, podemos especificar la `page_content_column`.
