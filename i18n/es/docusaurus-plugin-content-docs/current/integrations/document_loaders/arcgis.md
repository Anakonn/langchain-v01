...
Este cuaderno demuestra el uso de la clase `langchain_community.document_loaders.ArcGISLoader`.

Deberá instalar la API de ArcGIS para Python `arcgis` y, opcionalmente, `bs4.BeautifulSoup`.

Puede usar un objeto `arcgis.gis.GIS` para la carga de datos autenticada, o dejarlo en blanco para acceder a datos públicos.
...
Midamos la latencia del cargador.
...
Si desea recuperar las geometrías de las características, puede hacerlo con la palabra clave `return_geometry`.

La geometría de cada documento se almacenará en el diccionario de metadatos.
