...
>Un archivo [valores separados por tabulaciones (TSV)](https://en.wikipedia.org/wiki/Tab-separated_values) es un formato de archivo de texto simple para almacenar datos tabulares.[3] Los registros se separan por saltos de línea y los valores dentro de un registro se separan por caracteres de tabulación.

## `UnstructuredTSVLoader`

También puede cargar la tabla usando `UnstructuredTSVLoader`. Una ventaja de usar `UnstructuredTSVLoader` es que si lo usa en el modo `"elements"`, habrá una representación HTML de la tabla disponible en los metadatos.
