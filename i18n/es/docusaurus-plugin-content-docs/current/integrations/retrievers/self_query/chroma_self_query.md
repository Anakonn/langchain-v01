...
>[Chroma](https://docs.trychroma.com/getting-started) es una base de datos vectorial para construir aplicaciones de IA con incrustaciones.

En el cuaderno, demostraremos el `SelfQueryRetriever` envuelto alrededor de un almacén de vectores `Chroma`.

## Creando un almacén de vectores Chroma

Primero querremos crear un almacén de vectores Chroma y sembrarlo con algunos datos. Hemos creado un pequeño conjunto de datos de demostración de documentos que contienen resúmenes de películas.

**Nota:** El recuperador de autoconsuita requiere que tengas `lark` instalado (`pip install lark`). También necesitamos el paquete `langchain-chroma`.
