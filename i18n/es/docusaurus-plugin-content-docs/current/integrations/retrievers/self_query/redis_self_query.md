...
>[Redis](https://redis.com) es un almacén de claves y valores de código abierto que se puede utilizar como caché, intermediario de mensajes, base de datos, base de datos de vectores y más.

En el cuaderno, demostraremos el `SelfQueryRetriever` envuelto alrededor de un almacén de vectores `Redis`.

## Creando un almacén de vectores Redis

Primero querremos crear un almacén de vectores Redis y sembrarlo con algunos datos. Hemos creado un pequeño conjunto de datos de demostración de documentos que contienen resúmenes de películas.

**Nota:** El recuperador de autoconsutas requiere que tengas `lark` instalado (`pip install lark`) junto con los requisitos específicos de la integración.
...
## Creando nuestro recuperador de autoconsutas
...
También podemos usar el recuperador de autoconsutas para especificar `k`: el número de documentos a recuperar.
