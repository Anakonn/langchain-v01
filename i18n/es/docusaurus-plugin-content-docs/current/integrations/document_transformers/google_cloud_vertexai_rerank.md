...
> El [Vertex Search Ranking API](https://cloud.google.com/generative-ai-app-builder/docs/ranking) es una de las API independientes en [Vertex AI Agent Builder](https://cloud.google.com/generative-ai-app-builder/docs/builder-apis). Toma una lista de documentos y los reordena según la relevancia de los documentos para una consulta. En comparación con los incrustaciones, que solo tienen en cuenta la similitud semántica de un documento y una consulta, la API de clasificación puede darle puntuaciones precisas de qué tan bien un documento responde a una consulta determinada. La API de clasificación se puede usar para mejorar la calidad de los resultados de búsqueda después de recuperar un conjunto inicial de documentos candidatos.

>La API de clasificación es sin estado, por lo que no es necesario indexar documentos antes de llamar a la API. Todo lo que tiene que hacer es pasar la consulta y los documentos. Esto hace que la API sea adecuada para reordenar documentos de cualquier recuperador de documentos.

>Para obtener más información, consulte [Rank and rerank documents](https://cloud.google.com/generative-ai-app-builder/docs/ranking).
...
Para este ejemplo, usaremos la [página de Wikipedia de Google](https://en.wikipedia.org/wiki/Google) para demostrar cómo funciona la API de clasificación de Vertex.

Usamos una canalización estándar de `load -> split -> embed data`.

Los incrustaciones se crean usando el modelo [Vertex Embeddings API](https://cloud.google.com/vertex-ai/generative-ai/docs/embeddings/get-text-embeddings#supported_models) - `textembedding-gecko@003`
...
### Probar la API de clasificación de Vertex

Consultemos tanto el `basic_retriever` como el `retriever_with_reranker` con la misma consulta y comparemos los documentos recuperados.

La API de clasificación toma la entrada del `basic_retriever` y la pasa a la API de clasificación.

La API de clasificación se usa para mejorar la calidad de la clasificación y determinar una puntuación que indique la relevancia de cada registro para la consulta.

Puede ver la diferencia entre los documentos sin clasificar y los documentos clasificados. La API de clasificación mueve los documentos más semánticamente relevantes a la parte superior de la ventana de contexto del LLM, lo que ayuda a que forme una mejor respuesta con razonamiento.
...
Inspeccionemos un par de documentos reordenados. Observamos que el recuperador aún devuelve los [documentos](https://api.python.langchain.com/en/latest/documents/langchain_core.documents.base.Document.html) relevantes del tipo Langchain, pero como parte del campo de metadatos, también recibimos el `relevance_score` de la API de clasificación.
...
Esto muestra un ejemplo de una cadena RAG completa con una plantilla de solicitud simple sobre cómo puede realizar la reordenación usando la API de clasificación de Vertex.
