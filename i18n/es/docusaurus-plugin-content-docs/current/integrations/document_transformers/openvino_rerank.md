...
[OpenVINO™](https://github.com/openvinotoolkit/openvino) es un kit de herramientas de código abierto para optimizar y desplegar la inferencia de IA. El tiempo de ejecución de OpenVINO™ admite varios [dispositivos](https://github.com/openvinotoolkit/openvino?tab=readme-ov-file#supported-hardware-matrix) de hardware, incluidos CPU x86 y ARM, e Intel GPU. Puede ayudar a impulsar el rendimiento del aprendizaje profundo en Visión Artificial, Reconocimiento Automático del Habla, Procesamiento del Lenguaje Natural y otras tareas comunes.

El modelo de reordenación de Hugging Face se puede admitir mediante la clase ``OpenVINOReranker``. Si tiene una GPU Intel, puede especificar `model_kwargs={"device": "GPU"}` para ejecutar la inferencia en ella.
...
Comencemos inicializando un simple recuperador de almacén de vectores y almacenando el discurso del Estado de la Unión de 2023 (en fragmentos). Podemos configurar el recuperador para recuperar un número alto (20) de documentos.
...
## Reordenación con OpenVINO

Ahora envolvamos nuestro recuperador base con un `ContextualCompressionRetriever`, usando `OpenVINOReranker` como compresor.
...
Después de la reordenación, los 4 documentos principales son diferentes de los 4 documentos principales recuperados por el recuperador base.
...
Es posible exportar tu modelo de reordenación al formato IR de OpenVINO con ``OVModelForSequenceClassification`` y cargar el modelo desde una carpeta local.
