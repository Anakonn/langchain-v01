---
keywords:
- gemini
- ChatGoogleGenerativeAI
- gemini-pro
sidebar_label: Google AI
translated: true
---

# Modelos de chat de Google AI

Accede a los modelos `gemini` y `gemini-vision` de Google AI, así como a otros modelos generativos a través de la clase `ChatGoogleGenerativeAI` en el paquete de integración [langchain-google-genai](https://pypi.org/project/langchain-google-genai/).

```python
%pip install --upgrade --quiet  langchain-google-genai pillow
```

```python
import getpass
import os

if "GOOGLE_API_KEY" not in os.environ:
    os.environ["GOOGLE_API_KEY"] = getpass.getpass("Provide your Google API Key")
```

## Ejemplo de uso

```python
from langchain_google_genai import ChatGoogleGenerativeAI
```

```python
llm = ChatGoogleGenerativeAI(model="gemini-pro")
result = llm.invoke("Write a ballad about LangChain")
print(result.content)
```

```output
In realms where data streams like fervent tides,
Where algorithms dance and knowledge abides,
A tale unfolds of LangChain, grand and bold,
A ballad sung in bits and bytes untold.

Amidst the codes and circuits' hum,
A spark ignited, a vision would come.
From minds of brilliance, a tapestry formed,
A model to learn, to comprehend, to transform.

In layers deep, its architecture wove,
A neural network, ever-growing, in love.
With language's essence, it sought to entwine,
To unlock the secrets, each word's design.

From texts vast and varied, it feasted and learned,
Its grasp on meaning, swiftly discerned.
Context and syntax, it embraced with grace,
Unraveling stories, at an astonishing pace.

Translations sprang forth, with seamless art,
Bridging tongues and weaving hearts apart.
From English to French, Chinese to Spanish, behold,
LangChain's prowess, like a language untold.

It summarized texts, with insights profound,
Extracting knowledge, without a sound.
Questions it answered, with eloquence rare,
A digital sage, beyond compare.

Yet, its journey faced trials and tribulations,
Obstacles that tested its dedication.
Biases and errors, it sought to transcend,
For fairness and accuracy, it would contend.

With every challenge, it emerged more strong,
Adaptive and resilient, all along.
For LangChain's purpose was etched in its core,
To empower humans, forevermore.

In classrooms and workplaces, it lent its hand,
A tireless assistant, at every demand.
It aided students, in their quests for knowledge,
And professionals thrived, with its guidance and homage.

As years unfurled, its fame grew wide,
A testament to its unwavering stride.
Researchers and scholars, they all took heed,
Of LangChain's brilliance, a groundbreaking deed.

And so, the ballad of LangChain resounds,
A tribute to progress, where innovation abounds.
In the annals of AI, its name shall be etched,
A pioneer, forever in our hearts sketched.
```

Gemini no admite `SystemMessage` por el momento, pero se puede agregar al primer mensaje humano de la fila. Si deseas este comportamiento, simplemente establece `convert_system_message_to_human` en True:

```python
from langchain_core.messages import HumanMessage, SystemMessage

model = ChatGoogleGenerativeAI(model="gemini-pro", convert_system_message_to_human=True)
model(
    [
        SystemMessage(content="Answer only yes or no."),
        HumanMessage(content="Is apple a fruit?"),
    ]
)
```

## Streaming y Batching

`ChatGoogleGenerativeAI` admite streaming y batching de forma nativa. A continuación se muestra un ejemplo.

```python
for chunk in llm.stream("Write a limerick about LLMs."):
    print(chunk.content)
    print("---")
# Note that each chunk may contain more than one "token"
```

```output
There once was an AI named Bert,
Whose language skills were quite expert.
---

With a vast dataset,
It could chat, even bet,
And write limericks, for what it's worth.
---
```

```python
results = llm.batch(
    [
        "What's 2+2?",
        "What's 3+5?",
    ]
)
for res in results:
    print(res.content)
```

```output
4
8
```

## Soporte multimodal

Para proporcionar una imagen, pasa un mensaje humano con contenidos de tipo `List[dict]`, donde cada diccionario contiene un valor de imagen (tipo `image_url`) o un texto (tipo `text`).
El valor de `image_url` puede ser cualquiera de los siguientes:

- Una URL de imagen pública
- Un archivo gcs accesible (por ejemplo, "gcs://path/to/file.png")
- Una ruta de archivo local
- Una imagen codificada en base64 (por ejemplo, `data:image/png;base64,abcd124`)
- Una imagen PIL

```python
import requests
from IPython.display import Image

image_url = "https://picsum.photos/seed/picsum/300/300"
content = requests.get(image_url).content
Image(content)
```

![](data:image/jpg;base64,)

```python
from langchain_core.messages import HumanMessage
from langchain_google_genai import ChatGoogleGenerativeAI

llm = ChatGoogleGenerativeAI(model="gemini-pro-vision")
# example
message = HumanMessage(
    content=[
        {
            "type": "text",
            "text": "What's in this image?",
        },  # You can optionally provide text parts
        {"type": "image_url", "image_url": image_url},
    ]
)
llm.invoke([message])
```

```output
AIMessage(content=' The image contains a snow-capped mountain peak.')
```

## Preguntas frecuentes sobre el uso de Gemini

A partir de la fecha en que se escribió este documento (12/12/2023), Gemini tiene algunas restricciones sobre los tipos y la estructura de los mensajes que acepta. Específicamente:

1. Al proporcionar entradas multimodales (imágenes), se le restringe a un máximo de 1 mensaje de tipo "humano" (usuario). No puede pasar varios mensajes (aunque el único mensaje humano puede tener varias entradas de contenido).
2. No se aceptan mensajes de sistema.
3. Para conversaciones de chat regulares, los mensajes deben seguir el patrón alternante humano/ai/humano/ai. No puede proporcionar 2 mensajes de AI o humanos seguidos.
4. Los mensajes pueden bloquearse si violan los controles de seguridad del LLM. En este caso, el modelo devolverá una respuesta vacía.

### Configuración de seguridad

Los modelos Gemini tienen configuraciones de seguridad predeterminadas que se pueden anular. Si recibes muchas "Advertencias de seguridad" de tus modelos, puedes intentar ajustar el atributo `safety_settings` del modelo. Por ejemplo, para desactivar el bloqueo de seguridad para contenido peligroso, puedes construir tu LLM de la siguiente manera:

```python
from langchain_google_genai import (
    ChatGoogleGenerativeAI,
    HarmBlockThreshold,
    HarmCategory,
)

llm = ChatGoogleGenerativeAI(
    model="gemini-pro",
    safety_settings={
        HarmCategory.HARM_CATEGORY_DANGEROUS_CONTENT: HarmBlockThreshold.BLOCK_NONE,
    },
)
```

Para ver una enumeración de las categorías y umbrales disponibles, consulta la [lista de tipos de configuración de seguridad](https://ai.google.dev/api/python/google/generativeai/types/SafetySettingDict) de Google.

## Configuración adicional

Puedes pasar los siguientes parámetros a ChatGoogleGenerativeAI para personalizar el comportamiento del SDK:

- `client_options`: [Opciones de cliente](https://googleapis.dev/python/google-api-core/latest/client_options.html#module-google.api_core.client_options) para pasar al Cliente de la API de Google, como una `client_options["api_endpoint"]` personalizada.
- `transport`: El método de transporte a utilizar, como `rest`, `grpc` o `grpc_asyncio`.
