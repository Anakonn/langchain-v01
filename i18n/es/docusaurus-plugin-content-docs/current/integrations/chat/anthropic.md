...
Este cuaderno cubre cómo comenzar con los modelos de chat de Anthropic.

## Configuración

Para obtener instrucciones de configuración, consulte las secciones de Instalación y Configuración del entorno de la [página de la plataforma Anthropic](/docs/integrations/platforms/anthropic.mdx).
...
Necesitaremos obtener una clave de API de [Anthropic](https://console.anthropic.com/settings/keys) y establecer la variable de entorno `ANTHROPIC_API_KEY`:
...
El código proporcionado asume que su ANTHROPIC_API_KEY está establecido en sus variables de entorno. Si desea especificar manualmente su clave de API y también elegir un modelo diferente, puede usar el siguiente código:
...
En estas demostraciones, utilizaremos el modelo Claude 3 Opus, y también puede usar la versión de lanzamiento del modelo Sonnet con `claude-3-sonnet-20240229`.

Puede consultar el documento de comparación de modelos [aquí](https://docs.anthropic.com/claude/docs/models-overview#model-comparison).
...
## `ChatAnthropic` también admite funcionalidad asincrónica y de transmisión:
...
## [Beta] Llamada de herramientas

Con la [API de llamada de herramientas, o uso de herramientas, de Anthropic](https://docs.anthropic.com/claude/docs/functions-external-tools), puede definir herramientas para que el modelo las invoque. Esto es extremadamente útil para construir cadenas y agentes que utilizan herramientas, así como para obtener salidas estructuradas de un modelo.

:::note

La funcionalidad de llamada de herramientas de Anthropic aún está en beta.

:::

### bind_tools()

Con `ChatAnthropic.bind_tools`, podemos pasar fácilmente clases Pydantic, esquemas de diccionario, herramientas LangChain o incluso funciones como herramientas al modelo. Debajo del capó, estos se convierten en esquemas de herramientas de Anthropic, que se ven así:
...
y se pasan en cada invocación del modelo.
...
Tenga en cuenta que el contenido del mensaje de salida es una lista que contiene un bloque de texto y luego un bloque de uso de herramientas:
...
Crucialmente, las llamadas a herramientas también se extraen en `tool_calls` donde están en un formato estandarizado e independiente del modelo:
...
Las salidas del modelo ChatAnthropic son siempre un solo mensaje de IA que puede tener ya sea una sola cadena o una lista de bloques de contenido. Los bloques de contenido pueden ser bloques de texto o bloques de uso de herramientas. Puede haber múltiples de cada uno y pueden estar intercalados.

:::

### Analizar las llamadas a herramientas

El `langchain_anthropic.output_parsers.ToolsOutputParser` facilita el análisis de las llamadas a herramientas desde un mensaje de IA de Anthropic en objetos Pydantic si lo deseamos:
...
La interfaz [BaseChatModel.with_structured_output](/docs/modules/model_io/chat/structured_output) facilita la obtención de salida estructurada de los modelos de chat. Puede usar `ChatAnthropic.with_structured_output`, que usa la llamada de herramientas debajo del capó, para hacer que el modelo devuelva más confiablemente una salida en un formato específico:
...
es que devolverá solo la primera llamada GetWeather, mientras que el segundo enfoque devolverá una lista.

### Pasar los resultados de las herramientas al modelo

Podemos usar `ToolMessage`s con los `tool_call_id`s apropiados para pasar los resultados de las herramientas de vuelta al modelo:
...
Anthropic actualmente no admite la transmisión de llamadas a herramientas. Intentar transmitir dará como resultado un solo mensaje final.
...
Los modelos Claude-3 de Anthropic son compatibles con entradas de imagen y texto. Puede usar esto de la siguiente manera:
