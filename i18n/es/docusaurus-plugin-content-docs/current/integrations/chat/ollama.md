...
[Ollama](https://ollama.ai/) le permite ejecutar modelos de lenguaje grande de código abierto, como Llama 2, de forma local.

Ollama agrupa los pesos del modelo, la configuración y los datos en un solo paquete, definido por un Modelfile.

Optimiza los detalles de configuración y configuración, incluido el uso de la GPU.

Para obtener una lista completa de los modelos y variantes de modelos compatibles, consulte la [biblioteca de modelos Ollama](https://github.com/jmorganca/ollama#model-library).
...
* [Descargue](https://ollama.ai/download) e instale Ollama en las plataformas compatibles disponibles (incluido Windows Subsystem for Linux)
* Obtenga el modelo LLM disponible a través de `ollama pull <nombre-del-modelo>`
    * Ver una lista de los modelos disponibles a través de la [biblioteca de modelos](https://ollama.ai/library)
    * por ejemplo, `ollama pull llama3`
* Esto descargará la versión con etiqueta predeterminada del modelo. Normalmente, el valor predeterminado apunta al modelo de parámetros más pequeño más reciente.
...
* Especifique la versión exacta del modelo de interés de la siguiente manera `ollama pull vicuna:13b-v1.5-16k-q4_0` (Vea las [diferentes etiquetas para el modelo `Vicuna`](https://ollama.ai/library/vicuna/tags) en este caso)
* Para ver todos los modelos extraídos, use `ollama list`
* Para chatear directamente con un modelo desde la línea de comandos, use `ollama run <nombre-del-modelo>`
* Consulte la [documentación de Ollama](https://github.com/jmorganca/ollama) para obtener más comandos. Ejecute `ollama help` en la terminal para ver los comandos disponibles.

## Uso

Puede ver una lista completa de los parámetros compatibles en la [página de referencia de la API](https://api.python.langchain.com/en/latest/llms/langchain.llms.ollama.Ollama.html).

Si está utilizando un modelo de `chat` de LLaMA (por ejemplo, `ollama pull llama3`), puede utilizar la interfaz `ChatOllama`.

Esto incluye [tokens especiales](https://huggingface.co/blog/llama2#how-to-prompt-llama-2) para el mensaje del sistema y la entrada del usuario.

## Interactuando con los modelos

Aquí hay algunas formas de interactuar con los modelos locales extraídos

#### directamente en la terminal:

* Todos sus modelos locales se sirven automáticamente en `localhost:11434`
* Ejecute `ollama run <nombre-del-modelo>` para comenzar a interactuar a través de la línea de comandos directamente

### a través de una API

Envíe una solicitud `application/json` al punto final de la API de Ollama para interactuar.
...
Consulte la [documentación de la API](https://github.com/jmorganca/ollama/blob/main/docs/api.md) de Ollama para todos los puntos finales.

#### a través de LangChain

Vea un ejemplo básico típico de usar Ollama a través del modelo de chat `ChatOllama` en su aplicación LangChain.
...
Las cadenas LCEL, de forma predeterminada, proporcionan funcionalidades adicionales, como el streaming de respuestas y el soporte asíncrono.
...
Para obtener soporte de streaming asíncrono, aquí hay un ejemplo: todo esto es posible a través de la única cadena creada anteriormente.
...
Eche un vistazo a la [Interfaz de Lenguaje Expresivo de LangChain (LCEL)](/docs/expression_language/interface) para ver las otras interfaces disponibles para usar cuando se crea una cadena.

## Construir desde la fuente

Para obtener instrucciones actualizadas sobre cómo construir desde la fuente, consulte la documentación de Ollama sobre [Construir desde la fuente](https://github.com/jmorganca/ollama?tab=readme-ov-file#building)

## Extracción

Utilice la última versión de Ollama y suministre la bandera [`format`](https://github.com/jmorganca/ollama/blob/main/docs/api.md#json-mode). La bandera `format` obligará al modelo a producir la respuesta en JSON.

> **Nota:** También puede probar el [Envoltorio de OllamaFunctions](/docs/integrations/chat/ollama_functions) experimental para mayor comodidad.
...
Ollama tiene soporte para LLM multimodales, como [bakllava](https://ollama.ai/library/bakllava) y [llava](https://ollama.ai/library/llava).

Explore el conjunto completo de versiones para los modelos con `tags`, como [Llava](https://ollama.ai/library/llava/tags).

Descargue el LLM deseado a través de `ollama pull bakllava`

Asegúrese de actualizar Ollama para que tenga la versión más reciente que admita multimodal.

Echa un vistazo al ejemplo típico de cómo usar el soporte multimodal de ChatOllama a continuación:
