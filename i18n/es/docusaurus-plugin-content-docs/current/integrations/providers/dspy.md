...
[DSPy](https://github.com/stanfordnlp/dspy) es un marco fantástico para LLM que introduce un compilador automático que enseña a los LM a realizar los pasos declarativos en su programa. Específicamente, el compilador DSPy rastreará internamente su programa y luego creará indicaciones de alta calidad para LM grandes (o entrenará ajustes finos automáticos para LM pequeños) para enseñarles los pasos de su tarea.

Gracias a [Omar Khattab](https://twitter.com/lateinteraction) tenemos una integración. Funciona con cualquier cadena LCEL con algunas modificaciones menores.

Este tutorial corto demuestra cómo funciona esta característica de prueba de concepto. *Esto no le dará todo el poder de DSPy o LangChain todavía, pero lo expandiremos si hay una gran demanda.*

Nota: esto se modificó ligeramente del ejemplo original que Omar escribió para DSPy. Si está interesado en LangChain \<\> DSPy pero viene del lado de DSPy, le recomiendo que revise eso. Puede encontrarlo [aquí](https://github.com/stanfordnlp/dspy/blob/main/examples/tweets/compiling_langchain.md).

Echemos un vistazo a un ejemplo. En este ejemplo haremos una canalización RAG simple. Usaremos DSPy para "compilar" nuestro programa y aprender un indicador optimizado.
...
Estaremos usando OpenAI, así que debemos establecer una clave API
...
Ahora podemos configurar nuestro buscador. Para nuestro buscador, usaremos un buscador ColBERT a través de DSPy, aunque esto funcionará con cualquier buscador.
...
Primero, creemos una canalización RAG simple con LCEL como lo haríamos normalmente.

Para ilustrar, abordemos la siguiente tarea.

**Tarea:** Construir un sistema RAG para generar tweets informativos.

- **Entrada:** Una pregunta fáctica, que puede ser bastante compleja.

- **Salida:** Un tweet atractivo que responda correctamente la pregunta a partir de la información recuperada.

Usemos el lenguaje de expresión (LCEL) de LangChain para ilustrar esto. Cualquier indicación aquí servirá, optimizaremos la indicación final con DSPy.

Teniendo en cuenta eso, mantengámoslo a lo básico: **Dado {context}, responde a la pregunta {question} como un tweet.**
...
Para usar LangChain con DSPy, necesitas hacer dos modificaciones menores

**LangChainPredict**

Necesitas cambiar de hacer `prompt | llm` a usar `LangChainPredict(prompt, llm)` de `dspy`.

Este es un envoltorio que vinculará su indicación y llm juntos para que pueda optimizarlos

**LangChainModule**

Este es un envoltorio que envuelve su cadena LCEL final para que DSPy pueda optimizar todo el conjunto
...
¡Después de esto, podemos usarlo tanto como un ejecutable LangChain como un módulo DSPy!
...
¡Ah, eso suena bien! (Técnicamente no es perfecto: preguntamos por la región, no por la ciudad. Podemos hacerlo mejor a continuación).

¡Inspeccionar preguntas y respuestas manualmente es muy importante para tener una idea de su sistema. Sin embargo, un buen diseñador de sistemas siempre busca evaluar iterativamente su trabajo para cuantificar el progreso!

Para hacer esto, necesitamos dos cosas: la métrica que queremos maximizar y un (pequeño) conjunto de datos de ejemplos para nuestro sistema.

¿Hay métricas predefinidas para los buenos tweets? ¿Debería etiquetar 100,000 tweets a mano? Probablemente no. Sin embargo, podemos hacer algo razonable, hasta que comiences a obtener datos en producción.

## Cargar datos

Para compilar nuestra cadena, necesitamos un conjunto de datos con el que trabajar. Este conjunto de datos solo necesita ser entradas y salidas sin procesar. Para nuestros propósitos, usaremos el conjunto de datos HotPotQA

Nota: ¡Observe que nuestro conjunto de datos no incluye realmente ningún tweet! Solo tiene preguntas y respuestas. Eso está bien, nuestra métrica se encargará de evaluar las salidas en forma de tweet.
...
Ahora necesitamos definir una métrica. Esto se utilizará para determinar qué ejecuciones fueron exitosas y podemos aprender de ellas. Aquí usaremos las métricas de DSPy, aunque puedes escribir las tuyas.
...
## Evaluar la línea de base

Bien, evaluemos la versión "cero-shot" no optimizada de nuestra cadena, convertida de nuestro objeto LCEL de LangChain.
...
Bien, nuestro zeroshot_chain obtiene aproximadamente un 42.00% en las 150 preguntas del conjunto de desarrollo.

La tabla anterior muestra algunos ejemplos. Por ejemplo:

- Pregunta: ¿Quién fue un productor que produjo álbumes para las bandas de rock Juke Karten y Thirty Seconds to Mars?

- Tweet: Brian Virtue, que ha trabajado con bandas como Jane's Addiction y Velvet Revolver, produjo álbumes tanto para Juke Kartel como para Thirty Seconds to Mars, mostrando... [truncado]

- Métrica: 1.0 (¡Un tweet que es correcto, fiel y atractivo!*)

nota al pie: * Al menos según nuestra métrica, que es solo un programa DSPy, ¡así que también se puede optimizar si lo deseas! Tema para otro cuaderno, sin embargo.

## Optimizar

Ahora, vamos a optimizar el rendimiento
...
## Evaluación de la cadena optimizada

¿Qué tan bueno es esto? ¡Hagamos algunas evaluaciones adecuadas!
...
¡Bien! ¡Hemos mejorado nuestra cadena del 42% al casi 50%!

## Inspeccionar la cadena optimizada

Entonces, ¿qué sucedió realmente para mejorar esto? Podemos echar un vistazo a esto mirando la cadena optimizada. Podemos hacer esto de dos maneras

### Mirar el prompt utilizado

Podemos ver qué prompt se utilizó realmente. Podemos hacer esto mirando `dspy.settings`.
...
### Mirar las demostraciones

La forma en que se optimizó esto fue que recopilamos ejemplos (o "demostraciones") para incluirlos en el prompt. Podemos inspeccionar el `optimized_chain` para tener una idea de cuáles son.
