---
translated: true
---

# Puntos finales de NVIDIA AI Foundation

> Los [Puntos finales de NVIDIA AI Foundation](https://www.nvidia.com/en-us/ai-data-science/foundation-models/) brindan a los usuarios un acceso fácil a los puntos finales de API alojados por NVIDIA para modelos como Mixtral 8x7B, Llama 2, Stable Diffusion, etc. de NVIDIA AI Foundation. Estos modelos, alojados en el [catálogo de API de NVIDIA](https://build.nvidia.com/), están optimizados, probados y alojados en la plataforma de IA de NVIDIA, lo que los hace rápidos y fáciles de evaluar, personalizar aún más y ejecutar sin problemas al máximo rendimiento en cualquier pila acelerada.
>
> Con los [Puntos finales de NVIDIA AI Foundation](https://www.nvidia.com/en-us/ai-data-science/foundation-models/), puede obtener resultados rápidos de una pila totalmente acelerada que se ejecuta en [NVIDIA DGX Cloud](https://www.nvidia.com/en-us/data-center/dgx-cloud/). Una vez personalizados, estos modelos se pueden implementar en cualquier lugar con seguridad, estabilidad y soporte de nivel empresarial utilizando [NVIDIA AI Enterprise](https://www.nvidia.com/en-us/data-center/products/ai-enterprise/).
>
> Estos modelos se pueden acceder fácilmente a través del paquete [`langchain-nvidia-ai-endpoints`](https://pypi.org/project/langchain-nvidia-ai-endpoints/), como se muestra a continuación.

Este ejemplo explica cómo usar LangChain para interactuar con el [Modelo de incrustación de preguntas y respuestas de NVIDIA](https://catalog.ngc.nvidia.com/orgs/nvidia/teams/ai-foundation/models/nvolve-40k) compatible para la [generación con aumento de recuperación](https://developer.nvidia.com/blog/build-enterprise-retrieval-augmented-generation-apps-with-nvidia-retrieval-qa-embedding-model/) a través de la clase `NVIDIAEmbeddings`.

Para obtener más información sobre cómo acceder a los modelos de chat a través de esta API, consulte la documentación de [ChatNVIDIA](https://python.langchain.com/docs/integrations/chat/nvidia_ai_endpoints/).

## Instalación

```python
%pip install --upgrade --quiet  langchain-nvidia-ai-endpoints
```

```output
Note: you may need to restart the kernel to use updated packages.
```

## Configuración

**Para comenzar:**

1. Crea una cuenta gratuita con [NVIDIA](https://build.nvidia.com/), que aloja los modelos de NVIDIA AI Foundation

2. Selecciona la pestaña `Retrieval`, luego selecciona el modelo de tu elección

3. En `Input`, selecciona la pestaña `Python` y haz clic en `Get API Key`. Luego haz clic en `Generate Key`.

4. Copia y guarda la clave generada como `NVIDIA_API_KEY`. A partir de ahí, deberías tener acceso a los puntos finales.

```python
import getpass
import os

# del os.environ['NVIDIA_API_KEY']  ## delete key and reset
if os.environ.get("NVIDIA_API_KEY", "").startswith("nvapi-"):
    print("Valid NVIDIA_API_KEY already in environment. Delete to reset")
else:
    nvapi_key = getpass.getpass("NVAPI Key (starts with nvapi-): ")
    assert nvapi_key.startswith("nvapi-"), f"{nvapi_key[:5]}... is not a valid key"
    os.environ["NVIDIA_API_KEY"] = nvapi_key
```

```output
Valid NVIDIA_API_KEY already in environment. Delete to reset
```

Deberíamos poder ver un modelo de incrustación entre esa lista que se puede usar en conjunto con un LLM para soluciones RAG efectivas. Podemos interactuar con este modelo de manera bastante sencilla con la ayuda del modelo `NVIDIAEmbeddings`.

## Inicialización

Al inicializar un modelo de incrustación, puedes seleccionar un modelo pasándolo, por ejemplo, `ai-embed-qa-4` a continuación, o usar el predeterminado sin pasar ningún argumento.

```python
from langchain_nvidia_ai_endpoints import NVIDIAEmbeddings

embedder = NVIDIAEmbeddings(model="ai-embed-qa-4")
```

Este modelo es un modelo E5-large con ajuste fino que admite los métodos `Embeddings` esperados, incluidos:

- `embed_query`: Generar incrustación de consulta para una muestra de consulta.

- `embed_documents`: Generar incrustaciones de pasaje para una lista de documentos sobre los que desea buscar.

- `aembed_quey`/`embed_documents`: Versiones asincrónicas de lo anterior.

### **Similitud**

A continuación se muestra una prueba rápida de los métodos en términos de uso, formato y velocidad para el caso de uso de incrustar los siguientes puntos de datos:

**Consultas:**

- ¿Cómo está el clima en Komchatka?

- ¿Qué tipos de comida es famosa Italia?

- ¿Cuál es mi nombre? Apuesto a que no lo recuerdas...

- ¿Cuál es el sentido de la vida de todos modos?

- El sentido de la vida es divertirse :D

**Documentos:**

- El clima de Komchatka es frío, con inviernos largos y severos.

- Italia es famosa por la pasta, la pizza, el helado y el espresso.

- No puedo recordar nombres personales, solo puedo proporcionar información.

- El propósito de la vida varía, a menudo se ve como la realización personal.

- Disfrutar de los momentos de la vida es ciertamente un enfoque maravilloso.

### Tiempos de ejecución de incrustación

```python
import time

print("Single Query Embedding: ")
s = time.perf_counter()
q_embedding = embedder.embed_query("What's the weather like in Komchatka?")
elapsed = time.perf_counter() - s
print("\033[1m" + f"Executed in {elapsed:0.2f} seconds." + "\033[0m")
print("Shape:", (len(q_embedding),))

print("\nSequential Embedding: ")
s = time.perf_counter()
q_embeddings = [
    embedder.embed_query("What's the weather like in Komchatka?"),
    embedder.embed_query("What kinds of food is Italy known for?"),
    embedder.embed_query("What's my name? I bet you don't remember..."),
    embedder.embed_query("What's the point of life anyways?"),
    embedder.embed_query("The point of life is to have fun :D"),
]
elapsed = time.perf_counter() - s
print("\033[1m" + f"Executed in {elapsed:0.2f} seconds." + "\033[0m")
print("Shape:", (len(q_embeddings), len(q_embeddings[0])))

print("\nBatch Query Embedding: ")
s = time.perf_counter()
# To use the "query" mode, we have to add it as an instance arg
q_embeddings = NVIDIAEmbeddings(
    model="ai-embed-qa-4", model_type="query"
).embed_documents(
    [
        "What's the weather like in Komchatka?",
        "What kinds of food is Italy known for?",
        "What's my name? I bet you don't remember...",
        "What's the point of life anyways?",
        "The point of life is to have fun :D",
    ]
)
elapsed = time.perf_counter() - s
print("\033[1m" + f"Executed in {elapsed:0.2f} seconds." + "\033[0m")
print("Shape:", (len(q_embeddings), len(q_embeddings[0])))
```

```output
Single Query Embedding:
[1mExecuted in 2.19 seconds.[0m
Shape: (1024,)

Sequential Embedding:
[1mExecuted in 3.16 seconds.[0m
Shape: (5, 1024)

Batch Query Embedding:
[1mExecuted in 1.23 seconds.[0m
Shape: (5, 1024)
```

### Incrustación de documentos

```python
import time

print("Single Document Embedding: ")
s = time.perf_counter()
d_embeddings = embedder.embed_documents(
    [
        "Komchatka's weather is cold, with long, severe winters.",
    ]
)
elapsed = time.perf_counter() - s
print("\033[1m" + f"Executed in {elapsed:0.2f} seconds." + "\033[0m")
print("Shape:", (len(q_embedding),))

print("\nBatch Document Embedding: ")
s = time.perf_counter()
d_embeddings = embedder.embed_documents(
    [
        "Komchatka's weather is cold, with long, severe winters.",
        "Italy is famous for pasta, pizza, gelato, and espresso.",
        "I can't recall personal names, only provide information.",
        "Life's purpose varies, often seen as personal fulfillment.",
        "Enjoying life's moments is indeed a wonderful approach.",
    ]
)
elapsed = time.perf_counter() - s
print("\033[1m" + f"Executed in {elapsed:0.2f} seconds." + "\033[0m")
print("Shape:", (len(q_embeddings), len(q_embeddings[0])))
```

```output
Single Document Embedding:
[1mExecuted in 0.52 seconds.[0m
Shape: (1024,)

Batch Document Embedding:
[1mExecuted in 0.89 seconds.[0m
Shape: (5, 1024)
```

Ahora que hemos generado nuestras incrustaciones, podemos hacer una simple comprobación de similitud en los resultados para ver qué documentos se habrían activado como respuestas razonables en una tarea de recuperación:

```python
%pip install --upgrade --quiet  matplotlib scikit-learn
```

```output
Note: you may need to restart the kernel to use updated packages.
```

```python
import matplotlib.pyplot as plt
import numpy as np
from sklearn.metrics.pairwise import cosine_similarity

# Assuming embeddings1 and embeddings2 are your two sets of vectors
# Compute the similarity matrix between embeddings1 and embeddings2
cross_similarity_matrix = cosine_similarity(
    np.array(q_embeddings),
    np.array(d_embeddings),
)

# Plotting the cross-similarity matrix
plt.figure(figsize=(8, 6))
plt.imshow(cross_similarity_matrix, cmap="Greens", interpolation="nearest")
plt.colorbar()
plt.title("Cross-Similarity Matrix")
plt.xlabel("Query Embeddings")
plt.ylabel("Document Embeddings")
plt.grid(True)
plt.show()
```

![](data:image/png;base64,)

Como recordatorio, las consultas y los documentos enviados a nuestro sistema fueron:

**Consultas:**

- ¿Cómo está el clima en Komchatka?

- ¿Qué tipos de comida es famosa Italia?

- ¿Cuál es mi nombre? Apuesto a que no lo recuerdas...

- ¿Cuál es el sentido de la vida de todos modos?

- El sentido de la vida es divertirse :D

**Documentos:**

- El clima de Komchatka es frío, con inviernos largos y severos.

- Italia es famosa por la pasta, la pizza, el helado y el espresso.

- No puedo recordar nombres personales, solo puedo proporcionar información.

- El propósito de la vida varía, a menudo se ve como la realización personal.

- Disfrutar de los momentos de la vida es ciertamente un enfoque maravilloso.

## Recuperación de RAG:

Lo siguiente es una reutilización del ejemplo inicial de la [entrada del Libro de recetas de Recuperación del Lenguaje de Expresión de LangChain](
https://python.langchain.com/docs/expression_language/cookbook/retrieval), pero ejecutado con los [Modelos de Fundación de IA Mixtral 8x7B Instruct](https://catalog.ngc.nvidia.com/orgs/nvidia/teams/ai-foundation/models/mixtral-8x7b) y [Incrustación de Recuperación de Preguntas y Respuestas de NVIDIA](https://catalog.ngc.nvidia.com/orgs/nvidia/teams/ai-foundation/models/nvolve-40k) disponibles en sus entornos de juego. Los ejemplos posteriores en el libro de recetas también se ejecutan según lo esperado, y le recomendamos que explore con estas opciones.

**CONSEJO:** Recomendaríamos usar Mixtral para el razonamiento interno (es decir, seguimiento de instrucciones para extracción de datos, selección de herramientas, etc.) y Llama-Chat para una sola "recapitulación final haciendo una respuesta simple que funcione para este usuario en función del historial y el contexto" respuesta.

```python
%pip install --upgrade --quiet  langchain faiss-cpu tiktoken

from operator import itemgetter

from langchain_community.vectorstores import FAISS
from langchain_core.output_parsers import StrOutputParser
from langchain_core.prompts import ChatPromptTemplate
from langchain_core.runnables import RunnablePassthrough
from langchain_nvidia_ai_endpoints import ChatNVIDIA
```

```output
Note: you may need to restart the kernel to use updated packages.
```

```python
vectorstore = FAISS.from_texts(
    ["harrison worked at kensho"],
    embedding=NVIDIAEmbeddings(model="ai-embed-qa-4"),
)
retriever = vectorstore.as_retriever()

prompt = ChatPromptTemplate.from_messages(
    [
        (
            "system",
            "Answer solely based on the following context:\n<Documents>\n{context}\n</Documents>",
        ),
        ("user", "{question}"),
    ]
)

model = ChatNVIDIA(model="ai-mixtral-8x7b-instruct")

chain = (
    {"context": retriever, "question": RunnablePassthrough()}
    | prompt
    | model
    | StrOutputParser()
)

chain.invoke("where did harrison work?")
```

```output
'Based on the document provided, Harrison worked at Kensho.'
```

```python
prompt = ChatPromptTemplate.from_messages(
    [
        (
            "system",
            "Answer using information solely based on the following context:\n<Documents>\n{context}\n</Documents>"
            "\nSpeak only in the following language: {language}",
        ),
        ("user", "{question}"),
    ]
)

chain = (
    {
        "context": itemgetter("question") | retriever,
        "question": itemgetter("question"),
        "language": itemgetter("language"),
    }
    | prompt
    | model
    | StrOutputParser()
)

chain.invoke({"question": "where did harrison work", "language": "italian"})
```

```output
'Harrison ha lavorato presso Kensho.\n\n(In English: Harrison worked at Kensho.)'
```
