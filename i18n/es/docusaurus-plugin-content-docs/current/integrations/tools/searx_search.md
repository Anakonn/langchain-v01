...
Este cuaderno explica cómo usar una API de búsqueda `SearxNG` alojada de forma independiente para buscar en la web.

Puede [verificar este enlace](https://docs.searxng.org/dev/search_api.html) para obtener más información sobre los parámetros de la `API de Searx`.
...
Para algunos motores, si hay una `respuesta` directa disponible, el wrapper imprimirá la respuesta en lugar de la lista completa de resultados de búsqueda. Puede usar el método `results` del wrapper si desea obtener todos los resultados.
...
SearxNG admite [135 motores de búsqueda](https://docs.searxng.org/user/configured_engines.html). También puede personalizar el wrapper de Searx con parámetros con nombre arbitrarios que se pasarán a la API de búsqueda de Searx. En el siguiente ejemplo, haremos un uso más interesante de los parámetros de búsqueda personalizados de la API de búsqueda de searx.

En este ejemplo, usaremos el parámetro `engines` para consultar Wikipedia.
...
Pasar otros parámetros de Searx como `language`
...
## Obtener resultados con metadatos

En este ejemplo, buscaremos artículos científicos usando el parámetro `categories` y limitaremos los resultados a un `time_range` (no todos los motores admiten la opción de rango de tiempo).

También nos gustaría obtener los resultados de una manera estructurada, incluidos los metadatos. Para ello, usaremos el método `results` del wrapper.
...
En este ejemplo, consultamos `large language models` en la categoría `it`. Luego, filtramos los resultados que provienen de github.
...
También podríamos consultar directamente los resultados de `github` y otros forges de código fuente.
