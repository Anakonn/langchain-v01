...
Los `Natural Language API Toolkits` (`NLAToolkits`) permiten que los Agentes de LangChain planifiquen y combinen llamadas de manera eficiente a través de los puntos finales.

Este cuaderno demuestra una composición de ejemplo de las API `Speak`, `Klarna` y `Spoonacluar`.

### Primero, importar dependencias y cargar el LLM
...
### A continuación, cargar los Natural Language API Toolkits
...
### Usar Auth y agregar más Puntos Finales

Algunos puntos finales pueden requerir autenticación de usuario a través de cosas como tokens de acceso. Aquí mostramos cómo pasar la información de autenticación a través del objeto `Requests` wrapper.

Dado que cada NLATool expone una interfaz de lenguaje natural concisa a su API envuelta, el agente conversacional de nivel superior tiene un trabajo más fácil al incorporar cada punto final para satisfacer la solicitud de un usuario.

**Agregar los puntos finales de Spoonacular.**

1. Ir a la [Consola de la API de Spoonacular](https://spoonacular.com/food-api/console#Profile) y crear una cuenta gratuita.
2. Hacer clic en `Perfil` y copiar su clave API a continuación.
