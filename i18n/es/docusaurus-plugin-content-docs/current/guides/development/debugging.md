...
Si estás construyendo con LLM, en algún momento algo se romperá y tendrás que depurar. Una llamada al modelo fallará, o la salida del modelo estará mal formateada, o habrá algunas llamadas anidadas al modelo y no quedará claro dónde a lo largo del camino se creó una salida incorrecta.

Aquí hay algunas herramientas y funcionalidades diferentes para ayudar en la depuración.

## Rastreo

Las plataformas con capacidades de rastreo como [LangSmith](/docs/langsmith/) son las soluciones más completas para la depuración. Estas plataformas facilitan no solo el registro y la visualización de aplicaciones LLM, sino también la depuración, prueba y refinamiento activos.

Al construir aplicaciones LLM de nivel de producción, plataformas como esta son esenciales.

![Captura de pantalla de la interfaz de depuración de LangSmith que muestra una ejecución de AgentExecutor con detalles de entrada y salida, y una visualización del árbol de ejecución.](../../../../../../static/img/run_details.png "Interfaz de depuración de LangSmith")

## `set_debug` y `set_verbose`

Si estás prototipando en Jupyter Notebooks o ejecutando scripts de Python, puede ser útil imprimir los pasos intermedios de una ejecución de Chain.

Hay varias formas de habilitar la impresión con diferentes grados de verbosidad.

Supongamos que tenemos un agente simple y queremos visualizar las acciones que toma y las salidas de herramientas que recibe. Sin ninguna depuración, esto es lo que vemos:
...
Establecer el indicador global `debug` hará que todos los componentes de LangChain con soporte de devolución de llamada (cadenas, modelos, agentes, herramientas, recuperadores) impriman las entradas que reciben y las salidas que generan. Este es el ajuste más detallado y registrará completamente las entradas y salidas sin procesar.
...
<details> <summary>Salida de la consola</summary>
...
Establecer el indicador `verbose` imprimirá las entradas y salidas en un formato un poco más legible y omitirá el registro de ciertos resultados sin procesar (como las estadísticas de uso de tokens para una llamada a LLM) para que puedas centrarte en la lógica de la aplicación.
...
También puedes limitar la verbosidad a un solo objeto, en cuyo caso solo se imprimirán las entradas y salidas de ese objeto (junto con cualquier otra llamada de devolución de llamada adicional realizada específicamente por ese objeto).
...
Los `Callbacks` son lo que usamos para ejecutar cualquier funcionalidad dentro de un componente fuera de la lógica principal del componente. Todas las soluciones anteriores utilizan `Callbacks` bajo el capó para registrar los pasos intermedios de los componentes. Hay una serie de `Callbacks` relevantes para la depuración que vienen con LangChain de forma predeterminada, como el [FileCallbackHandler](/docs/modules/callbacks/filecallbackhandler). También puedes implementar tus propios callbacks para ejecutar funcionalidad personalizada.

Consulta aquí más información sobre [Callbacks](/docs/modules/callbacks/), cómo usarlos y personalizarlos.
