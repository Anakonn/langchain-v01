...
- LCEL
sidebar_position: 3
title: Lógica de ruta basada en la entrada
translated: true
---

# Lógicamente enrutar dinámicamente basado en la entrada

Este cuaderno cubre cómo hacer enrutamiento en el Lenguaje de Expresión LangChain.

El enrutamiento te permite crear cadenas no deterministas donde la salida de un paso anterior define el siguiente paso. El enrutamiento ayuda a proporcionar estructura y consistencia en las interacciones con LLMs.

Hay dos formas de realizar el enrutamiento:

1. Retornar condicionalmente ejecutables desde un [`RunnableLambda`](/docs/expression_language/primitives/functions) (recomendado)
2. Utilizando un `RunnableBranch`.

Ilustraremos ambos métodos utilizando una secuencia de dos pasos donde el primer paso clasifica una pregunta de entrada como sobre `LangChain`, `Anthropic`, u `Otro`, y luego se enruta a una cadena de prompts correspondiente.

## Configuración del Ejemplo

Primero, creemos una cadena que identificará preguntas entrantes como sobre `LangChain`, `Anthropic`, u `Otro`:
...
Ahora, creemos tres sub cadenas:
...
## Usando una función personalizada (Recomendado)

También puedes usar una función personalizada para enrutar entre diferentes salidas. Aquí tienes un ejemplo:
...
## Usando un RunnableBranch

Un `RunnableBranch` es un tipo especial de runnable que te permite definir un conjunto de condiciones y runnables a ejecutar basado en la entrada. **No** ofrece nada que no puedas lograr en una función personalizada como se describió anteriormente, por lo que recomendamos usar una función personalizada en su lugar.

Un `RunnableBranch` se inicializa con una lista de pares (condición, runnable) y un runnable por defecto. Selecciona qué rama pasando a cada condición la entrada con la que se invoca. Selecciona la primera condición que evalúe a True, y ejecuta el runnable correspondiente a esa condición con la entrada.

Si ninguna de las condiciones proporcionadas coincide, ejecuta el runnable por defecto.
...
Una técnica especialmente útil es usar embeddings para enrutar una consulta al prompt más relevante. Aquí tienes un ejemplo.
