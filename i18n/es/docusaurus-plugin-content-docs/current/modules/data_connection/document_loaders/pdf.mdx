...
>[Formato de documento portátil (PDF)](https://en.wikipedia.org/wiki/PDF), estandarizado como ISO 32000, es un formato de archivo desarrollado por Adobe en 1992 para presentar documentos, incluido el formato de texto e imágenes, de manera independiente del software de aplicación, el hardware y los sistemas operativos.

Esto cubre cómo cargar documentos `PDF` en el formato de documento que usamos aguas abajo.

## Usando PyPDF

Carga PDF usando `pypdf` en una matriz de documentos, donde cada documento contiene el contenido de la página y los metadatos con el número de `página`.
...
Una ventaja de este enfoque es que los documentos se pueden recuperar con los números de página.
...
Usando el paquete `rapidocr-onnxruntime` también podemos extraer imágenes como texto:
...
Esta es la más rápida de las opciones de análisis de PDF y contiene metadatos detallados sobre el PDF y sus páginas, además de devolver un documento por página.
...
Además, puede pasar cualquiera de las opciones de la [documentación de PyMuPDF](https://pymupdf.readthedocs.io/en/latest/app1.html#plain-text/) como argumentos de palabra clave en la llamada `load`, y se pasará a la llamada `get_text()`.

## Usando MathPix

Inspirado en el [https://gist.github.com/danielgross/3ab4104e14faccc12b49200843adab21](https://gist.github.com/danielgross/3ab4104e14faccc12b49200843adab21) de Daniel Gross
...
El paquete `unstructured[all-docs]` actualmente admite la carga de archivos de texto, presentaciones de PowerPoint, HTML, PDF, imágenes y más.
...
Debajo de la superficie, Unstructured crea diferentes "elementos" para diferentes fragmentos de texto. De forma predeterminada, los combinamos, pero puede mantener fácilmente esa separación especificando `mode="elements"`.
...
### Recuperar PDF remotos usando Unstructured

Esto cubre cómo cargar PDF en línea en un formato de documento que podamos usar aguas abajo. Esto se puede usar para varios sitios web de PDF, como https://open.umn.edu/opentextbooks/textbooks/ y https://arxiv.org/archive/

Nota: todos los otros cargadores de PDF también se pueden usar para recuperar PDF remotos, pero `OnlinePDFLoader` es una función heredada y funciona específicamente con `UnstructuredPDFLoader`.
...
PDFMiner es una herramienta que puede ayudar con la extracción de información y el análisis de datos de documentos PDF.
...
### Usando PDFMiner para generar texto HTML

Esto puede ser útil para dividir los textos semánticamente en secciones, ya que el contenido de salida HTML se puede analizar a través de `BeautifulSoup` para obtener información más estructurada y rica sobre el tamaño de la fuente, los números de página, los encabezados/pies de página de PDF, etc.
...
Al igual que PyMuPDF, los Documentos de salida contienen metadatos detallados sobre el PDF y sus páginas, y devuelven un documento por página.
...
## Usando AmazonTextractPDFParser

El cargador AmazonTextractPDFLoader llama al [Servicio Amazon Textract](https://aws.amazon.com/textract/) para convertir los PDF en una estructura de Documento. El cargador realiza OCR puro en este momento, con más funciones como soporte de diseño planificado, dependiendo de la demanda. Se admiten documentos de una y varias páginas con hasta 3000 páginas y 512 MB de tamaño.

Para que la llamada tenga éxito, se requiere una cuenta de AWS, similar a los requisitos de la [CLI de AWS](https://docs.aws.amazon.com/cli/latest/userguide/cli-chap-configure.html).

Además de la configuración de AWS, es muy similar a los otros cargadores de PDF, mientras también admite JPEG, PNG y TIFF y formatos PDF no nativos.
...
## Uso de AzureAIDocumentIntelligenceLoader
...
Esta [implementación actual](https://aka.ms/di-langchain) de un cargador que utiliza `Document Intelligence` puede incorporar el contenido por página y convertirlo en documentos LangChain. El formato de salida predeterminado es Markdown, que se puede encadenar fácilmente con `MarkdownHeaderTextSplitter` para el fragmentado semántico de documentos. También puede usar `mode="single"` o `mode="page"` para devolver textos puros en una sola página o documento dividido por página.
...
## Uso de UpstageLayoutAnalysisLoader

El UpstageLayoutAnalysisLoader invoca la [API de análisis de diseño de Upstage](https://developers.upstage.ai/docs/apis/layout-analyzer) para detectar elementos de documentos, incluidas tablas y figuras, de varios formatos de documentos. Este cargador emplea OCR puro para extraer información textual y detectar elementos dentro de documentos como archivos `JPEG`, `PNG`, `BMP`, `PDF`, `TIFF` y `HEIC`. En el caso de los documentos PDF nacidos digitales, los usuarios tienen la opción de omitir el OCR y utilizar la información de texto dentro del archivo estableciendo use_ocr=False, que es el valor predeterminado. Se admiten documentos de una sola página y de varias páginas, con un límite de 100 páginas y un tamaño de archivo de 50 MB cuando use_ocr=True, mientras que no hay restricciones cuando use_ocr=False (aplicable solo a archivos PDF).

### Requisito previo

Para acceder a la API de análisis de diseño de Upstage, necesita un token de acceso a la API. Consulte la [guía de inicio rápido](https://developers.upstage.ai/docs/getting-started/quick-start) proporcionada para obtener el token de acceso y comenzar a utilizar la API de análisis de diseño de Upstage.
