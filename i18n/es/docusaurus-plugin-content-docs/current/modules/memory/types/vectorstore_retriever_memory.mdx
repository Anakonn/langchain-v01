...
# Respaldado por un Vector Store

`VectorStoreRetrieverMemory` almacena recuerdos en un vector store y consulta los K documentos más "relevantes" cada vez que se llama.

Esto difiere de la mayoría de las otras clases de Memoria en que no rastrea explícitamente el orden de las interacciones.

En este caso, los "documentos" son fragmentos de conversaciones anteriores. Esto puede ser útil para hacer referencia a piezas relevantes de información que se le dijo al AI anteriormente en la conversación.
...
### Inicializa tu vector store

Dependiendo de la tienda que elijas, este paso puede verse diferente. Consulta la documentación relevante del vector store para más detalles.
...
### Crea tu `VectorStoreRetrieverMemory`

El objeto de memoria se instancia a partir de cualquier recuperador de vector store.
...
Vamos a recorrer un ejemplo, nuevamente estableciendo `verbose=True` para que podamos ver el prompt.
