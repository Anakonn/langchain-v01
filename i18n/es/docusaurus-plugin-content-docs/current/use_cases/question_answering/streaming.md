...
Trabajaremos a partir de la aplicación de preguntas y respuestas con fuentes que construimos en el blog post [LLM Powered Autonomous Agents](https://lilianweng.github.io/posts/2023-06-23-agent/) de Lilian Weng en la guía [Returning sources](/docs/use_cases/question_answering/sources).
...
Aquí está la aplicación de preguntas y respuestas con fuentes que construimos en el blog post [LLM Powered Autonomous Agents](https://lilianweng.github.io/posts/2023-06-23-agent/) de Lilian Weng en la guía [Returning sources](/docs/use_cases/question_answering/sources):
...
## Transmisión de resultados finales

Con LCEL es fácil transmitir los resultados finales:
...
Podemos agregar algo de lógica para compilar nuestro flujo a medida que se devuelve:
...
## Transmisión de pasos intermedios

Supongamos que queremos transmitir no solo los resultados finales de la cadena, sino también algunos pasos intermedios. Como ejemplo, tomemos nuestra cadena [Chat history](/docs/use_cases/question_answering/chat_history). Aquí reformulamos la pregunta del usuario antes de pasarla al recuperador. Esta pregunta reformulada no se devuelve como parte de la salida final. Podríamos modificar nuestra cadena para devolver la nueva pregunta, pero con fines de demostración la dejaremos como está.
...
Para transmitir pasos intermedios, utilizaremos el método `astream_log`. Este es un método asincrónico que genera operaciones JSONPatch que, cuando se aplican en el mismo orden que se recibieron, construyen el RunState:
...
Puedes transmitir todos los pasos (predeterminado) o incluir/excluir pasos por nombre, etiquetas o metadatos. En este caso, solo transmitiremos los pasos intermedios que forman parte de la `contextualize_q_chain` y la salida final. Tenga en cuenta que al definir la `contextualize_q_chain` le dimos una etiqueta correspondiente, que ahora podemos filtrar.

Solo mostramos los primeros 20 fragmentos del flujo por legibilidad:
...
Si quisiéramos obtener nuestros documentos recuperados, podríamos filtrar por nombre "Retriever":
...
Para más información sobre cómo transmitir pasos intermedios, consulta la documentación de [LCEL Interface](/docs/expression_language/interface#async-stream-intermediate-steps).
