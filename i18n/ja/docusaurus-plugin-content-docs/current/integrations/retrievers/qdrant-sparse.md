...
>[Qdrant](https://qdrant.tech/) は、オープンソースの高性能なベクトル検索エンジン/データベースです。

>`QdrantSparseVectorRetriever` は、`Qdrant` [v1.7.0](https://qdrant.tech/articles/qdrant-1.7.x/) で導入された[スパース ベクトル](https://qdrant.tech/articles/sparse-vectors/)を使用してドキュメントを検索します。

'qdrant_client' パッケージをインストールします:
