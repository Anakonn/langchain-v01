...
>[Databricks ベクトル検索](https://docs.databricks.com/en/generative-ai/vector-search.html)は、データのベクトル表現とメタデータを ベクトルデータベースに保存できるサーバーレスの類似検索エンジンです。ベクトル検索を使うと、Unity Catalogで管理されるDeltaテーブルから自動更新されるベクトル検索インデックスを作成し、シンプルなAPIでそれらを照会して最も類似したベクトルを返すことができます。

このウォークスルーでは、Databricks ベクトル検索を使って `SelfQueryRetriever` をデモンストレーションします。

## Databricks ベクトルストアインデックスの作成

まずはDatabricks ベクトルストアインデックスを作成し、いくつかのデータを投入しましょう。ここでは、映画のサマリーを含む小さなデモデータセットを用意しています。

**注意:** self-query retrieverを使うには、`lark`(`pip install lark`)とインテグレーション固有の要件をインストールする必要があります。
...
次に、retrieverをインスタンス化しましょう。事前に、ドキュメントがサポートするメタデータフィールドと、ドキュメントの内容の簡単な説明を提供する必要があります。
