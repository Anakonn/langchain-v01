...
>[PubMed®](https://pubmed.ncbi.nlm.nih.gov/) by `The National Center for Biotechnology Information, National Library of Medicine` は、`MEDLINE`、ライフサイエンス雑誌、オンラインブックからの35百万件を超える文献引用で構成されています。引用には、`PubMed Central`およびパブリッシャーのウェブサイトからのフルテキストコンテンツへのリンクが含まれる場合があります。

このノートブックでは、`PubMed`をリトリーバーとして使用する方法について説明します。
