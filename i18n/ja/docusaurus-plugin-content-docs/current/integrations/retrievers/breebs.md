...
>[BREEBS](https://www.breebs.com/)は、オープンコラボレーションの知識プラットフォームです。
誰でも、Google Driveのフォルダに保存されたPDFファイルに基づいて、Breeb(知識カプセル)を作成することができます。
Breebは、LLM/チャットボットがその専門知識を向上させ、幻覚を減らし、情報源にアクセスするために使用できます。
裏では、Breebs は、各反復で有用なコンテキストを seamlessly 提供するために、いくつかの Retrieval Augmented Generation (RAG) モデルを実装しています。

## 利用可能なBreebs一覧

Breebs の完全なリスト(キー(breeb_key)と説明)を取得するには:
https://breebs.promptbreeders.com/web/listbreebs.
コミュニティによってすでに多数のBreebs が作成され、自由に使用できるようになっています。有機化学から神話学、そして誘惑のテクニックや分散型金融のヒントまで、幅広い専門知識をカバーしています。

## 新しいBreebs の作成

新しいBreebs を生成するには、公開共有のGoogle DriveフォルダにPDFファイルをコンパイルし、[BREEBS ウェブサイト](https://www.breebs.com/)の「Breeb を作成」ボタンをクリックして作成プロセスを開始するだけです。現在、最大120ファイル、合計1500万文字までを含めることができます。
