...
# LLMLingua ドキュメントコンプレッサー

>[LLMLingua](https://github.com/microsoft/LLMLingua)は、コンパクトで十分に訓練された言語モデル(例えば、GPT2-small、LLaMA-7B)を利用して、プロンプトから非必須のトークンを識別し、除去します。このアプローチにより、大規模な言語モデル(LLM)での効率的な推論が可能となり、パフォーマンスの低下を最小限に抑えつつ、最大20倍のコンプレッションを実現できます。

このノートブックでは、LLMLinguaをドキュメントコンプレッサーとして使用する方法を示します。
...
まず、シンプルなベクトルストアリトリーバーを初期化し、(チャンクに分割された)2023年の一般教書演説を格納しましょう。リトリーバーを設定して、多数(20件)のドキュメントを取得できるようにします。
...
## LLMLinguaによるコンプレッション

次に、`LLMLinguaCompressor`をコンプレッサーとして使用して、ベースのリトリーバーを`ContextualCompressionRetriever`でラップしましょう。
