...
>[Weaviate](https://weaviate.io/developers/weaviate)はオープンソースのベクトルデータベースです。

>[ハイブリッド検索](https://weaviate.io/blog/hybrid-search-explained)は、検索結果の精度と関連性を向上させるために、複数の検索アルゴリズムを組み合わせる手法です。キーワードベースの検索アルゴリズムとベクトル検索の両方の長所を活かしています。

>`Weaviateのハイブリッド検索`は、検索クエリーとドキュメントの意味とコンテキストを表現するために、疎なベクトルと密なベクトルを使用しています。

このノートブックでは、`Weaviateハイブリッド検索`をLangChainのリトリーバーとして使用する方法を示します。
