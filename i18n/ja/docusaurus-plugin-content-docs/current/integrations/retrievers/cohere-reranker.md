...
>[Cohere](https://cohere.ai/about)は、企業がヒト-マシン間のインタラクションを改善するのに役立つ自然言語処理モデルを提供するカナダのスタートアップです。

このノートブックでは、[Cohereのrerank endpoint](https://docs.cohere.com/docs/reranking)をリトリーバーで使う方法を示します。これは[ContextualCompressionRetriever](/docs/modules/data_connection/retrievers/contextual_compression/)のアイデアを発展させたものです。
...
まずは単純なベクトルストアリトリーバーを初期化し、(チャンクに分けた)2023年の一般教書演説を保存しましょう。リトリーバーには多数(20件)のドキュメントを取得するよう設定できます。
...
## CohereRerank によるリランキング

次に、ベースのリトリーバーを`ContextualCompressionRetriever`でラップします。`CohereRerank`を追加し、Cohereのrerank endpointを使ってリターンされた結果をリランキングします。
