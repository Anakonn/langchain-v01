...
>[you.com API](https://api.you.com)は、LLMの出力を最新の、最も正確な、最も関連性の高い情報に基づかせるためのツールのスイートです。これらの情報は、LLMの学習データセットに含まれていない可能性があります。

## セットアップ

レトリーバーは `langchain-community` パッケージに存在します。
...
[LangSmith](https://smith.langchain.com/)を設定することで、最高レベルの可視化が得られます(必須ではありません)。
