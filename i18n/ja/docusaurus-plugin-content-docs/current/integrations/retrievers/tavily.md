...
>[Tavily's Search API](https://tavily.com)は、リアルタイム、正確、かつ迅速な結果を提供するAIエージェント(LLM)専用の検索エンジンです。

これを[リトリーバー](/docs/modules/data_connection/retrievers)として使用できます。この統合に固有の機能を示します。完了後、[関連するユースケースページ](/docs/use_cases/question_answering)を探索すると、このベクトルストアをより大きなチェーンの一部として使用する方法を学ぶのに役立つかもしれません。
...
[LangSmith](https://smith.langchain.com/)を設定することも役立ちます(必須ではありません)が、最高のオブザーバビリティを実現できます。
