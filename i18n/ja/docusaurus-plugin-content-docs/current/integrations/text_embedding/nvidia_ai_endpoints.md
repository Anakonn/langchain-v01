...
> [NVIDIA AI Foundation Endpoints](https://www.nvidia.com/en-us/ai-data-science/foundation-models/)は、NVIDIA Mixtral 8x7B、Llama 2、Stable DiffusionなどのニューラルネットワークモデルのAPIエンドポイントにユーザーが簡単にアクセスできるようにします。これらのモデルは[NVIDIA APIカタログ](https://build.nvidia.com/)でホストされており、NVIDIAのAIプラットフォーム上で最適化、テスト、ホストされているため、評価、カスタマイズ、加速スタック上での高パフォーマンス実行が簡単です。
>
> [NVIDIA AI Foundation Endpoints](https://www.nvidia.com/en-us/ai-data-science/foundation-models/)を使うと、[NVIDIA DGX Cloud](https://www.nvidia.com/en-us/data-center/dgx-cloud/)上で完全に加速されたスタックから迅速な結果を得ることができます。カスタマイズ後、これらのモデルは[NVIDIA AI Enterprise](https://www.nvidia.com/en-us/data-center/products/ai-enterprise/)を使って企業レベルのセキュリティ、安定性、サポートを備えた任意の場所にデプロイできます。
>
> これらのモデルは、以下に示すように、[`langchain-nvidia-ai-endpoints`](https://pypi.org/project/langchain-nvidia-ai-endpoints/)パッケージを使って簡単にアクセスできます。

このサンプルでは、[NVIDIA Retrieval QA Embedding Model](https://catalog.ngc.nvidia.com/orgs/nvidia/teams/ai-foundation/models/nvolve-40k)を使って[retrieval-augmented generation](https://developer.nvidia.com/blog/build-enterprise-retrieval-augmented-generation-apps-with-nvidia-retrieval-qa-embedding-model/)を行うためのLangChainの使用方法を説明します。

チャットモデルにこのAPIでアクセスする方法については、[ChatNVIDIA](https://python.langchain.com/docs/integrations/chat/nvidia_ai_endpoints/)のドキュメントを参照してください。
...
1. [NVIDIA](https://build.nvidia.com/)のフリーアカウントを作成する
2. `Retrieval`タブを選択し、使用したいモデルを選択する
3. `Input`で`Python`タブを選択し、`Get API Key`をクリックする。次に`Generate Key`をクリックする
...
そのリストの中に、LLMと組み合わせて効果的なRAGソリューションを実現できる埋め込みモデルが見つかるはずです。`NVIDIAEmbeddings`モデルを使えば、このモデルとうまく連携できます。

## 初期化

埋め込みモデルを初期化する際は、引数でモデルを指定するか(例: `ai-embed-qa-4`)、デフォルトのモデルを使うこともできます。
...
このモデルは微調整されたE5-largeモデルで、以下のような`Embeddings`メソッドをサポートしています:

- `embed_query`: クエリサンプルのクエリ埋め込みを生成する
- `embed_documents`: 検索対象のドキュメントリストの埋め込みを生成する
- `aembed_quey`/`embed_documents`: 上記の非同期バージョン

### **類似度**

以下のデータポイントを埋め込むための使用方法、フォーマット、速度を簡単にテストしてみます:

**クエリ:**

- コムチャトカの天気はどうですか?
- イタリアはどのような料理で有名ですか?
- 私の名前は何ですか? 覚えていないと思いますよ...
- 人生の目的って何ですか?
- 人生の目的は楽しむことですね :D

**ドキュメント:**

- コムチャトカの天気は寒く、冬が長くて厳しいです。
- イタリアはパスタ、ピザ、ジェラート、エスプレッソで有名です。
- 個人名は覚えられませんが、情報は提供できます。
- 人生の目的は個人によって異なり、多くは自己実現と考えられています。
- 人生の瞬間を楽しむのは素晴らしいアプローチですね。
...
埋め込みを生成したので、その結果の類似度をチェックして、リトリーバルタスクで適切な回答となりそうなドキュメントを確認しましょう:
...
参考までに、システムに送信したクエリとドキュメントは以下の通りです:
...
以下は、[LangChain Expression Language Retrieval Cookbook entry](
https://python.langchain.com/docs/expression_language/cookbook/retrieval)の初期例を再利用したものですが、AI Foundation Models' [Mixtral 8x7B Instruct](https://catalog.ngc.nvidia.com/orgs/nvidia/teams/ai-foundation/models/mixtral-8x7b)および[NVIDIA Retrieval QA Embedding](https://catalog.ngc.nvidia.com/orgs/nvidia/teams/ai-foundation/models/nvolve-40k)モデルを使用して実行しています。クックブックの後続の例も期待どおりに実行されるので、これらのオプションを探索することをお勧めします。

**ヒント:** Mixtrialを内部推論(データ抽出、ツール選択など)に、Llama-Chatを履歴とコンテキストに基づいてこのユーザーに適した単純な応答を行う最終的な "まとめ" に使うことをお勧めします。
