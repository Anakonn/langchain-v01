...
>[LLMonitor](https://llmonitor.com?utm_source=langchain&utm_medium=py&utm_campaign=docs)は、コストと使用状況の分析、ユーザー追跡、トレーシング、評価ツールを提供するオープンソースの可観測性プラットフォームです。
...
[llmonitor.com](https://llmonitor.com?utm_source=langchain&utm_medium=py&utm_campaign=docs)でアカウントを作成し、新しいアプリの`tracking id`をコピーします。

それが手に入ったら、次のように実行してそれを環境変数に設定します:
...
コールバックハンドラを`run`メソッドに渡して、関連するチェーンとLLMコールがすべて正しく追跡されるようにしてください。

メタデータに`agent_name`を渡すことをお勧めします。これにより、ダッシュボードでエージェントを区別できます。
...
ユーザー追跡を使うと、ユーザーを識別し、そのコストや会話などを追跡できます。
...
統合に関する質問や問題がある場合は、[Discord](http://discord.com/invite/8PafSG58kK)でLLMonitorチームに連絡するか、[email](mailto:vince@llmonitor.com)でお問い合わせください。
