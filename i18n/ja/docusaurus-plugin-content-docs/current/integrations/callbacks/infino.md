...
>[Infino](https://github.com/infinohq/infino)は、ログ、メトリクス、トレースのための拡張可能なテレメトリストアです。Infinoは、単独の可観測性ソリューションとして機能したり、可観測性スタックのストレージレイヤーとして機能したりすることができます。

この例では、`LangChain`と[Infino](https://github.com/infinohq/infino)を介してOpenAIおよびChatOpenAIモデルを呼び出す際に、以下のようなことを追跡する方法を示しています:

* プロンプト入力
* `ChatGPT`または他の`LangChain`モデルからの応答
* レイテンシ
* エラー
* 消費したトークン数
...
## Infinoサーバーの起動、Infinoクライアントの初期化
...
## 例1: LangChain OpenAI Q&A; Infinoにメトリクスとログを公開する
...
次に、matplotlibを使ってレイテンシ、エラー、消費トークン数のグラフを作成します。
