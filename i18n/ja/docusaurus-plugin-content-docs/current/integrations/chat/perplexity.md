...
このノートブックでは、Perplexity chat モデルの使い始め方について説明します。
...
提供されたコードは、環境変数に PPLX_API_KEY が設定されていることを前提としています。API キーを手動で指定し、別のモデルを選択したい場合は、次のコードを使用できます:

```python
chat = ChatPerplexity(temperature=0, pplx_api_key="YOUR_API_KEY", model="pplx-70b-online")
```

利用可能なモデルのリストは[こちら](https://docs.perplexity.ai/docs/model-cards)で確認できます。再現性を確保するため、このノートブックでは API キーを動的に取得することにします。
...
プロンプトの書式設定や構造は、通常通りに行うことができます。次の例では、猫に関するジョークを尋ねています。
...
## `ChatPerplexity` はストリーミング機能もサポートしています:
