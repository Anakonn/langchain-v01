...
このノートブックでは、Anthropicチャットモデルの使い方を紹介します。

## Setup

セットアップ手順については、[Anthropic Platformページ](/docs/integrations/platforms/anthropic.mdx)のインストールおよび環境設定セクションを参照してください。
...
提供されたコードは、環境変数に`ANTHROPIC_API_KEY`が設定されていることを前提としています。APIキーを手動で指定し、異なるモデルを選択したい場合は、次のコードを使用できます。
...
これらのデモでは、Claude 3 Opusモデルを使用します。また、`claude-3-sonnet-20240229`を使用してSonnetモデルのローンチバージョンを使用することもできます。

モデルの比較ドキュメントは[こちら](https://docs.anthropic.com/claude/docs/models-overview#model-comparison)で確認できます。
...
## `ChatAnthropic` は非同期およびストリーミング機能もサポートしています。
...
Anthropicの[ツール呼び出し、またはツール使用API](https://docs.anthropic.com/claude/docs/functions-external-tools)を使用すると、モデルが呼び出すツールを定義できます。これは、ツールを使用するチェーンやエージェントを構築するため、およびモデルから構造化された出力を得るために非常に便利です。

:::note

Anthropicのツール呼び出し機能はまだベータ版です。

:::

### bind_tools()

`ChatAnthropic.bind_tools`を使用すると、Pydanticクラス、dictスキーマ、LangChainツール、さらには関数をツールとしてモデルに簡単に渡すことができます。内部ではこれらがAnthropicツールスキーマに変換され、以下のようになります。
...
出力メッセージの内容がテキストブロックとtool_useブロックを含むリストであることに注意してください。
...
重要なのは、ツール呼び出しが`tool_calls`に抽出され、標準化されたモデルに依存しない形式であることです。
...
ChatAnthropicモデルの出力は常に単一のAIメッセージで、単一の文字列またはコンテンツブロックのリストを含むことができます。コンテンツブロックはテキストブロックまたはツール使用ブロックであり、複数の各ブロックが交互に存在することができます。

:::

### ツール呼び出しの解析

`langchain_anthropic.output_parsers.ToolsOutputParser`を使用すると、Anthropic AIメッセージからツール呼び出しをPydanticオブジェクトに簡単に解析できます。
...
[BaseChatModel.with_structured_outputインターフェース](/docs/modules/model_io/chat/structured_output)を使用すると、チャットモデルから構造化された出力を簡単に得ることができます。`ChatAnthropic.with_structured_output`を使用すると（内部でツール呼び出しを使用）、特定の形式で出力をより信頼性高く返すようモデルに指示できます。
...
の間にあり、最初のGetWeather呼び出しのみを返すのに対し、2番目のアプローチはリストを返します。

### モデルにツール結果を渡す

適切な`tool_call_id`を持つ`ToolMessage`を使用して、ツール結果をモデルに渡すことができます。
...
Anthropicは現在、ツール呼び出しのストリーミングをサポートしていません。ストリーミングを試みると、単一の最終メッセージが返されます。
...
AnthropicのClaude-3モデルは、画像およびテキスト入力の両方に対応しています。以下のように使用できます。
