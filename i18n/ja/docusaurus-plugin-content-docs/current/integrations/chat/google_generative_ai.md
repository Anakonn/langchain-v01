...
[langchain-google-genai](https://pypi.org/project/langchain-google-genai/)統合パッケージの`ChatGoogleGenerativeAI`クラスを使用して、Google AIの`gemini`、`gemini-vision`モデル、およびその他の生成モデルにアクセスできます。
...
Geminiは現時点で`SystemMessage`をサポートしていませんが、最初の人間のメッセージに追加することができます。そのような動作が必要な場合は、`convert_system_message_to_human`を`True`に設定してください。
...
`ChatGoogleGenerativeAI`はネイティブでストリーミングとバッチ処理をサポートしています。以下に例を示します。
...
画像を提供するには、`List[dict]`型のコンテンツを持つ人間のメッセージを渡します。各dictには、`image_url`型の画像値または`text`型のテキスト値が含まれています。
`image_url`の値は以下のいずれかになります:
...
このドキュメントが書かれた時点(2023/12/12)では、Geminiはプロンプトの種類と構造に一定の制限があります。具体的には以下の通りです:

1. マルチモーダル(画像)入力を提供する場合、"human"(ユーザー)タイプのメッセージは1つのみ許可されます。複数のメッセージ(ただし、単一の人間のメッセージには複数のコンテンツエントリーを含めることができます)は許可されません。
2. システムメッセージは受け付けられません。
3. 通常のチャット会話では、human/ai/human/aiの交互パターンに従う必要があります。連続してAIまたは人間のメッセージを提供することはできません。
4. メッセージがLLMのセーフティチェックに違反する場合、ブロックされる可能性があります。その場合、モデルは空の応答を返します。

### セーフティ設定

Geminiモデルにはデフォルトのセーフティ設定がありますが、これらは上書きできます。モデルから多くの"Safety Warnings"が返される場合は、モデルの`safety_settings`属性を調整してみてください。例えば、危険なコンテンツのセーフティブロックを無効にするには、以下のようにLLMを構築できます:
...
利用可能なカテゴリーとしきい値の一覧については、Google の[safety setting types](https://ai.google.dev/api/python/google/generativeai/types/SafetySettingDict)を参照してください。

## その他の設定

`ChatGoogleGenerativeAI`には以下のパラメーターを渡して、SDKの動作をカスタマイズできます:

- `client_options`: Google APIクライアントに渡す[クライアントオプション](https://googleapis.dev/python/google-api-core/latest/client_options.html#module-google.api_core.client_options)、例えば`client_options["api_endpoint"]`のようなカスタムオプション
