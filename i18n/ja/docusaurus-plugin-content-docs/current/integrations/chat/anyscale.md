...
このノートブックでは、[Anyscale Endpoints](https://endpoints.anyscale.com/)用の `langchain.chat_models.ChatAnyscale` の使用方法を示します。

* `ANYSCALE_API_KEY` 環境変数を設定する
* または `anyscale_api_key` キーワード引数を使用する
...
# Anyscale Endpointsで提供されているモデルをそれぞれ試してみましょう
...
# ChatOpenAIでサポートされている非同期メソッドやその他の機能を使うことができます

このようにすれば、3つのリクエストの所要時間は最も長いリクエストの時間だけで済みます。
