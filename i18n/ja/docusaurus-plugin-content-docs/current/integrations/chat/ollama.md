...
[Ollama](https://ollama.ai/) は、Llama 2 などのオープンソースの大規模言語モデルをローカルで実行することを可能にします。

Ollama は、モデルの重み、設定、およびデータを Modelfile によって定義された単一のパッケージにまとめます。

これにより、セットアップおよび設定の詳細（GPU 使用を含む）が最適化されます。

サポートされているモデルおよびモデルのバリアントの完全なリストについては、[Ollama model library](https://github.com/jmorganca/ollama#model-library) を参照してください。

## セットアップ

まず、ローカル Ollama インスタンスをセットアップして実行するための[これらの指示](https://github.com/jmorganca/ollama)に従います：

* Ollama をサポートされているプラットフォーム（Windows Subsystem for Linux など）に[ダウンロード](https://ollama.ai/download)してインストールします
* `ollama pull <name-of-model>` を使用して使用可能な LLM モデルを取得します
    * [model library](https://ollama.ai/library) で利用可能なモデルのリストを表示します
    * 例: `ollama pull llama3`
* これにより、デフォルトのタグ付けされたバージョンのモデルがダウンロードされます。通常、デフォルトは最新の最小サイズのパラメータモデルを指します。

> Mac では、モデルは `~/.ollama/models` にダウンロードされます
>
> Linux（または WSL）では、モデルは `/usr/share/ollama/.ollama/models` に保存されます

* このように興味のあるモデルの正確なバージョンを指定します `ollama pull vicuna:13b-v1.5-16k-q4_0`（このインスタンスでの[`Vicuna`](https://ollama.ai/library/vicuna/tags)モデルのさまざまなタグを表示）
* すべてのプルされたモデルを表示するには、`ollama list` を使用します
* コマンドラインから直接モデルとチャットするには、`ollama run <name-of-model>` を使用します
* さらにコマンドについては [Ollama documentation](https://github.com/jmorganca/ollama) を参照してください。利用可能なコマンドを見るためにターミナルで `ollama help` を実行してください。

## 使用法

サポートされているパラメータの完全なリストは、[API reference page](https://api.python.langchain.com/en/latest/llms/langchain.llms.ollama.Ollama.html) で確認できます。

LLaMA `chat` モデル（例：`ollama pull llama3`）を使用している場合は、`ChatOllama` インターフェースを使用できます。

これには、システムメッセージとユーザー入力のための[特別なトークン](https://huggingface.co/blog/llama2#how-to-prompt-llama-2)が含まれます。

## モデルとの対話

プルされたローカルモデルと対話するいくつかの方法を紹介します

#### 直接ターミナルで：

* すべてのローカルモデルは自動的に `localhost:11434` で提供されます
* `ollama run <name-of-model>` を実行して、コマンドラインから直接対話を開始します

### API 経由

Ollama の API エンドポイントに `application/json` リクエストを送信して対話します。
...
すべてのエンドポイントについては、Ollama の[API documentation](https://github.com/jmorganca/ollama/blob/main/docs/api.md) を参照してください。

#### LangChain 経由

LangChain アプリケーションで `ChatOllama` チャットモデルを使用する典型的な基本例を参照してください。
...
LCEL チェーンは、レスポンスのストリーミングや非同期サポートなどの追加機能を標準で提供します
...
ストリーミング非同期サポートの例はこちらです - 上記で作成された単一のチェーンですべて可能です。
...
チェーンが作成されたときに使用できる他のインターフェースについては、[LangChain Expressive Language (LCEL) Interface](/docs/expression_language/interface) を参照してください。

## ソースからのビルド

ソースからのビルドに関する最新の指示については、[Building from Source](https://github.com/jmorganca/ollama?tab=readme-ov-file#building)に関する Ollama ドキュメントを確認してください。

## 抽出

最新バージョンの Ollama を使用し、[`format`](https://github.com/jmorganca/ollama/blob/main/docs/api.md#json-mode) フラグを指定します。`format` フラグは、モデルに JSON 形式で応答を生成させます。

> **注意:** 便利な実験的な[OllamaFunctions](/docs/integrations/chat/ollama_functions)ラッパーも試してみてください。
...
Ollama は、[bakllava](https://ollama.ai/library/bakllava) や [llava](https://ollama.ai/library/llava) などのマルチモーダル LLM をサポートしています。

`tags` を使用してモデルのバージョンの完全なセットを参照してください。例えば [Llava](https://ollama.ai/library/llava/tags)。

`ollama pull bakllava` で希望する LLM をダウンロードします。

マルチモーダルをサポートするために、最新バージョンの Ollama に更新してください。

以下に ChatOllama マルチモーダルサポートの使用方法の典型的な例を示します：
