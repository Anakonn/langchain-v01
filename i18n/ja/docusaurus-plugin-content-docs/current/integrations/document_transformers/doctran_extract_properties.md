...
[Doctran](https://github.com/psychic-api/doctran) ライブラリを使用すると、OpenAIの関数呼び出し機能を利用して、特定のメタデータを抽出することで、文書から有用な機能を抽出することができます。

文書からメタデータを抽出することは、以下のようなさまざまなタスクに役立ちます:

* **分類:** 文書を異なるカテゴリに分類する
* **データマイニング:** 分析に使用できる構造化データを抽出する
* **スタイル変換:** テキストの書き方を、ユーザーの入力に合わせてより近いものに変更し、ベクトル検索の結果を改善する
...
文書からプロパティを抽出した後、メタデータに提供されたプロパティを含む新しい文書として結果が返されます。
