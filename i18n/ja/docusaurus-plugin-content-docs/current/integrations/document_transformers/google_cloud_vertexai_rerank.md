...
> [Vertex Search Ranking API](https://cloud.google.com/generative-ai-app-builder/docs/ranking)は、[Vertex AI Agent Builder](https://cloud.google.com/generative-ai-app-builder/docs/builder-apis)の単独APIの1つです。クエリに対して最も関連性の高い文書をランク付けします。エンベディングは文書とクエリの意味的類似性しか見ませんが、ランキングAPIは文書がクエリにどの程度よく答えているかを正確なスコアで提供できます。ランキングAPIは、初期の候補文書を取得した後、検索結果の品質を向上させるのに使えます。

>ランキングAPIはステートレスなので、APIを呼び出す前に文書をインデックス化する必要はありません。クエリと文書を渡すだけです。これにより、任意の文書検索システムからの文書をランク付けするのに適しています。

>詳細は[Rank and rerank documents](https://cloud.google.com/generative-ai-app-builder/docs/ranking)をご覧ください。
...
この例では、[Google Wikiページ](https://en.wikipedia.org/wiki/Google)を使って、Vertex Ranking APIの動作を説明します。

標準的なパイプラインの`load -> split -> embed data`を使います。

エンベディングは[Vertex Embeddings API](https://cloud.google.com/vertex-ai/generative-ai/docs/embeddings/get-text-embeddings#supported_models)モデル - `textembedding-gecko@003`を使って作成しています。
...
### Vertex Ranking APIのテスト

`basic_retriever`と`retriever_with_reranker`に同じクエリを投げ、取得した文書を比較してみましょう。

ランキングAPIは`basic_retriever`の出力を受け取り、ランク付けします。

ランキングAPIは、ランキングの質を向上させ、クエリに対する各レコードの関連性を示すスコアを決定するのに使われます。

ランク付けされていない文書とランク付けされた文書の違いがわかります。ランキングAPIは、意味的に最も関連性の高い文書をLLMのコンテキストウィンドウの上位に移動させ、より良い答えと推論を形成するのに役立ちます。
...
ランク付けされた文書をいくつか見てみましょう。retrieverはまだ関連性の高いLangchainタイプの[documents](https://api.python.langchain.com/en/latest/documents/langchain_core.documents.base.Document.html)を返しますが、メタデータフィールドには、ランキングAPIから得られた`relevance_score`も含まれています。
...
これは、Vertex Ranking APIを使ったランク付けを行うRAGチェーンの完全な例で、簡単なプロンプトテンプレートを示しています。
