...
文書をエンベディングを通して比較することには、複数の言語にわたって機能するという利点がある。"Harrison says hello"と"Harrison dice hola"は、意味的に同じ意味を持つため、ベクトル空間上で同様の位置を占めることになる。

しかし、文書をベクトル化する前に、LLMを使って**他の言語に文書を翻訳**するのが有用な場合もある。これは、ユーザーが異なる言語でナレッジベースに問い合わせることが期待される場合や、特定の言語に対して最先端のエンベディングモデルが利用できない場合に特に役立つ。

これは、OpenAIの関数呼び出し機能を使って文書を言語間で翻訳する[Doctran](https://github.com/psychic-api/doctran)ライブラリを使って実現できる。
...
文書を翻訳した後、結果は対象言語に翻訳されたpage_contentを持つ新しい文書として返されます。
