...
ベクトルストアナレッジベースで使用されるドキュメントは通常、ナラティブまたは会話形式で保存されています。しかし、ほとんどのユーザークエリは質問形式です。ドキュメントをベクトル化する前に**ドキュメントをQ&A形式に変換**すれば、関連性の高いドキュメントを検索する可能性が高まり、関連性の低いドキュメントを検索する可能性が低くなります。

[Doctran](https://github.com/psychic-api/doctran)ライブラリを使用すると、OpenAIの関数呼び出し機能を使ってドキュメントを「質問」することができます。

生のドキュメントと質問されたドキュメントのベクトル類似度スコアに関するベンチマークについては、[このノートブック](https://github.com/psychic-api/doctran/blob/main/benchmark.md)を参照してください。
...
ドキュメントを質問すると、メタデータにクエスチョンとアンサーが含まれた新しいドキュメントが返されます。
