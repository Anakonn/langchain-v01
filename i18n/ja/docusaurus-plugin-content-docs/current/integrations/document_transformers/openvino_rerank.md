...
[OpenVINO™](https://github.com/openvinotoolkit/openvino) は、AI推論の最適化とデプロイのためのオープンソースツールキットです。 OpenVINO™ ランタイムは、x86およびARM CPU、Intel GPUを含む様々なハードウェア[デバイス](https://github.com/openvinotoolkit/openvino?tab=readme-ov-file#supported-hardware-matrix)をサポートしています。コンピュータービジョン、音声認識、自然言語処理、その他の一般的なタスクでの深層学習のパフォーマンスを向上させることができます。

Hugging Faceのrerank modelは、``OpenVINOReranker``クラスを使ってOpenVINOでサポートできます。Intel GPUをお持ちの場合は、`model_kwargs={"device": "GPU"}`を指定して、GPUでの推論を行うことができます。
...
2023年の一般教書演説(チャンク単位)を保存し、簡単なベクトルストアリトリーバーを初期化してみましょう。リトリーバーには、多数(20件)のドキュメントを取得するように設定できます。
...
次に、`ContextualCompressionRetriever`でベースのリトリーバーをラップし、`OpenVINOReranker`をコンプレッサーとして使用します。
...
リランキング後、トップ4のドキュメントはベースのリトリーバーによって取得されたトップ4のドキュメントとは異なります。
...
``OVModelForSequenceClassification``を使って、rerank modelをOpenVINO IRフォーマットにエクスポートし、ローカルフォルダからモデルを読み込むことができます。
