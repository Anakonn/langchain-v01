...
> [Annoy](https://github.com/spotify/annoy) (`Approximate Nearest Neighbors Oh Yeah`) は、与えられた問い合わせ点に近い点を検索するためのPythonバインディングを持つC++ライブラリです。また、多くのプロセスが同じデータを共有できるように、メモリにmmapされた大規模な読み取り専用ファイルベースのデータ構造を作成します。

このノートブックでは、`Annoy`ベクトルデータベースに関連する機能の使用方法を示します。

:::note
注意: Annoyは読み取り専用です - インデックスが構築されたら、これ以上埋め込みを追加することはできません!
新しエントリを段階的に追加したい場合は、代替手段を選択するのがよいでしょう。
