...
このノートブックでは、`Google Cloud Vertex AI Vector Search`ベクトルデータベースに関連する機能の使用方法を示します。

> [Google Vertex AI Vector Search](https://cloud.google.com/vertex-ai/docs/vector-search/overview)は、以前はVertex AI Matching Engineとして知られていた、業界をリードする高スケール低レイテンシのベクトルデータベースを提供します。これらのベクトルデータベースは、ベクトル類似性マッチングまたは近似最近傍(ANN)サービスと一般に呼ばれています。

**注意**: Langchain APIでは、すでに作成されたエンドポイントとデプロイされたインデックスを期待しています。インデックスの作成には最大1時間かかる可能性があります。

> インデックスの作成方法については、[インデックスの作成とエンドポイントへのデプロイ](#create-index-and-deploy-it-to-an-endpoint)のセクションを参照してください。
すでにデプロイされたインデックスがある場合は、[テキストからベクトルストアを作成する](#create-vector-store-from-texts)に進んでください。

## インデックスの作成とエンドポイントへのデプロイ

- このセクションでは、新しいインデックスを作成し、エンドポイントにデプロイする方法を示します。
...
### [VertexAIEmbeddings](https://python.langchain.com/docs/integrations/text_embedding/google_vertex_ai_palm/)をembeddingsモデルとして使用する
...
**注意:** インデックスを作成する際は、"BATCH_UPDATE"または"STREAM_UPDATE"のいずれかの"index_update_method"を指定する必要があります。
> バッチインデックスは、一定期間にわたって蓄積されたデータをまとめてインデックス化する場合に使用します。ストリーミングインデックスは、新しいデータがデータストアに追加されるたびにすぐにインデックス化する必要がある場合に使用します。どのタイプを選択するかは重要です。設定や要件が異なるためです。

詳細については、[公式ドキュメント](https://cloud.google.com/vertex-ai/docs/vector-search/create-manage-index#create-index-batch)を参照してください。
...
注意: 既存のインデックスとエンドポイントがある場合は、以下のコードを使ってそれらをロードできます。
...
### オプション: データストアにチャンクを格納したベクトルを作成することもできます。
