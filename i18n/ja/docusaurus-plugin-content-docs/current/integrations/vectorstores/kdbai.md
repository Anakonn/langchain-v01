...
> [KDB.AI](https://kdb.ai/)は、リアルタイムデータを使用して、高度な検索、レコメンデーション、パーソナライゼーションを提供することで、スケーラブルで信頼性の高いAIアプリケーションを構築できる強力な知識ベースのベクトルデータベースおよび検索エンジンです。

[このサンプル](https://github.com/KxSystems/kdbai-samples/blob/main/document_search/document_search.md)では、KDB.AIを使用して非構造化テキストドキュメントに対してセマンティック検索を実行する方法を示します。

エンドポイントとAPIキーにアクセスするには、[こちらからKDB.AIに登録してください](https://kdb.ai/get-started/)。

開発環境のセットアップは、[KDB.AIの前提条件ページ](https://code.kx.com/kdbai/pre-requisites.html)の手順に従ってください。

以下の例は、LangChainを通してKDB.AIと対話する方法の一部を示しています。
