...
> 過去の会話を思い出し、理解し、抽出することで、パーソナライズされたAI体験を実現します。

> [Zep](https://www.getzep.com)は、AIアシスタントアプリ用の長期メモリサービスです。
> Zepを使えば、AIアシスタントに過去の会話を思い出す機能を提供でき、
> 幻覚、レイテンシ、コストを削減できます。

> Zep Cloudに興味がありますか? [Zep Cloud Installation Guide](https://help.getzep.com/sdks)と[Zep Cloud Vector Store example](https://help.getzep.com/langchain/examples/vectorstore-example)をご覧ください。
...
以下の例では、Zepの自動埋め込み機能を使用しています。これにより、ドキュメントがZepサーバーで低レイテンシの埋め込みモデルを使って自動的に埋め込まれます。

## 注意

- これらの例ではZepの非同期インターフェイスを使用しています。同期インターフェイスを使用する場合は、メソッド名から`a`を削除してください。
- `Embeddings`インスタンスを渡した場合、Zepはこれを使ってドキュメントを埋め込み、自動埋め込みは行いません。
  また、ドキュメントコレクションの`isAutoEmbedded`を`false`に設定する必要があります。
- `isAutoEmbedded`を`false`に設定した場合は、`Embeddings`インスタンスを渡す必要があります。

## コレクションからドキュメントを読み込むか作成する
...
Zepには、検索結果をMMRで再ランキングする機能が組み込まれています。
...
メタデータフィルタを使用して結果を絞り込みます。まず、別の本"Adventures of Sherlock Holmes"を読み込みます。
...
両方の本からの結果が表示されます。`source`メタデータに注目してください。
