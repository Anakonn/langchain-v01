...
# Oracle AIベクトル検索: ベクトルストア

Oracle AIベクトル検索は、キーワードではなくセマンティクスに基づいてデータを検索できるAI(人工知能)ワークロード向けに設計されています。
Oracle AIベクトル検索の最大の利点の1つは、非構造化データのセマンティック検索と、ビジネスデータの関係検索を1つのシステムで組み合わせられることです。
これは強力であるだけでなく、専用のベクトルデータベースを追加する必要がないため、複数のシステム間でのデータ断片化の問題も解決できます。

さらに、Oracleはデータベース技術の構築に長年取り組んできたため、ベクトルはOracle Databaseの最強の機能から恩恵を受けることができます。以下がその一例です:

 * パーティショニングサポート
 * Real Application Clustersによるスケーラビリティ
 * Exadataスマートスキャン
 * 地理的に分散したデータベース間でのシャード処理
 * トランザクション
 * 並列SQL
 * ディザスターリカバリ
 * セキュリティ
...
### Langchain with Oracle AI Vector Searchを使用するための前提条件

Oracle AIベクトル検索とLangchainを使用するには、Oracleのpythonクライアントドライバーをインストールする必要があります。

```python
# pip install oracledb
```

### Oracle AIベクトル検索に接続する
...
### Oracle AIベクトル検索で遊ぶために必要な依存関係をインポートする
...
### 異なる距離戦略でAIベクトル検索のベクトルストアを作成する

まず、異なる距離関数を使用した3つのベクトルストアを作成します。まだインデックスを作成していないため、これらはテーブルの作成のみを行います。後ほど、これらのベクトルストアを使ってHNSWインデックスを作成します。

Oracleデータベースに手動で接続すると、Documents_DOT、Documents_COSINE、Documents_EUCLIDEANの3つのテーブルが表示されます。

さらに、Documents_DOT_IVF、Documents_COSINE_IVF、Documents_EUCLIDEAN_IVFの3つのテーブルを作成し、HNSWインデックスの代わりにIVFインデックスを作成します。
...
### 各距離戦略に特定のパラメーターを使ってインデックスを作成する実演
...
### 6つのベクトルストアすべてで高度な検索を行います。これらの3つの検索には、フィルタ付きとフィルタなしのバージョンがあります。フィルタは、ID 101のドキュメントのみを選択し、それ以外をすべてフィルタリングします。
...
Oracle AIベクトル検索を使ったエンドツーエンドのRAGパイプラインの構築については、完全なデモガイド[Oracle AI Vector Search End-to-End Demo Guide](https://github.com/langchain-ai/langchain/tree/master/cookbook/oracleai_demo.md)をご覧ください。
