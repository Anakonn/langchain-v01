...
> [Upstash Vector](https://upstash.com/docs/vector/overall/whatisvector)は、ベクトル埋め込みを扱うためのサーバーレスベクトルデータベースです。
>
> ベクトル langchain統合は、[upstash-vector](https://github.com/upstash/vector-py)パッケージのラッパーです。
>
> Pythonパッケージは、[ベクトルREST API](https://upstash.com/docs/vector/api/get-started)を使用しています。

## インストール

[Upstashコンソール](https://console.upstash.com/vector)から、希望の次元とディスタンスメトリックを持つ無料のベクトルデータベースを作成します。

次に、`UpstashVectorStore`インスタンスを作成できます:

- `UPSTASH_VECTOR_URL`と`UPSTASH_VECTOR_TOKEN`の環境変数を提供する
- コンストラクタにそれらのパラメータを渡す
- コンストラクタにUpstash Vectorの`Index`インスタンスを渡す

また、与えられたテキストをベクトル埋め込みに変換するための`Embeddings`インスタンスが必要です。ここでは`OpenAIEmbeddings`を例として使用しています。
...
`UpstashVectorStore`を作成する別の方法は、[モデルを選択してUpstash Vectorインデックスを作成](https://upstash.com/docs/vector/features/embeddingmodels#using-a-model)し、`embedding=True`を渡すことです。この設定では、ドキュメントやクエリがテキストとしてUpstashに送信され、そこで埋め込まれます。

```python
store = UpstashVectorStore(embedding=True)
```

この方法を試してみたい場合は、上記のようにstoreの初期化を更新し、チュートリアルの残りの部分を実行できます。

## ドキュメントの読み込み

サンプルテキストファイルを読み込み、ベクトル埋め込みに変換できるチャンクに分割します。
...
ベクトルストアは、埋め込みオブジェクトを使ってテキストチャンクを埋め込み、それらをデータベースにバッチ挿入します。これにより、挿入されたベクトルのIDの配列が返されます。
...
データベースは、ベクトルまたはテキストプロンプトを使ってクエリできます。
テキストプロンプトが使用される場合、最初に埋め込みに変換されてからクエリされます。

`k`パラメータは、クエリから返す結果の数を指定します。
...
クエリの結果にスコアを含めることができます。

> クエリリクエストで返されるスコアは、0から1の間の正規化された値で、1は最も高い類似性、0は最も低い類似性を示します。使用された類似性関数の詳細については、[ドキュメント](https://upstash.com/docs/vector/overall/features#vector-similarity-functions)を参照してください。
...
info関数を使って、ディスタンスメトリックや次元など、データベースに関する情報を取得できます。
