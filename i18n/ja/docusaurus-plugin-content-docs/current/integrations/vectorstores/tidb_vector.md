...
> [TiDB Cloud](https://tidbcloud.com/)は、専用およびサーバーレスのオプションを提供する包括的なデータベースアズアサービス(DBaaS)ソリューションです。TiDB Serverlessは、MySQL環境に組み込みのベクトル検索を統合しています。この機能強化により、新しいデータベースや追加の技術スタックを必要とせずに、TiDB Serverlessを使ってAIアプリケーションを開発できるようになりました。プライベートベータに参加して、この機能を初めて体験してみてください。https://tidb.cloud/ai

このノートブックでは、TiDB Vectorの機能と実用的な応用例を詳しく説明しています。

## 環境設定

必要なパッケージをインストールから始めます。
...
OpenAIとTiDB Hostの設定を行います。このノートブックでは、TiDB Cloudが提供する標準的な接続方法に従って、安全で効率的なデータベース接続を確立します。
...
TiDBは、コサイン距離とユークリッド距離('cosine'、'l2')の両方をサポートしており、デフォルトはコサイン距離です。

以下のコードスニペットは、ベクトル検索に最適化された`TABLE_NAME`という名前のテーブルをTiDBに作成します。このコードを正常に実行すると、`TABLE_NAME`テーブルをTiDBデータベース内で直接表示およびアクセスできるようになります。
...
コサイン距離が小さいほど、類似度が高いことに注意してください。
...
さらに、similarity_search_with_relevance_scores メソッドを使用すると、関連性スコアを取得できます。スコアが高いほど、類似度が高いことを示します。
...
メタデータフィルターを使用して、適用したフィルターに合致する最近傍の結果を特定の数だけ取得できます。

## サポートされるメタデータの種類

TiDB Vector Storeの各ベクトルには、JSONオブジェクト内のキーと値のペアとして構造化されたメタデータを関連付けることができます。キーは文字列で、値は以下のタイプのいずれかになります:

- 文字列
- 数値(整数または浮動小数点)
- ブール値(true、false)

例えば、以下のようなメタデータペイロードが有効です:
...
利用可能なフィルターには以下のものがあります:

- $or - 指定された条件のいずれかを満たすベクトルを選択します。
- $and - 指定された条件をすべて満たすベクトルを選択します。
- $eq - 等しい
- $ne - 等しくない
- $gt - より大きい
- $gte - 以上
...
各メタデータフィルターのキーと値のペアは別個のフィルター条件として扱われ、これらの条件はAND論理演算子で組み合わされることに注意してください。
...
LangChainでは、Retrieverはストラクチャ化されていないクエリに応答してドキュメントを取得するインターフェースで、ベクトルストアよりも広い機能を提供します。以下のコードは、TiDB Vectorをリトリーバーとして使用する方法を示しています。
...
空港レビューの意味的検索を使って、きれいなラウンジと菜食オプションのある空港コードを抽出し、その後のSQL問合せでルート情報を結合して、クライアントの好みに合った航空会社と目的地を特定するという高度な使用例を見てみましょう。
...
あるいは、1つのSQL問合せですべての検索を行うことで、プロセスを簡略化できます。
...
`.drop_vectorstore()`メソッドを使用して、TiDB Vector Storeを削除できます。
