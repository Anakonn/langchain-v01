...
>[Kinetica](https://www.kinetica.com/)はベクトル類似検索をサポートしているデータベースです。

以下をサポートしています:
- 正確および近似最近傍検索
- L2距離、内積、コサイン距離

このノートブックでは、Kineticaベクトルストア(`Kinetica`)の使用方法を示します。

Kineticaのインスタンスが必要で、ここで説明されている手順を使って簡単にセットアップできます - [インストール手順](https://www.kinetica.com/developer-edition/)。
...
最大限の限界関連性は、クエリに対する類似性と選択されたドキュメントの多様性を最適化します。
...
上記では、ベクトルストアを最初から作成しました。しかし、多くの場合、既存のベクトルストアを使用したいことがあります。
...
既存のコレクションがある場合は、`from_documents`を使って上書きすることができ、`pre_delete_collection`を`True`に設定します。
