...
[JSONFormer](https://github.com/1rgs/jsonformer)は、Hugging FaceのパイプラインモデルをラップしてJSONスキーマのサブセットの構造化デコーディングを行うライブラリです。

モデルから構造トークンを埋め、コンテンツトークンをサンプリングすることで動作します。
...
まず、構造化デコーディングなしでモデルの出力を確認し、定性的なベースラインを確立しましょう。
...
***印象的ではありませんね。JSONフォーマットに従っていません! 構造化デコーダーを使ってみましょう。***

## JSONFormer LLMラッパー

今度は、Actionの入力のJSONスキーマをモデルに提供してみましょう。
