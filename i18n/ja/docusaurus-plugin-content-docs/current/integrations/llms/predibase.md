...
[Predibase](https://predibase.com/)は、線形回帰から大規模言語モデルまで、あらゆるMLモデルの学習、微調整、デプロイを可能にします。

この例では、Predibaseにデプロイされたモデルを使ってLangchainを使う方法を示します。

# セットアップ

このノートブックを実行するには、[Predibaseアカウント](https://predibase.com/free-trial/?utm_source=langchain)と[APIキー](https://docs.predibase.com/sdk-guide/intro)が必要です。

また、PredibaseのPythonパッケージをインストールする必要があります:
...
## 微調整済みLLM (Predibaseから自分の微調整済みLLMを使用してください)
