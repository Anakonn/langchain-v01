...
[Google Generative AI](https://developers.generativeai.google/) モデルを Langchain で使用するためのガイド。注: これは Google Cloud Vertex AI [integration](/docs/integrations/llms/google_vertex_ai_palm) とは別です。

## 設定

Google Generative AI を使用するには、`langchain-google-genai` Python パッケージをインストールし、API キーを生成する必要があります。[詳細はこちら](https://developers.generativeai.google/)。
...
Gemini モデルにはデフォルトのセーフティ設定があり、これをオーバーライドすることができます。モデルから多数の「セーフティ警告」を受け取っている場合は、モデルの `safety_settings` 属性を調整してみてください。例えば、危険なコンテンツに対するセーフティブロックをオフにするには、次のように LLM を構築できます:
...
利用可能なカテゴリーとしきい値の列挙については、Google の [safety setting types](https://ai.google.dev/api/python/google/generativeai/types/SafetySettingDict) を参照してください。
