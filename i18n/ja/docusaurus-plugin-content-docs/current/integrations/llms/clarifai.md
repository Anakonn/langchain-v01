...
このサンプルでは、LangChainを使って`Clarifai`[モデル](https://clarifai.com/explore/models)と対話する方法を説明します。

Clarifaiを使うには、アカウントと個人アクセストークン(PAT)キーが必要です。
...
ここでは個人アクセストークンを設定します。PATは、Clarifaiアカウントの[設定/セキュリティ](https://clarifai.com/settings/security)で確認できます。
...
ユーザーIDとモデルが存在するアプリIDを設定します。https://clarifai.com/exploreでパブリックモデルの一覧を確認できます。

また、モデルIDとモデルバージョンIDを初期化する必要があります。一部のモデルには複数のバージョンがあるため、タスクに適したバージョンを選択してください。

または、モデルのURL(例: "https://clarifai.com/anthropic/completion/models/claude-v2")を使って初期化することもできます。
...
## GPTのための推論パラメータを使ったモデル予測

また、温度やmax_tokensなどの推論パラメータを使ってGPTモデルを使うこともできます。
