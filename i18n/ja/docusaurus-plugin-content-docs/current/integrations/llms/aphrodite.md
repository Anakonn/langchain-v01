...
[Aphrodite](https://github.com/PygmalionAI/aphrodite-engine)は、[PygmalionAI](https://pygmalion.chat)ウェブサイトで数千人のユーザーにサービスを提供するためのオープンソースの大規模推論エンジンです。

* 高スループットと低レイテンシのためのvLLMによる注意機構
* 多くのSOTAサンプリング手法のサポート
* より低いバッチサイズでのスループットを向上させるExllamav2 GPTQ カーネル

このノートブックでは、langchainとAphroditeを使ってLLMを使う方法について説明します。

使用するには、`aphrodite-engine`Pythonパッケージをインストールする必要があります。
...
Aphroditeは分散テンソー並列推論とサービングをサポートしています。

LLMクラスで複数のGPUを使って推論を実行するには、`tensor_parallel_size`引数を使用するGPUの数に設定します。例えば、4つのGPUで推論を実行するには
