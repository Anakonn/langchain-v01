...
[ExLlamav2](https://github.com/turboderp/exllamav2)は、最新のコンシューマークラスのGPUでローカルにLLMを実行するための高速な推論ライブラリです。

GPTQ & EXL2量子化モデルの推論をサポートしており、これらのモデルは[Hugging Face](https://huggingface.co/TheBloke)で入手できます。

このノートブックでは、`exllamav2`をLangChainで実行する方法について説明します。
...
公式の[ドキュメント](https://github.com/turboderp/exllamav2)を参照してください。
このノートブックの要件は以下の通りです:
- python 3.11
- langchain 0.1.7
- CUDA: 12.1.0 (以下参照)
...
同じexllamav2バージョンをインストールする場合:

```shell
pip install https://github.com/turboderp/exllamav2/releases/download/v0.0.12/exllamav2-0.0.12+cu121-cp311-cp311-linux_x86_64.whl
```

condaを使う場合の依存関係は以下の通りです:
...
どのモデルがお使いのマシンに適しているかを理解することが重要です。

[TheBloke's](https://huggingface.co/TheBloke) Hugging Faceモデルには、量子化サイズと手法(例: [Mistral-7B-Instruct-v0.2-GPTQ](https://huggingface.co/TheBloke/Mistral-7B-Instruct-v0.2-GPTQ)))によるRAM要件が記載された`Provided files`セクションがあります。
