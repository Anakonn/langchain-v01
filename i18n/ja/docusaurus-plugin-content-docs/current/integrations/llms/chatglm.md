...
[ChatGLM-6B](https://github.com/THUDM/ChatGLM-6B)は、General Language Model (GLM)フレームワークに基づいた、6.2億パラメーターの双方向言語モデルです。量子化技術を使うことで、消費者向けのグラフィックスカード(INT4量子化レベルでは6GBのGPUメモリーのみ必要)でローカルにデプロイできます。

[ChatGLM2-6B](https://github.com/THUDM/ChatGLM2-6B)は、オープンソースの双方向(中国語-英語)チャットモデルであるChatGLM-6Bの第2世代バージョンです。第1世代モデルのスムーズな会話の流れと低いデプロイメントのしきい値を維持しつつ、より良いパフォーマンス、長いコンテキスト、より効率的な推論といった新機能を導入しています。

[ChatGLM3](https://github.com/THUDM/ChatGLM3)は、智浦AIとTsinghua KEGが共同でリリースした新世代の事前学習対話モデルです。ChatGLM3-6Bは、ChatGLM3シリーズのオープンソースモデルです。
...
このサンプルでは、LangChainを使ってChatGLM3-6Bの推論を使ってテキスト補完を行う方法を説明します。
...
次のサンプルでは、LangChainを使ってChatGLM2-6Bの推論を使ってテキスト補完を行う方法を示します。
ChatGLM-6BとChatGLM2-6BのAPIは同じなので、このサンプルは両方のモデルで動作します。
