...
このノートブックでは、duck-duck-go検索コンポーネントの使用方法について説明します。
...
より詳細な情報(例:リンク、ソース)を取得するには、`DuckDuckGoSearchResults()`を使用します。
...
ニュース記事を検索することもできます。キーワード ``backend="news"``を使用してください。
...
カスタムの ``DuckDuckGoSearchAPIWrapper``を直接 ``DuckDuckGoSearchResults``に渡すこともできます。これにより、検索結果をより詳細に制御できます。
