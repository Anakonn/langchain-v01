...
>[OpenAI Dall-E](https://openai.com/dall-e-3) は、自然言語の説明、いわゆる「プロンプト」からデジタル画像を生成するために、深層学習の手法を使って OpenAI が開発したテキストからイメージへの変換モデルです。

このノートブックでは、OpenAI の LLM を使って生成されたプロンプトからイメージを生成する方法を示します。イメージは `Dall-E` を使って生成されており、LLM と同じ OpenAI の API キーを使用しています。
