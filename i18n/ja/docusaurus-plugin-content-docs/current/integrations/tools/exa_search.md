...
Exa (旧 Metaphor Search) は、LLM の使用を完全に設計された検索エンジンです。**自然言語クエリ**を使用してインターネット上のドキュメントを検索し、目的のドキュメントから**クリーンな HTML コンテンツ**を取得します。

キーワードベースの検索 (Google) とは異なり、Exa の神経検索機能により、クエリを意味的に理解し、関連するドキュメントを返すことができます。例えば、`"fascinating article about cats"`と検索すると、[Google](https://www.google.com/search?q=fascinating+article+about+cats) と [Exa](https://search.exa.ai/search?q=fascinating%20article%20about%20cats&autopromptString=Here%20is%20a%20fascinating%20article%20about%20cats%3A) の検索結果を比較できます。Google は「fascinating」というキーワードに基づいた SEO 最適化されたリスト記事を提供しますが、Exa はそれだけで機能します。

このノートブックでは、LangChain を使用して Exa 検索を使用する方法について説明します。

まず、Exa API キーを取得し、環境変数として追加します。[ここで登録](https://dashboard.exa.ai/)すると、月間 1000 件の無料検索が利用できます。
...
## ExaSearchRetriever の使用

ExaSearchRetriever は、Exa 検索を使用して関連するドキュメントを取得するリトリーバーです。
...
## LangChain エージェントツールとしての Exa SDK の使用

[Exa SDK](https://docs.exa.ai/) は、Exa API を使用して 3 つの機能を実行できるクライアントを作成します:
- `search`: 自然言語検索クエリを指定すると、検索結果のリストを取得します。
- `find_similar`: URL を指定すると、その URL のドキュメントと類似したウェブページの検索結果のリストを取得します。
- `get_content`: `search` または `find_similar` から取得したドキュメント ID のリストを指定すると、各ドキュメントのクリーンな HTML コンテンツを取得します。

`@tool` デコレータとドキュメント文字列を使用して、LangChain Tool ラッパーを作成し、LLM エージェントに Exa の使用方法を伝えることができます。
...
### エージェントに Exa ツールを提供する

先ほど作成した Exa ツールを LangChain `OpenAIFunctionsAgent` に提供できます。「cats について魅力的な記事の要約をしてください」と尋ねられると、エージェントは `search` ツールを使用して適切な検索クエリで Exa 検索を実行し、`get_contents` ツールを使用して Exa コンテンツ取得を行い、取得したコンテンツの要約を返します。
...
Exa は、ドメインと日付によるパワフルなフィルタリングをサポートしています。目的に応じてフィルタを適用するかどうかを決定できる、より強力な `search` ツールをエージェントに提供できます。Exa の検索機能の全てを[ここ](https://github.com/metaphorsystems/metaphor-python/)で確認できます。
...
ドメインと公開日の制約付きで記事の要約を求めます。より強力な推論機能を持つ GPT-4 エージェントを使用します。

エージェントは正しく検索フィルタを使用して目的の制約に合う記事を見つけ、コンテンツを取得して要約を返します。
