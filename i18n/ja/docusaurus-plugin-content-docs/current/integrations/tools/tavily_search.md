...
[Tavily の検索 API](https://tavily.com) は、リアルタイム、正確、そして事実に基づいた結果を高速で提供するために特に AI エージェント (LLM) 向けに構築された検索エンジンです。

## セットアップ

このインテグレーションは `langchain-community` パッケージに含まれています。また、`tavily-python` パッケージ自体もインストールする必要があります。

```bash
pip install -U langchain-community tavily-python
```

また、Tavily の API キーを設定する必要があります。
...
[LangSmith](https://smith.langchain.com/) を設定することで、最高水準のオブザーバビリティを得ることができます (必須ではありません)。
...
ここでは、[エージェント](/docs/modules/agents)の一部として使用する方法を示します。OpenAI Functions Agent を使用するため、必要な依存関係をセットアップおよびインストールする必要があります。また、[LangSmith Hub](https://smith.langchain.com/hub) を使用してプロンプトを取得するため、それもインストールする必要があります。
