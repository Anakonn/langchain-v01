...
このノートブックでは、自己ホスティングされた `SearxNG` 検索 API を使用してウェブを検索する方法について説明します。

`Searx API` パラメーターの詳細については、[このリンク](https://docs.searxng.org/dev/search_api.html)を確認できます。
...
一部のエンジンでは、直接の `answer` が利用可能な場合、ラッパーは検索結果の完全なリストではなく、答えを出力します。 すべての結果を取得したい場合は、ラッパーの `results` メソッドを使用できます。
...
SearxNG は [135 の検索エンジン](https://docs.searxng.org/user/configured_engines.html)をサポートしています。 Searx ラッパーをカスタムの名前付きパラメーターで設定し、Searx 検索 API に渡すこともできます。 以下の例では、Searx 検索 API からのカスタム検索パラメーターをより興味深く使用します。

この例では、`engines` パラメーターを使用して Wikipedia を検索します。
...
`language` などの他の Searx パラメーターを渡す
...
この例では、`categories` パラメーターを使用して科学論文を検索し、`time_range` (一部のエンジンではタイムレンジオプションがサポートされていない) に制限します。

構造化された方法で結果とメタデータを取得したいので、ラッパーの `results` メソッドを使用します。
...
この例では、`it` カテゴリの下の `large language models` を検索します。 その後、GitHub からの結果をフィルタリングします。
...
GitHub やその他のソースフォージから直接結果を検索することもできます。
