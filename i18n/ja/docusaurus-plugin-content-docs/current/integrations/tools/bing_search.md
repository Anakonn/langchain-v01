...
>[Microsoft Bing](https://www.bing.com/)、一般的に `Bing` または `Bing 検索` と呼ばれる、`Microsoft` が所有・運営する Web 検索エンジンです。

このノートブックでは、Bing 検索コンポーネントの使用方法について説明します。

まず、適切な API キーと環境変数を設定する必要があります。設定方法は[こちら](https://levelup.gitconnected.com/api-tutorial-how-to-use-bing-web-search-api-in-python-4165d5592a7e)の手順に従ってください。
...
BingSearch を使用してクエリを実行し、スニペット、タイトル、リンクのメタデータを返します。
