...
>[Microsoft 365](https://www.office.com/)は、`Microsoft`が所有する生産性ソフトウェア、コラボレーション、クラウドベースのサービスのプロダクトファミリーです。
>
>注意: `Office 365`は`Microsoft 365`にリブランディングされました。

このノートブックでは、LangChainを`Office365`の電子メールとカレンダーに接続する方法を説明します。

このツールキットを使用するには、[Microsoft Graphの認証と承認の概要](https://learn.microsoft.com/en-us/graph/auth/)で説明されている資格情報を設定する必要があります。 CLIENT_IDとCLIENT_SECRETを受け取ったら、以下の環境変数として入力できます。

[ここからの認証手順](https://o365.github.io/python-o365/latest/getting_started.html#oauth-setup-pre-requisite)も使用できます。
...
このツールキットは、ユーザーを認証するために`CLIENT_ID`と`CLIENT_SECRET`の環境変数を読み取ります。後でエージェントを使用するには、`OPENAI_API_KEY`も設定する必要があります。
...
まず、後でツールにアクセスできるようにツールキットを作成する必要があります。
