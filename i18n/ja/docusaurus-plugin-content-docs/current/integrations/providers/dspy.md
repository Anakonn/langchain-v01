...
[DSPy](https://github.com/stanfordnlp/dspy)は、LLMsのための素晴らしいフレームワークで、プログラムの宣言的なステップを自動的にコンパイルしてLMsに教える機能を導入しています。具体的には、DSPyコンパイラはプログラムを内部的に追跡し、大規模LM(または小規模LMの自動ファインチューニング)に高品質なプロンプトを作成して、タスクのステップを教えます。

[Omar Khattab](https://twitter.com/lateinteraction)さんのおかげで、統合が実現しました。いくつかの小さな修正を加えれば、任意のLCELチェーンで動作します。

このチュートリアルでは、この概念実証機能がどのように動作するかを示します。*これではDSPyやLangChainの完全な力は得られませんが、需要が高ければ拡張していきます。*

注意: これは、Omar氏がDSPyのために書いた元の例を少し修正したものです。DSPy側からLangChainに興味がある場合は、そちらをチェックすることをお勧めします。[こちら](https://github.com/stanfordnlp/dspy/blob/main/examples/tweets/compiling_langchain.md)にあります。

例を見てみましょう。この例では、簡単なRAGパイプラインを作成します。DSPyを使ってプログラムをコンパイルし、最適化されたプロンプトを学習します。
...
OpenAIを使用するので、APIキーを設定する必要があります。
...
retrieverを設定することができます。retrieverには、DSPyを通じたColBERT retrieverを使用しますが、任意のretrieverで動作します。
...
まずは、LCELを使って簡単なRAGパイプラインを作成しましょう。

例として、以下のタスクに取り組みます。

**タスク:** 情報的なツイートを生成するRAGシステムを構築する。

- **入力:** 複雑な事実の質問。
- **出力:** 取得した情報から正しく質問に答えるエンゲージングなツイート。

LangChainの式言語(LCEL)を使って説明しましょう。ここで使用するプロンプトは何でも構いません。最終的なプロンプトはDSPyで最適化します。

そのため、ベアボーンに保ちます。**{context}を考慮して、{question}に対するツイートを作成してください。**
...
LangChainをDSPyで使うには、2つの小さな変更が必要です。

**LangChainPredict**

`prompt | llm`ではなく、`dspy`の`LangChainPredict(prompt, llm)`を使う必要があります。

これはラッパーで、プロンプトとLLMを一緒にバインドし、最適化できるようにします。

**LangChainModule**

これはラッパーで、最終的なLCELチェーンをラップし、DSPyが全体を最適化できるようにします。
...
これで、LangChainの実行可能なものとDSPyのモジュールの両方として使用できます。
...
ああ、それはほぼ正解ですね!(厳密には完璧ではありません。地域ではなく都市を尋ねました。以下でさらに良いものを作ることができます。)

質問と回答を手動で検査することは、システムの感触を掴むために非常に重要です。しかし、優れたシステム設計者は常に、進捗を定量化するためにベンチマークを反復的に行います。

これには2つのものが必要です。最大化したいメトリックと、システムのための(小さな)サンプルデータセットです。

ツイートの良さを表すメトリックは事前に定義されていますか? 10万件のツイートを手動でラベル付けすべきですか? おそらくそうではありません。しかし、本番環境でデータが得られるまでは、合理的なことを行うことができます。

## データのロード

チェーンをコンパイルするには、データセットが必要です。このデータセットには、入力と出力のみが含まれている必要があります。今回の目的では、HotPotQAデータセットを使用します。

注意: データセットには実際のツイートは含まれていないことに注意してください。質問と回答のみです。それでも問題ありません。メトリックがツイートの形式で出力を評価してくれます。
...
次に、メトリックを定義する必要があります。これは、成功したランを特定し、それらから学習するために使用されます。ここでは、DSPyのメトリックを使用しますが、独自のメトリックを書くこともできます。
...
では、LangChain LCELオブジェクトから変換された最適化されていない "ゼロショット "バージョンのチェーンを評価しましょう。
...
良いですね。zeroshot_chainは、devsetの150の質問に対して約42.00%の精度を得ています。

上の表には、いくつかの例が示されています。例えば:

- 質問: どのプロデューサーがJuke KartenとThirty Seconds to Marsの両方のロックバンドのアルバムをプロデュースしましたか?
- ツイート: ジェーンズ・アディクションやベルベット・リボルバーなどのバンドで仕事をしてきたブライアン・ヴァーチューは、Juke Kartelとサーティー・セカンズ・トゥ・マーズの両方のアルバムをプロデュースし、...（省略）
- メトリック: 1.0 (正しく、忠実で、魅力的なツイート!*)

脚注: * 少なくともメトリックによると。これもDSPyプログラムなので、最適化することができます。別のノートブックのトピックですが。
...
さて、これはどのくらい良いでしょうか? 適切な評価をしてみましょう!
...
よし! 42%から50%近くまで改善できました!

## 最適化されたチェーンの検査

では、実際にどのように改善されたのでしょうか? 最適化されたチェーンを見てみることで確認できます。2つの方法で行うことができます。

### 使用されたプロンプトを確認する

実際に使用されたプロンプトを確認することができます。`dspy.settings`を見ることで行えます。
...
最適化の方法は、サンプル(「デモ」)を収集してプロンプトに追加することでした。最適化されたチェーンを確認することで、それらがどのようなものかを把握できます。
