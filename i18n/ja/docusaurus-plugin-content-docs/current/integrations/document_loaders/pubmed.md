...
>[PubMed®](https://pubmed.ncbi.nlm.nih.gov/) by `The National Center for Biotechnology Information, National Library of Medicine` は、`MEDLINE`、生命科学ジャーナル、オンラインブックからの35百万件を超える文献引用で構成されています。引用には、`PubMed Central`およびパブリッシャーのウェブサイトからのフルテキストコンテンツへのリンクが含まれる場合があります。
