...
>[Google BigQuery](https://cloud.google.com/bigquery)は、クラウドをまたいで拡張可能な、サーバーレスでコストパフォーマンスの高いエンタープライズデータウェアハウスです。
`BigQuery`は`Google Cloud Platform`の一部です。

1行ごとにドキュメントを含む`BigQuery`クエリをロードします。
