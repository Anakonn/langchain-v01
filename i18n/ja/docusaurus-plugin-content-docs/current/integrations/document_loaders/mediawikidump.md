...
>[MediaWiki XMLダンプ](https://www.mediawiki.org/wiki/Manual:Importing_XML_dumps)には、ウィキのコンテンツ(すべての改訂版のウィキページ)が含まれていますが、サイト関連のデータは含まれていません。XMLダンプはウィキデータベースの完全なバックアップを作成するものではなく、ユーザーアカウント、画像、編集履歴などは含まれていません。

これは、MediaWikiのXMLダンプファイルをドキュメント形式にロードする方法について説明しています。

`mediawiki-utilities`の`mwxml`と`earwig`の`mwparserfromhell`を使用してダンプし、MediaWikiのウィキコードを解析します。

ダンプファイルは、dumpBackup.phpまたはウィキの特別:統計ページから取得できます。
