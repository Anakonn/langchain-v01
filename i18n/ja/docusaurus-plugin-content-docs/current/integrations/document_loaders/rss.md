...
これは、一連のRSSフィードURLからHTMLニュース記事をドキュメント形式にロードする方法について説明しています。
...
NewsURLLoaderに引数を渡すことができ、それを使ってArticleをロードします。
...
FeedlyエクスポートなどのOPMLファイルも使用できます。URLまたはOPMLコンテンツを渡します。
