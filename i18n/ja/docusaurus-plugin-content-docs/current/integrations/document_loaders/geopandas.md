...
[Geopandas](https://geopandas.org/en/stable/index.html)は、Pythonでの地理空間データの操作を容易にするためのオープンソースプロジェクトです。

GeoPandasは、pandasで使用されるデータ型を拡張し、幾何学的な型に対する空間演算を可能にします。

幾何学的な演算はshapelyによって実行されます。GeoPandasはさらに、ファイルアクセスにfiona、プロットにmatplotlibに依存しています。

地理空間データを活用するLLMアプリケーション(チャット、QA)は、探索の興味深い分野です。
...
[`Open City Data`](/docs/integrations/document_loaders/open_city_data)からGeoPandasデータフレームを作成する例。
...
GeoPandasデータフレームを`Document`としてロードし、後続の処理(埋め込み、チャットなど)に使用する。

`geometry`が`page_content`列のデフォルトとなり、その他の列は`metadata`に配置される。

ただし、`page_content_column`を指定することもできる。
