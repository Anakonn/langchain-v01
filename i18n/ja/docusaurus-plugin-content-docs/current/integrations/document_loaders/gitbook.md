...
>[GitBook](https://docs.gitbook.com/)は、製品から内部ナレッジベース、APIまで、チームが文書化できる現代的なドキュメンテーションプラットフォームです。

このノートブックでは、任意の `GitBook` からページデータを取得する方法を示します。

```python
from langchain_community.document_loaders import GitbookLoader
```

### 単一の GitBook ページからロードする
...
### 指定された GitBook の全パスからロードする

これを機能させるには、GitbookLoaderをルートパス(`https://docs.gitbook.com`この例では)で初期化し、`load_all_paths`を`True`に設定する必要があります。
