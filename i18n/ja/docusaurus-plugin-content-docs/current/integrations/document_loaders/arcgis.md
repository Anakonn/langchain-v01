...
このノートブックでは、`langchain_community.document_loaders.ArcGISLoader`クラスの使用方法を示します。

`arcgis`と、オプションで`bs4.BeautifulSoup`をインストールする必要があります。

認証されたデータ読み込みには`arcgis.gis.GIS`オブジェクトを使用できますが、公開データにアクセスする場合は空のままでかまいません。
...
フィーチャ地理情報を取得したい場合は、`return_geometry`キーワードを使用できます。

各ドキュメントの地理情報はメタデータ辞書に格納されます。
