...
>[Etherscan](https://docs.etherscan.io/)  は、分散型スマートコントラクトプラットフォームである Ethereum の最先端のブロックチェーンエクスプローラー、検索、API、分析プラットフォームです。

## 概要

`Etherscan` ローダーは `Ethereum Mainnet` 上の特定のアカウントのトランザクション履歴を読み込むために `Etherscan API` を使用します。

`Etherscan api key` が必要です。無料の api key は1秒あたり5回のクォータがあります。

ローダーは以下の6つの機能をサポートしています:

* Ethereum Mainnet 上の特定のアカウントの通常のトランザクションを取得
* Ethereum Mainnet 上の特定のアカウントの内部トランザクションを取得
* Ethereum Mainnet 上の特定のアカウントの erc20 トランザクションを取得
* Ethereum Mainnet 上の特定のアカウントの erc721 トランザクションを取得
* Ethereum Mainnet 上の特定のアカウントの erc1155 トランザクションを取得
* Ethereum Mainnet 上の特定のアカウントの wei 建てのイーサ残高を取得

アカウントに対応するトランザクションがない場合、ローダーは1つのドキュメントのリストを返します。ドキュメントの内容は ''です。

上記の機能にアクセスするには、ローダーに異なるフィルターを渡すことができます:
...
* "erc721_transaction"
* "erc1155_transaction"
フィルターはデフォルトで normal_transaction に設定されています。

質問がある場合は、[Etherscan API Doc](https://etherscan.io/tx/0x0ffa32c787b1398f44303f731cb06678e086e4f82ce07cebf75e99bb7c079c77) にアクセスするか、i@inevitable.techまでお問い合わせください。

トランザクション履歴に関するすべての機能は、Etherscanの制限により最大1000件に制限されています。必要なトランザクション履歴を見つけるには、以下のパラメーターを使用できます:

* offset: デフォルトは20。1回に20件のトランザクションを表示します。
* page: デフォルトは1。これはページネーションを制御します。
* start_block: デフォルトは0。トランザクション履歴は0ブロックから始まります。
* end_block: デフォルトは99999999。トランザクション履歴は99999999ブロックまで続きます。
* sort: "desc" または "asc"。デフォルトは "desc" で、最新のトランザクションを取得します。
...
## カスタマイズされたパラメーターを使用した通常のトランザクションローダーの作成
