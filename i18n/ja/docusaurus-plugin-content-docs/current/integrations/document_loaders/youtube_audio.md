...
YouTubeビデオのチャットやQAアプリケーションを構築することは、非常に関心の高いトピックです。

以下では、`YouTubeのURL`から`ビデオのオーディオ`、`テキスト`、`チャット`まで、簡単に進める方法を示します!

`OpenAIWhisperParser`を使用し、OpenAI WhisperのAPIを使ってオーディオをテキストに変換します。
また、ローカルサポートと、プライベートクラウドやオンプレミスでの実行のために`OpenAIWhisperParserLocal`も使用します。

注意: `OPENAI_API_KEY`を用意する必要があります。
...
YouTubeのURLからオーディオをダウンロードするために、`yt_dlp`を使用します。

`pydub`を使用して、ダウンロードしたオーディオファイルを分割します(Whisper APIの25MBファイルサイズ制限に従うため)。
...
`YoutubeAudioLoader`を使用して、オーディオファイルを取得/ダウンロードします。

次に、`OpenAIWhisperParser()`を使ってテキストに変換します。

Andrej Karpathyのユーチューブコースの最初の講義を例として取り上げましょう!
...
### YouTubeビデオからチャットアプリを構築する

`Documents`があれば、簡単にチャット/質問回答を実現できます。
