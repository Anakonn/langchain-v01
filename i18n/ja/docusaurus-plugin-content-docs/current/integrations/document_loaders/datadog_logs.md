...
>[Datadog](https://www.datadoghq.com/)は、クラウドスケールアプリケーションの監視およびアナリティクスプラットフォームです。

このローダーは、`datadog_api_client`Pythonパッケージを使用して、Datadogからアプリケーションのログを取得します。ローダーを初期化するには、`Datadog API key`と`APP key`を指定する必要があり、目的のログを抽出するクエリを渡す必要があります。
