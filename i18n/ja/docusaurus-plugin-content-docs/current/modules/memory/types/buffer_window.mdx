...
`ConversationBufferWindowMemory` は、時間の経過とともに会話の相互作用のリストを保持します。最後の K 個の相互作用のみを使用します。これは、バッファーが大きくなりすぎないように、最近の相互作用のスライディングウィンドウを保持するのに役立つ可能性があります。

まずこのタイプのメモリの基本的な機能を探ってみましょう。
...
また、メッセージのリストとしてヒストリーを取得することもできます(これはチャットモデルと一緒に使用する場合に便利です)。
...
例を見ていきましょう。再び `verbose=True` を設定して、プロンプトを確認できるようにします。
