...
>[Portable Document Format (PDF)](https://en.wikipedia.org/wiki/PDF)は、アプリケーションソフトウェア、ハードウェア、オペレーティングシステムに依存せずにドキュメントを表示できるようにするために、Adobeが1992年に開発したファイル形式です。これは、ISO 32000として標準化されています。

これは、ドキュメントフォーマットにPDFドキュメントを読み込む方法について説明しています。

## PyPDFを使う

`pypdf`を使ってPDFを読み込み、ページ番号を持つドキュメントの配列に格納します。
...
このアプローチの利点は、ページ番号でドキュメントを取得できることです。

`OpenAIEmbeddings`を使いたいので、OpenAI APIキーを取得する必要があります。
...
`rapidocr-onnxruntime`パッケージを使うと、画像をテキストとしても抽出できます。
...
これはPDF解析オプションの中で最も高速で、PDFとそのページに関する詳細なメタデータが含まれ、ページごとに1つのドキュメントが返されます。
...
さらに、[PyMuPDF documentation](https://pymupdf.readthedocs.io/en/latest/app1.html#plain-text/)のオプションをキーワード引数として`load`呼び出しに渡すことで、`get_text()`呼び出しに渡すことができます。

## MathPixを使う

Daniel Grossの[https://gist.github.com/danielgross/3ab4104e14faccc12b49200843adab21](https://gist.github.com/danielgross/3ab4104e14faccc12b49200843adab21)に触発されています。
...
`unstructured[all-docs]`パッケージは現在、テキストファイル、PowerPoint、HTML、PDF、画像などの読み込みをサポートしています。
...
Unstructuredは、テキストの異なる部分に対して異なる "要素"を作成します。デフォルトではそれらを結合しますが、`mode="elements"`を指定することで、その分離を簡単に維持できます。
...
### Unstructuredを使ってリモートPDFをフェッチする

これは、ダウンストリームで使用できるドキュメント形式にオンラインPDFを読み込む方法について説明しています。これは、https://open.umn.edu/opentextbooks/textbooks/やhttps://arxiv.org/archive/などのさまざまなオンラインPDFサイトで使用できます。

注意: 他のすべてのPDFローダーもリモートPDFをフェッチするために使用できますが、`OnlinePDFLoader`は旧式の関数で、`UnstructuredPDFLoader`と特に連携するように設計されています。
...
PDFMinerは、PDFドキュメントから情報を抽出し、データを分析するのに役立つツールです。
...
### PDFMinerでHTMLテキストを生成する

これは、出力HTMLコンテンツをBeautifulSoupで解析して、フォントサイズ、ページ番号、PDFのヘッダー/フッターなどの構造化された豊富な情報を取得できるため、テキストを意味的にセクションに分割するのに役立ちます。
...
PyMuPDFと同様に、出力ドキュメントにはPDFとそのページに関する詳細なメタデータが含まれ、ページごとに1つのドキュメントが返されます。
...
## AmazonTextractPDFParserを使う

AmazonTextractPDFLoaderは、[Amazon Textract Service](https://aws.amazon.com/textract/)を呼び出してPDFをドキュメント構造に変換します。現時点ではOCRのみをサポートしていますが、需要に応じて、レイアウトサポートなどの機能が追加される予定です。最大3000ページ、512 MBのサイズの単一ページおよび複数ページのドキュメントがサポートされています。

呼び出しを成功させるには、[AWS CLI](https://docs.aws.amazon.com/cli/latest/userguide/cli-chap-configure.html)の要件と同様に、AWS アカウントが必要です。

AWS設定以外は、他のPDFローダーとよく似ていますが、JPEG、PNG、TIFFおよびネイティブ以外のPDFフォーマットもサポートしています。
...
## AzureAIDocumentIntelligenceLoaderの使用

[Azure AI Document Intelligence](https://aka.ms/doc-intelligence)（旧称「Azure Form Recognizer」）は、機械学習ベースのサービスで、デジタルまたはスキャンしたPDF、画像、Office、HTMLファイルから、テキスト（手書きを含む）、表、文書構造（タイトル、セクション見出しなど）、キーバリューペアを抽出します。Document Intelligenceは、`PDF`、`JPEG/JPG`、`PNG`、`BMP`、`TIFF`、`HEIF`、`DOCX`、`XLSX`、`PPTX`、`HTML`をサポートしています。

この[現在の実装](https://aka.ms/di-langchain)では、`Document Intelligence`を使用したローダーを使用して、コンテンツをページごとに取り込み、LangChainドキュメントに変換できます。デフォルトの出力形式はMarkdownで、`MarkdownHeaderTextSplitter`を使用してセマンティックドキュメントチャンキングを簡単に行えます。`mode="single"`または`mode="page"`を使用して、単一ページまたはページ単位で純テキストを返すこともできます。

### 前提条件

Azure AI Document Intelligenceリソースは、**東部アメリカ**、**西部アメリカ2**、**西ヨーロッパ**の3つのプレビュー リージョンのいずれかに必要です。[このドキュメント](https://learn.microsoft.com/azure/ai-services/document-intelligence/create-document-intelligence-resource?view=doc-intel-4.0.0)に従って作成していない場合は、作成してください。`<endpoint>`と`<key>`をパラメーターとしてローダーに渡します。
...
## UpstageLayoutAnalysisLoaderの使用

UpstageLayoutAnalysisLoaderは、[Upstage Layout Analysis API](https://developers.upstage.ai/docs/apis/layout-analyzer)を呼び出して、さまざまな文書形式から文書要素（表や図など）を検出します。このローダーは、OCRを使用してテキスト情報を抽出し、`JPEG`、`PNG`、`BMP`、`PDF`、`TIFF`、`HEIC`ファイルの要素を検出します。デジタルで作成されたPDFドキュメントの場合、use_ocr=Falseを設定することで、ファイル内のテキスト情報を利用できます。これがデフォルト値です。単一ページおよび複数ページのドキュメントがサポートされ、use_ocr=Trueの場合は100ページ、50MBが上限ですが、use_ocr=Falseの場合（PDFファイルのみ）には制限はありません。

### 前提条件

Upstage Layout Analysis APIにアクセスするには、APIアクセストークンが必要です。[クイックスタートガイド](https://developers.upstage.ai/docs/getting-started/quick-start)を参照して、アクセストークンを取得し、Upstage Layout Analysis APIの使用を開始してください。
