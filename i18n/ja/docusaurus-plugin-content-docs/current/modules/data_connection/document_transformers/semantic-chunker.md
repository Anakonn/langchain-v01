...
テキストをセマンティックな類似性に基づいて分割します。

Greg Kamradt氏の素晴らしいノートブックから引用:
...
高レベルでは、これはまず文に分割し、その後3文ずつのグループに分け、最後に埋め込み空間での類似性に基づいて統合します。
...
このチャンカーは、文を "分割" するタイミングを決定することで機能します。これは、任意の2つの文間の埋め込みの違いを見ることで行われます。その違いが特定のしきい値を超えた場合、それらは分割されます。

そのしきい値を決定する方法がいくつかあります。

### パーセンタイル

デフォルトの分割方法はパーセンタイルに基づいています。この方法では、すべての文間の違いが計算され、X パーセンタイルを超える違いがある場合に分割されます。
...
この方法では、X 標準偏差を超える違いがある場合に分割されます。
