...
Pythonプログラミング言語で一般的に使用されるデータ構造であるPandas DataFrameは、データ操作と分析のために広く使用されています。構造化データを扱うための包括的なツールを提供し、データクリーニング、変換、分析などの様々なタスクに適した柔軟なオプションとなっています。

このoutputパーサーを使用すると、ユーザーは任意のPandas DataFrameを指定し、対応するDataFrameからデータを抽出する形式化された辞書を返すLLMに対してクエリを実行できます。ただし、大規模な言語モデルは漏れ抜けた抽象化であることに注意してください。定義された形式指示に従って適切なクエリを生成するには、十分な容量のLLMを使用する必要があります。

Pandas' DataFrame オブジェクトを使用して、クエリを実行するDataFrameを宣言してください。
...
[PandasDataFrameOutputParser](https://api.python.langchain.com/en/latest/output_parsers/langchain.output_parsers.pandas_dataframe.PandasDataFrameOutputParser.html#langchain.output_parsers.pandas_dataframe.PandasDataFrameOutputParser)のAPIドキュメントを確認してください。
