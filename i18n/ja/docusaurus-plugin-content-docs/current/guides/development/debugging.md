...
LLMを使ってビルドしていると、何かが壊れて、デバッグする必要が出てくるでしょう。モデルの呼び出しが失敗したり、モデル出力がフォーマットされていなかったり、ネストされたモデルの呼び出しがあって、どこかでおかしな出力が生成されたのかわからなくなったりするかもしれません。

デバッグに役立つ様々なツールや機能があります。

## トレーシング

[LangSmith](/docs/langsmith/)のようなトレーシング機能を持つプラットフォームが、デバッグにとって最も包括的なソリューションです。これらのプラットフォームでは、LLMアプリケーションのログ記録やビジュアル化だけでなく、アクティブなデバッグ、テスト、改善も簡単に行えます。

本番環境のLLMアプリケーションを構築する際は、このようなプラットフォームが不可欠です。

![LangSmithのデバッグインターフェイスのスクリーンショット。入力と出力の詳細、およびrunツリーの視覚化が表示されています。](../../../../../../static/img/run_details.png "LangSmithのデバッグインターフェイス")

## `set_debug`と`set_verbose`

Jupyter NotebooksやPythonスクリプトでプロトタイピングしている場合は、Chainの実行過程を出力することが役立つかもしれません。

さまざまな方法で、さまざまな程度の詳細さで出力を有効にできます。

簡単なエージェントがあり、その行動と受け取るツールの出力を可視化したいとしましょう。デバッグなしでは、以下のような出力が得られます:
...
グローバルな`debug`フラグを有効にすると、コールバックサポートのあるすべてのLangChainコンポーネント(Chains、Models、Agents、Tools、Retrievers)が、受け取る入力と生成する出力を出力します。これが最も詳細な設定で、生の入出力がすべて記録されます。
...
<details> <summary>コンソール出力</summary>
...
`verbose`フラグを有効にすると、入出力がより読みやすい形式で出力され、LLMの呼び出しに関するトークン使用統計などの一部の生の出力は省略されるため、アプリケーションロジックに集中できます。
...
特定のオブジェクトにのみ詳細出力を適用することもできます。この場合、そのオブジェクト自体の入出力と、そのオブジェクトによって行われた追加のコールバック呼び出しのみが出力されます。
...
`Callbacks`は、コンポーネントのプライマリロジック以外の機能を実行するために使用するものです。上記のすべての解決策は、コンポーネントの中間ステップをログ記録するために、`Callbacks`を内部で使用しています。LangChainには、[FileCallbackHandler](/docs/modules/callbacks/filecallbackhandler)のようなデバッグに関連する多くの`Callbacks`が用意されています。独自のコールバックを実装して、カスタムの機能を実行することもできます。

[Callbacks](/docs/modules/callbacks/)、その使用方法、カスタマイズ方法の詳細については、こちらをご覧ください。
