...
Runnablesを使えば、簡単に複数のチェーンを連結することができます。
...
1つのコンポーネントの出力を2つ以上の他のコンポーネントで処理したい場合があります。[RunnableParallels](https://api.python.langchain.com/en/latest/runnables/langchain_core.runnables.base.RunnableParallel.html#langchain_core.runnables.base.RunnableParallel)を使えば、チェーンを分割または分岐させて、複数のコンポーネントが入力を並行して処理できるようになります。その後、他のコンポーネントが結果をマージして最終的な応答を合成することができます。このようなチェーンでは、以下のようなコンピュテーショングラフが作成されます。
