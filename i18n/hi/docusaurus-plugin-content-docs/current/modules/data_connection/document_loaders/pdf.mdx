...
>[पोर्टेबल डॉक्यूमेंट फॉर्मेट (PDF)](https://en.wikipedia.org/wiki/PDF), जिसे ISO 32000 के रूप में मानकीकृत किया गया है, एक फ़ाइल प्रारूप है जिसे Adobe ने 1992 में विकसित किया था ताकि एप्लिकेशन सॉफ़्टवेयर, हार्डवेयर और ऑपरेटिंग सिस्टम से स्वतंत्र रूप से दस्तावेजों, जिसमें पाठ स्वरूपण और छवियां शामिल हैं, को प्रस्तुत किया जा सके।

यह बताता है कि डॉक्यूमेंट प्रारूप में जो हम आगे उपयोग करते हैं उसमें PDF दस्तावेजों को कैसे लोड किया जाए।

## PyPDF का उपयोग करना

`pypdf` का उपयोग करके PDF को दस्तावेजों के एक सरणी में लोड करें, जहां प्रत्येक दस्तावेज में पृष्ठ सामग्री और मेटाडेटा `पृष्ठ` संख्या के साथ होती है।
...
```python
<!--IMPORTS:[{"imported": "PyPDFLoader", "source": "langchain_community.document_loaders", "docs": "https://api.python.langchain.com/en/latest/document_loaders/langchain_community.document_loaders.pdf.PyPDFLoader.html", "title": "PDF"}]-->
...
इस アプローチ का एक लाभ यह है कि दस्तावेज पृष्ठ संख्याओं के साथ पुनः प्राप्त किए जा सकते हैं।
...
```python
<!--IMPORTS:[{"imported": "FAISS", "source": "langchain_community.vectorstores", "docs": "https://api.python.langchain.com/en/latest/vectorstores/langchain_community.vectorstores.faiss.FAISS.html", "title": "PDF"}, {"imported": "OpenAIEmbeddings", "source": "langchain_openai", "docs": "https://api.python.langchain.com/en/latest/embeddings/langchain_openai.embeddings.base.OpenAIEmbeddings.html", "title": "PDF"}]-->
...
`rapidocr-onnxruntime` पैकेज का उपयोग करके हम छवियों को पाठ के रूप में भी निकाल सकते हैं:
...
## PyMuPDF का उपयोग करना

यह PDF पार्सिंग विकल्पों में सबसे तेज है, और PDF और इसके पृष्ठों के बारे में विस्तृत मेटाडेटा भी शामिल है, साथ ही प्रति पृष्ठ एक दस्तावेज भी वापस देता है।

```python
<!--IMPORTS:[{"imported": "PyMuPDFLoader", "source": "langchain_community.document_loaders", "docs": "https://api.python.langchain.com/en/latest/document_loaders/langchain_community.document_loaders.pdf.PyMuPDFLoader.html", "title": "PDF"}]-->
...
इसके अलावा, आप [PyMuPDF दस्तावेज](https://pymupdf.readthedocs.io/en/latest/app1.html#plain-text/) से किसी भी विकल्प को `लोड` कॉल में कीवर्ड तर्क के रूप में पास कर सकते हैं, और यह `get_text()` कॉल में पास किया जाएगा।

## MathPix का उपयोग करना

डैनियल ग्रॉस के [https://gist.github.com/danielgross/3ab4104e14faccc12b49200843adab21](https://gist.github.com/danielgross/3ab4104e14faccc12b49200843adab21) से प्रेरित

```python
<!--IMPORTS:[{"imported": "MathpixPDFLoader", "source": "langchain_community.document_loaders", "docs": "https://api.python.langchain.com/en/latest/document_loaders/langchain_community.document_loaders.pdf.MathpixPDFLoader.html", "title": "PDF"}]-->
...
`unstructured[all-docs]` पैकेज वर्तमान में पाठ फ़ाइलों, पावरपॉइंट, HTML, PDF, छवियों और अधिक को लोड करने का समर्थन करता है।
...
```python
<!--IMPORTS:[{"imported": "UnstructuredPDFLoader", "source": "langchain_community.document_loaders", "docs": "https://api.python.langchain.com/en/latest/document_loaders/langchain_community.document_loaders.pdf.UnstructuredPDFLoader.html", "title": "PDF"}]-->
...
### Unstructured का उपयोग करके दूरस्थ PDF फ़ेच करना

यह कवर करता है कि हम डाउनस्ट्रीम में उपयोग कर सकते हैं उस दस्तावेज़ प्रारूप में ऑनलाइन PDF को कैसे लोड किया जाए। इसका उपयोग https://open.umn.edu/opentextbooks/textbooks/ और https://arxiv.org/archive/ जैसी विभिन्न ऑनलाइन PDF साइटों के लिए किया जा सकता है।

नोट: अन्य सभी PDF लोडर भी दूरस्थ PDF को फ़ेच करने के लिए उपयोग किए जा सकते हैं, लेकिन `OnlinePDFLoader` एक पुरानी कार्यक्षमता है, और `UnstructuredPDFLoader` के साथ विशेष रूप से काम करता है।

```python
<!--IMPORTS:[{"imported": "OnlinePDFLoader", "source": "langchain_community.document_loaders", "docs": "https://api.python.langchain.com/en/latest/document_loaders/langchain_community.document_loaders.pdf.OnlinePDFLoader.html", "title": "PDF"}]-->
...
## PyPDFium2 का उपयोग करना

```python
<!--IMPORTS:[{"imported": "PyPDFium2Loader", "source": "langchain_community.document_loaders", "docs": "https://api.python.langchain.com/en/latest/document_loaders/langchain_community.document_loaders.pdf.PyPDFium2Loader.html", "title": "PDF"}]-->
...
## PDFMiner का उपयोग करना

PDFMiner एक ऐसा उपकरण है जो PDF दस्तावेजों से जानकारी निकालने और डेटा का विश्लेषण करने में मदद कर सकता है।
...
```python
<!--IMPORTS:[{"imported": "PDFMinerLoader", "source": "langchain_community.document_loaders", "docs": "https://api.python.langchain.com/en/latest/document_loaders/langchain_community.document_loaders.pdf.PDFMinerLoader.html", "title": "PDF"}]-->
...
### HTML पाठ उत्पन्न करने के लिए PDFMiner का उपयोग करना

यह पाठ को अनुभागों में अर्थपूर्ण रूप से विभाजित करने के लिए उपयोगी हो सकता है क्योंकि आउटपुट HTML सामग्री को `BeautifulSoup` के माध्यम से पार्स किया जा सकता है ताकि फ़ॉन्ट आकार, पृष्ठ संख्या, PDF हेडर/फ़ुटर आदि के बारे में अधिक संरचित और समृद्ध जानकारी प्राप्त की जा सके।

```python
<!--IMPORTS:[{"imported": "PDFMinerPDFasHTMLLoader", "source": "langchain_community.document_loaders", "docs": "https://api.python.langchain.com/en/latest/document_loaders/langchain_community.document_loaders.pdf.PDFMinerPDFasHTMLLoader.html", "title": "PDF"}]-->
...
```python
<!--IMPORTS:[{"imported": "Document", "source": "langchain_community.docstore.document", "docs": "https://api.python.langchain.com/en/latest/documents/langchain_core.documents.base.Document.html", "title": "PDF"}]-->
...
निर्देशिका से PDF लोड करें

```python
<!--IMPORTS:[{"imported": "PyPDFDirectoryLoader", "source": "langchain_community.document_loaders", "docs": "https://api.python.langchain.com/en/latest/document_loaders/langchain_community.document_loaders.pdf.PyPDFDirectoryLoader.html", "title": "PDF"}]-->
...
## PDFPlumber का उपयोग करना

PyMuPDF की तरह, आउटपुट दस्तावेज PDF और इसके पृष्ठों के बारे में विस्तृत मेटाडेटा रखते हैं, और प्रति पृष्ठ एक दस्तावेज वापस देते हैं।

```python
<!--IMPORTS:[{"imported": "PDFPlumberLoader", "source": "langchain_community.document_loaders", "docs": "https://api.python.langchain.com/en/latest/document_loaders/langchain_community.document_loaders.pdf.PDFPlumberLoader.html", "title": "PDF"}]-->
...
## AmazonTextractPDFParser का उपयोग करना

AmazonTextractPDFLoader [Amazon Textract Service](https://aws.amazon.com/textract/) को कॉल करता है ताकि PDF को एक दस्तावेज़ संरचना में रूपांतरित किया जा सके। लोडर वर्तमान में शुद्ध OCR कर रहा है, और लेआउट समर्थन सहित और सुविधाएं भविष्य में मांग के आधार पर योजना बनाई जा सकती हैं। एकल और बहु-पृष्ठ दस्तावेज 3000 पृष्ठों और 512 MB आकार तक समर्थित हैं।

कॉल सफल होने के लिए एक AWS खाता आवश्यक है, [AWS CLI](https://docs.aws.amazon.com/cli/latest/userguide/cli-chap-configure.html) आवश्यकताओं के समान।

AWS कॉन्फ़िगरेशन के अलावा, यह अन्य PDF लोडर्स के बहुत समान है, जबकि JPEG, PNG और TIFF और गैर-मूल PDF प्रारूपों का भी समर्थन करता है।

```python
<!--IMPORTS:[{"imported": "AmazonTextractPDFLoader", "source": "langchain_community.document_loaders", "docs": "https://api.python.langchain.com/en/latest/document_loaders/langchain_community.document_loaders.pdf.AmazonTextractPDFLoader.html", "title": "PDF"}]-->
from langchain_community.document_loaders import AmazonTextractPDFLoader
loader = AmazonTextractPDFLoader("example_data/alejandro_rosalez_sample-small.jpeg")
documents = loader.load()
```

## AzureAIDocumentIntelligenceLoader का उपयोग करना

[Azure AI Document Intelligence](https://aka.ms/doc-intelligence) (पहले `Azure Form Recognizer` के रूप में जाना जाता था) एक मशीन-लर्निंग आधारित सेवा है जो डिजिटल या स्कैन किए गए PDF, छवि, Office और HTML फ़ाइलों से पाठ (हस्तलिखित सहित), तालिकाएं, दस्तावेज़ संरचनाएं (जैसे शीर्षक, अनुभाग शीर्षक आदि) और कुंजी-मूल्य-युग्म को निकालती है। Document Intelligence `PDF`, `JPEG/JPG`, `PNG`, `BMP`, `TIFF`, `HEIF`, `DOCX`, `XLSX`, `PPTX` और `HTML` का समर्थन करता है।

इस [वर्तमान कार्यान्वयन](https://aka.ms/di-langchain) में `Document Intelligence` का उपयोग करके लोडर को सामग्री को पृष्ठ-वार शामिल करके LangChain दस्तावेजों में बदल दिया जाता है। डिफ़ॉल्ट आउटपुट प्रारूप मार्कडाउन है, जिसे `MarkdownHeaderTextSplitter` के साथ आसानी से सेमांटिक दस्तावेज़ चंकिंग के लिए श्रृंखलित किया जा सकता है। आप `mode="single"` या `mode="page"` का भी उपयोग कर सकते हैं ताकि एक पृष्ठ या दस्तावेज़ में विभाजित शुद्ध पाठ वापस मिल सके।

### पूर्वापेक्षा

**पूर्व**, **पश्चिम यूएस2**, **पश्चिम यूरोप** में से किसी एक प्रीव्यू क्षेत्र में एक Azure AI Document Intelligence संसाधन - यदि आपके पास नहीं है तो [इस दस्तावेज़](https://learn.microsoft.com/azure/ai-services/document-intelligence/create-document-intelligence-resource?view=doc-intel-4.0.0) का पालन करके एक बनाएं। आप लोडर को `<endpoint>` और `<key>` पैरामीटर के रूप में पारित करेंगे।

```python
<!--IMPORTS:[{"imported": "AzureAIDocumentIntelligenceLoader", "source": "langchain_community.document_loaders", "docs": "https://api.python.langchain.com/en/latest/document_loaders/langchain_community.document_loaders.doc_intelligence.AzureAIDocumentIntelligenceLoader.html", "title": "PDF"}]-->
...
## UpstageLayoutAnalysisLoader का उपयोग करना

UpstageLayoutAnalysisLoader [Upstage Layout Analysis API](https://developers.upstage.ai/docs/apis/layout-analyzer) को कॉल करता है जो विभिन्न दस्तावेज़ प्रारूपों से दस्तावेज़ तत्वों, जिनमें तालिकाएं और आकृतियां शामिल हैं, का पता लगाता है। यह लोडर पाठीय जानकारी को निकालने और `JPEG`, `PNG`, `BMP`, `PDF`, `TIFF` और `HEIC` फ़ाइलों में तत्वों का पता लगाने के लिए शुद्ध OCR का उपयोग करता है। डिजिटल जन्म PDF दस्तावेजों के मामले में, उपयोगकर्ताओं के पास use_ocr=False सेट करके फ़ाइल के भीतर के पाठ जानकारी का उपयोग करने का विकल्प है, जो डिफ़ॉल्ट मान है। एकल और बहु-पृष्ठ दस्तावेज़ दोनों का समर्थन किया जाता है, जहां use_ocr=True के लिए 100 पृष्ठों और 50 MB के फ़ाइल आकार की सीमा है, जबकि use_ocr=False (केवल PDF फ़ाइलों पर लागू) के लिए कोई प्रतिबंध नहीं हैं।

### पूर्वापेक्षा

Upstage Layout Analysis API का उपयोग करने के लिए, आपको एक API एक्सेस टोकन की आवश्यकता है। कृपया [त्वरित शुरुआत गाइड](https://developers.upstage.ai/docs/getting-started/quick-start) का संदर्भ लें और एक्सेस टोकन प्राप्त करें।
...
```python
<!--IMPORTS:[{"imported": "UpstageLayoutAnalysisLoader", "source": "langchain_upstage", "docs": "https://api.python.langchain.com/en/latest/layout_analysis/langchain_upstage.layout_analysis.UpstageLayoutAnalysisLoader.html", "title": "PDF"}]-->
import os
