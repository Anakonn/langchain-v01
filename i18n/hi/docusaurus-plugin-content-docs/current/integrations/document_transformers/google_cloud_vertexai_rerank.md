...
# Google Cloud Vertex AI रीरैंकर

> [Vertex Search Ranking API](https://cloud.google.com/generative-ai-app-builder/docs/ranking) [Vertex AI Agent Builder](https://cloud.google.com/generative-ai-app-builder/docs/builder-apis) के स्टैंडअलोन API में से एक है। यह दस्तावेजों की एक सूची लेता है और उन दस्तावेजों को उनकी प्रासंगिकता के आधार पर रीरैंक करता है। एम्बेडिंग की तुलना में, जो केवल दस्तावेज और क्वेरी की语义समानता पर देखते हैं, रैंकिंग API आपको यह बताने के लिए सटीक स्कोर दे सकता है कि एक दस्तावेज किस हद तक एक दिए गए क्वेरी का जवाब देता है। रैंकिंग API का उपयोग प्रारंभिक उम्मीदवार दस्तावेजों को पुनः प्राप्त करने के बाद खोज परिणामों की गुणवत्ता में सुधार करने के लिए किया जा सकता है।

>रैंकिंग API स्थिरावस्था है, इसलिए API को कॉल करने से पहले दस्तावेजों को इंडेक्स करने की आवश्यकता नहीं है। आपको केवल क्वेरी और दस्तावेज पास करने की आवश्यकता है। यह API किसी भी दस्तावेज प्राप्तकर्ता से प्राप्त दस्तावेजों को रीरैंक करने के लिए उपयुक्त है।

>अधिक जानकारी के लिए, [दस्तावेजों को रैंक और रीरैंक करें](https://cloud.google.com/generative-ai-app-builder/docs/ranking) देखें।
...
इस उदाहरण में, हम [Google Wiki पृष्ठ](https://en.wikipedia.org/wiki/Google) का उपयोग करेंगे ताकि हम Vertex Ranking API कैसे काम करता है, दिखा सकें।

हम `लोड -> विभाजित -> डेटा एम्बेड` का एक मानक पाइपलाइन का उपयोग करते हैं।

एम्बेडिंग [Vertex Embeddings API](https://cloud.google.com/vertex-ai/generative-ai/docs/embeddings/get-text-embeddings#supported_models) मॉडल - `textembedding-gecko@003` का उपयोग करके बनाई जाती हैं।
...
### Vertex Ranking API का परीक्षण करना

आइए `basic_retriever` और `retriever_with_reranker` दोनों को एक ही क्वेरी के साथ क्वेरी करें और प्राप्त दस्तावेजों की तुलना करें।

रैंकिंग API `basic_retriever` से इनपुट लेता है और उसे रैंकिंग API में पास करता है।

रैंकिंग API का उपयोग रैंकिंग की गुणवत्ता में सुधार करने और प्रत्येक रिकॉर्ड की क्वेरी से प्रासंगिकता को दर्शाने वाले स्कोर निर्धारित करने के लिए किया जाता है।

आप अरैंक्ड और रैंक्ड दस्तावेजों के बीच अंतर देख सकते हैं। रैंकिंग API सबसे प्रासंगिक दस्तावेजों को एलएलएम के संदर्भ विंडो के शीर्ष पर ले जाता है, जिससे यह बेहतर उत्तर तर्क के साथ बना सकता है।
...
आइए कुछ रीरैंक किए गए दस्तावेजों का निरीक्षण करें। हम देखते हैं कि retriever अभी भी प्रासंगिक Langchain प्रकार [दस्तावेज](https://api.python.langchain.com/en/latest/documents/langchain_core.documents.base.Document.html) वापस देता है, लेकिन मेटाडेटा फ़ील्ड के रूप में, हमें रैंकिंग API से `relevance_score` भी प्राप्त होता है।
...
यह एक पूर्ण RAG श्रृंखला का एक उदाहरण दिखाता है, जिसमें Vertex Ranking API का उपयोग करके रीरैंकिंग करने के लिए एक सरल प्रॉम्प्ट टेम्प्लेट है।
