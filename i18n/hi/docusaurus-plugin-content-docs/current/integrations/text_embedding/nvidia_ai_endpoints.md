---
translated: true
---

# NVIDIA AI Foundation Endpoints

> [NVIDIA AI Foundation Endpoints](https://www.nvidia.com/en-us/ai-data-science/foundation-models/) उपयोगकर्ताओं को NVIDIA होस्ट किए गए API एंडपॉइंट्स तक आसान पहुंच प्रदान करते हैं जो NVIDIA AI Foundation मॉडल्स जैसे Mixtral 8x7B, Llama 2, Stable Diffusion आदि के लिए हैं। [NVIDIA API कैटलॉग](https://build.nvidia.com/) पर होस्ट किए गए ये मॉडल, NVIDIA AI प्लेटफॉर्म पर अनुकूलित, परीक्षित और होस्ट किए गए हैं, जिससे वे किसी भी एक्सलरेटेड स्टैक पर तेज और आसानी से मूल्यांकन, और आगे अनुकूलन और सुचारु प्रदर्शन करने में सक्षम हैं।

> [NVIDIA AI Foundation Endpoints](https://www.nvidia.com/en-us/ai-data-science/foundation-models/) के साथ, आप [NVIDIA DGX Cloud](https://www.nvidia.com/en-us/data-center/dgx-cloud/) पर चल रहे पूरी तरह से एक्सलरेटेड स्टैक से तेज परिणाम प्राप्त कर सकते हैं। एक बार अनुकूलित होने के बाद, ये मॉडल [NVIDIA AI Enterprise](https://www.nvidia.com/en-us/data-center/products/ai-enterprise/) का उपयोग करके उद्यम-स्तरीय सुरक्षा, स्थिरता और समर्थन के साथ कहीं भी तैनात किए जा सकते हैं।

> ये मॉडल [`langchain-nvidia-ai-endpoints`](https://pypi.org/project/langchain-nvidia-ai-endpoints/) पैकेज के माध्यम से आसानी से एक्सेस किए जा सकते हैं, जैसा कि नीचे दिखाया गया है।

यह उदाहरण [NVIDIA Retrieval QA Embedding Model](https://catalog.ngc.nvidia.com/orgs/nvidia/teams/ai-foundation/models/nvolve-40k) के साथ [retrieval-augmented generation](https://developer.nvidia.com/blog/build-enterprise-retrieval-augmented-generation-apps-with-nvidia-retrieval-qa-embedding-model/) के लिए `NVIDIAEmbeddings` क्लास का उपयोग करके LangChain का उपयोग करने के बारे में बताता है।

चैट मॉडल तक पहुंचने के लिए इस एपीआई का उपयोग करने के बारे में अधिक जानकारी के लिए, [ChatNVIDIA](https://python.langchain.com/docs/integrations/chat/nvidia_ai_endpoints/) दस्तावेज देखें।

## स्थापना

```python
%pip install --upgrade --quiet  langchain-nvidia-ai-endpoints
```

```output
Note: you may need to restart the kernel to use updated packages.
```

## सेटअप

**शुरू करने के लिए:**

1. [NVIDIA](https://build.nvidia.com/) के साथ एक मुफ्त खाता बनाएं, जो NVIDIA AI Foundation मॉडल होस्ट करता है

2. `Retrieval` टैब चुनें, फिर अपने पसंदीदा मॉडल का चयन करें

3. `Input` के तहत `Python` टैब चुनें, और `Get API Key` पर क्लिक करें। फिर `Generate Key` पर क्लिक करें।

4. उत्पन्न कुंजी को `NVIDIA_API_KEY` के रूप में कॉपी और सहेजें। इसके बाद, आपके पास एंडपॉइंट्स तक पहुंच होनी चाहिए।

```python
import getpass
import os

# del os.environ['NVIDIA_API_KEY']  ## delete key and reset
if os.environ.get("NVIDIA_API_KEY", "").startswith("nvapi-"):
    print("Valid NVIDIA_API_KEY already in environment. Delete to reset")
else:
    nvapi_key = getpass.getpass("NVAPI Key (starts with nvapi-): ")
    assert nvapi_key.startswith("nvapi-"), f"{nvapi_key[:5]}... is not a valid key"
    os.environ["NVIDIA_API_KEY"] = nvapi_key
```

```output
Valid NVIDIA_API_KEY already in environment. Delete to reset
```

हम उस सूची में एक embedding मॉडल देख सकते हैं जिसका उपयोग LLM के साथ प्रभावी RAG समाधान के लिए किया जा सकता है। हम `NVIDIAEmbeddings` मॉडल की मदद से इस मॉडल के साथ बहुत आसानी से काम कर सकते हैं।

## प्रारंभीकरण

एक embedding मॉडल को प्रारंभ करते समय, आप इसे पास कर सकते हैं, उदाहरण के लिए `ai-embed-qa-4` नीचे, या डिफ़ॉल्ट का उपयोग कर सकते हैं।

```python
from langchain_nvidia_ai_endpoints import NVIDIAEmbeddings

embedder = NVIDIAEmbeddings(model="ai-embed-qa-4")
```

यह मॉडल एक फाइन-ट्यून्ड E5-large मॉडल है जो अपेक्षित `Embeddings` विधियों का समर्थन करता है, जिनमें शामिल हैं:

- `embed_query`: किसी प्रश्न नमूने के लिए क्वेरी एम्बेडिंग उत्पन्न करें।

- `embed_documents`: खोज के लिए जिन दस्तावेजों को आप खोजना चाहते हैं, उनके लिए पैसेज एम्बेडिंग उत्पन्न करें।

- `aembed_quey`/`embed_documents`: उपरोक्त के असिंक्रोनस संस्करण।

### **समानता**

निम्नलिखित एक उपयोग, प्रारूप और गति के लिहाज से विधियों की एक त्वरित जांच है, जिसका उपयोग निम्नलिखित डेटा बिंदुओं को एम्बेड करने के लिए किया जा सकता है:

**क्वेरी:**

- Komchatka में मौसम कैसा है?

- इटली किस प्रकार के खाद्य पदार्थों के लिए जाना जाता है?

- मेरा नाम क्या है? मैं दांव लगाता हूं कि आप याद नहीं करेंगे...

- जीवन का मकसद क्या है, वैसे भी?

- जीवन का मकसद मजा करना है :D

**दस्तावेज:**

- Komchatka का मौसम ठंडा है, लंबे और कठोर सर्दियों के साथ।

- इटली पास्ता, पिज्जा, जेलाटो और एस्प्रेसो के लिए प्रसिद्ध है।

- मैं व्यक्तिगत नामों को याद नहीं कर सकता, केवल जानकारी प्रदान कर सकता हूं।

- जीवन का उद्देश्य व्यक्तिगत संतुष्टि के रूप में देखा जाता है।

- जीवन के क्षणों का आनंद लेना निश्चित रूप से एक अद्भुत दृष्टिकोण है।

### एम्बेडिंग रनटाइम

```python
import time

print("Single Query Embedding: ")
s = time.perf_counter()
q_embedding = embedder.embed_query("What's the weather like in Komchatka?")
elapsed = time.perf_counter() - s
print("\033[1m" + f"Executed in {elapsed:0.2f} seconds." + "\033[0m")
print("Shape:", (len(q_embedding),))

print("\nSequential Embedding: ")
s = time.perf_counter()
q_embeddings = [
    embedder.embed_query("What's the weather like in Komchatka?"),
    embedder.embed_query("What kinds of food is Italy known for?"),
    embedder.embed_query("What's my name? I bet you don't remember..."),
    embedder.embed_query("What's the point of life anyways?"),
    embedder.embed_query("The point of life is to have fun :D"),
]
elapsed = time.perf_counter() - s
print("\033[1m" + f"Executed in {elapsed:0.2f} seconds." + "\033[0m")
print("Shape:", (len(q_embeddings), len(q_embeddings[0])))

print("\nBatch Query Embedding: ")
s = time.perf_counter()
# To use the "query" mode, we have to add it as an instance arg
q_embeddings = NVIDIAEmbeddings(
    model="ai-embed-qa-4", model_type="query"
).embed_documents(
    [
        "What's the weather like in Komchatka?",
        "What kinds of food is Italy known for?",
        "What's my name? I bet you don't remember...",
        "What's the point of life anyways?",
        "The point of life is to have fun :D",
    ]
)
elapsed = time.perf_counter() - s
print("\033[1m" + f"Executed in {elapsed:0.2f} seconds." + "\033[0m")
print("Shape:", (len(q_embeddings), len(q_embeddings[0])))
```

```output
Single Query Embedding:
[1mExecuted in 2.19 seconds.[0m
Shape: (1024,)

Sequential Embedding:
[1mExecuted in 3.16 seconds.[0m
Shape: (5, 1024)

Batch Query Embedding:
[1mExecuted in 1.23 seconds.[0m
Shape: (5, 1024)
```

### दस्तावेज एम्बेडिंग

```python
import time

print("Single Document Embedding: ")
s = time.perf_counter()
d_embeddings = embedder.embed_documents(
    [
        "Komchatka's weather is cold, with long, severe winters.",
    ]
)
elapsed = time.perf_counter() - s
print("\033[1m" + f"Executed in {elapsed:0.2f} seconds." + "\033[0m")
print("Shape:", (len(q_embedding),))

print("\nBatch Document Embedding: ")
s = time.perf_counter()
d_embeddings = embedder.embed_documents(
    [
        "Komchatka's weather is cold, with long, severe winters.",
        "Italy is famous for pasta, pizza, gelato, and espresso.",
        "I can't recall personal names, only provide information.",
        "Life's purpose varies, often seen as personal fulfillment.",
        "Enjoying life's moments is indeed a wonderful approach.",
    ]
)
elapsed = time.perf_counter() - s
print("\033[1m" + f"Executed in {elapsed:0.2f} seconds." + "\033[0m")
print("Shape:", (len(q_embeddings), len(q_embeddings[0])))
```

```output
Single Document Embedding:
[1mExecuted in 0.52 seconds.[0m
Shape: (1024,)

Batch Document Embedding:
[1mExecuted in 0.89 seconds.[0m
Shape: (5, 1024)
```

अब जब हमने अपने एम्बेडिंग्स उत्पन्न कर लिए हैं, तो हम परिणामों पर एक सरल समानता जांच कर सकते हैं ताकि देख सकें कि कौन से दस्तावेज एक पुनर्प्राप्ति कार्य में उचित जवाब के रूप में ट्रिगर किए गए होंगे:

```python
%pip install --upgrade --quiet  matplotlib scikit-learn
```

```output
Note: you may need to restart the kernel to use updated packages.
```

```python
import matplotlib.pyplot as plt
import numpy as np
from sklearn.metrics.pairwise import cosine_similarity

# Assuming embeddings1 and embeddings2 are your two sets of vectors
# Compute the similarity matrix between embeddings1 and embeddings2
cross_similarity_matrix = cosine_similarity(
    np.array(q_embeddings),
    np.array(d_embeddings),
)

# Plotting the cross-similarity matrix
plt.figure(figsize=(8, 6))
plt.imshow(cross_similarity_matrix, cmap="Greens", interpolation="nearest")
plt.colorbar()
plt.title("Cross-Similarity Matrix")
plt.xlabel("Query Embeddings")
plt.ylabel("Document Embeddings")
plt.grid(True)
plt.show()
```

![](data:image/png;base64,)

याद रखने के लिए, हमारी प्रणाली को भेजे गए क्वेरी और दस्तावेज निम्नलिखित थे:

**क्वेरी:**

- Komchatka में मौसम कैसा है?

- इटली किस प्रकार के खाद्य पदार्थों के लिए जाना जाता है?

- मेरा नाम क्या है? मैं दांव लगाता हूं कि आप याद नहीं करेंगे...

- जीवन का मकसद क्या है, वैसे भी?

- जीवन का मकसद मजा करना है :D

**दस्तावेज:**

- Komchatka का मौसम ठंडा है, लंबे और कठोर सर्दियों के साथ।

- इटली पास्ता, पिज्जा, जेलाटो और एस्प्रेसो के लिए प्रसिद्ध है।

- मैं व्यक्तिगत नामों को याद नहीं कर सकता, केवल जानकारी प्रदान कर सकता हूं।

- जीवन का उद्देश्य व्यक्तिगत संतुष्टि के रूप में देखा जाता है।

- जीवन के क्षणों का आनंद लेना निश्चित रूप से एक अद्भुत दृष्टिकोण है।

## RAG पुनर्प्राप्ति:

निम्नलिखित [LangChain अभिव्यक्ति भाषा पुनर्प्राप्ति कुकबुक प्रविष्टि](
https://python.langchain.com/docs/expression_language/cookbook/retrieval) के प्रारंभिक उदाहरण का पुनर्उपयोग है, लेकिन AI Foundation Models' [Mixtral 8x7B Instruct](https://catalog.ngc.nvidia.com/orgs/nvidia/teams/ai-foundation/models/mixtral-8x7b) और [NVIDIA Retrieval QA Embedding](https://catalog.ngc.nvidia.com/orgs/nvidia/teams/ai-foundation/models/nvolve-40k) मॉडल्स के साथ निष्पादित किया गया है जो उनके खेल पर्यावरण में उपलब्ध हैं। कुकबुक में बाद के उदाहरण भी अपेक्षित रूप से चलते हैं, और हम आपको इन विकल्पों का अन्वेषण करने के लिए प्रोत्साहित करते हैं।

**सुझाव:** हम Mixtral का उपयोग आंतरिक तर्कशक्ति (यानी डेटा निष्कर्षण, उपकरण चयन आदि के लिए निर्देश पालन) के लिए और Llama-Chat का उपयोग एक अंतिम "इतिहास और संदर्भ के आधार पर इस उपयोगकर्ता के लिए एक सरल प्रतिक्रिया बनाकर समाप्त करने" के लिए करने की सिफारिश करेंगे।

```python
%pip install --upgrade --quiet  langchain faiss-cpu tiktoken

from operator import itemgetter

from langchain_community.vectorstores import FAISS
from langchain_core.output_parsers import StrOutputParser
from langchain_core.prompts import ChatPromptTemplate
from langchain_core.runnables import RunnablePassthrough
from langchain_nvidia_ai_endpoints import ChatNVIDIA
```

```output
Note: you may need to restart the kernel to use updated packages.
```

```python
vectorstore = FAISS.from_texts(
    ["harrison worked at kensho"],
    embedding=NVIDIAEmbeddings(model="ai-embed-qa-4"),
)
retriever = vectorstore.as_retriever()

prompt = ChatPromptTemplate.from_messages(
    [
        (
            "system",
            "Answer solely based on the following context:\n<Documents>\n{context}\n</Documents>",
        ),
        ("user", "{question}"),
    ]
)

model = ChatNVIDIA(model="ai-mixtral-8x7b-instruct")

chain = (
    {"context": retriever, "question": RunnablePassthrough()}
    | prompt
    | model
    | StrOutputParser()
)

chain.invoke("where did harrison work?")
```

```output
'Based on the document provided, Harrison worked at Kensho.'
```

```python
prompt = ChatPromptTemplate.from_messages(
    [
        (
            "system",
            "Answer using information solely based on the following context:\n<Documents>\n{context}\n</Documents>"
            "\nSpeak only in the following language: {language}",
        ),
        ("user", "{question}"),
    ]
)

chain = (
    {
        "context": itemgetter("question") | retriever,
        "question": itemgetter("question"),
        "language": itemgetter("language"),
    }
    | prompt
    | model
    | StrOutputParser()
)

chain.invoke({"question": "where did harrison work", "language": "italian"})
```

```output
'Harrison ha lavorato presso Kensho.\n\n(In English: Harrison worked at Kensho.)'
```
