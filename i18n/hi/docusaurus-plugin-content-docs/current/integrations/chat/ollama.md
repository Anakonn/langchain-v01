...
[Ollama](https://ollama.ai/) आपको Llama 2 जैसे ओपन-सोर्स बड़े भाषा मॉडल्स को लोकल रूप से चलाने की अनुमति देता है।

Ollama मॉडल वेट्स, कॉन्फ़िगरेशन, और डेटा को एक पैकेज में बंडल करता है, जिसे एक Modelfile द्वारा परिभाषित किया जाता है।

यह सेटअप और कॉन्फ़िगरेशन विवरणों को ऑप्टिमाइज़ करता है, जिसमें GPU उपयोग भी शामिल है।

सपोर्टेड मॉडल्स और मॉडल वेरिएंट्स की पूरी सूची के लिए, [Ollama मॉडल लाइब्रेरी](https://github.com/jmorganca/ollama#model-library) देखें।

## सेटअप

सबसे पहले, एक लोकल Ollama इंस्टेंस सेटअप और रन करने के लिए [इन निर्देशों](https://github.com/jmorganca/ollama) का पालन करें:

* [डाउनलोड](https://ollama.ai/download) करें और समर्थित प्लेटफार्मों (जिसमें Windows Subsystem for Linux भी शामिल है) पर Ollama इंस्टॉल करें
* उपलब्ध LLM मॉडल को `ollama pull <name-of-model>` के माध्यम से प्राप्त करें
    * उपलब्ध मॉडल्स की सूची देखने के लिए [मॉडल लाइब्रेरी](https://ollama.ai/library) देखें
    * उदाहरण के लिए, `ollama pull llama3`
* यह मॉडल का डिफ़ॉल्ट टैग्ड संस्करण डाउनलोड करेगा। सामान्यतया, डिफ़ॉल्ट सबसे नवीनतम, सबसे छोटे आकार के पैरामीटर मॉडल की ओर इशारा करता है।

> Mac पर, मॉडल्स `~/.ollama/models` में डाउनलोड होंगे
>
> Linux (या WSL) पर, मॉडल्स `/usr/share/ollama/.ollama/models` में संग्रहीत होंगे

* रुचि के मॉडल का सटीक संस्करण निर्दिष्ट करें जैसे `ollama pull vicuna:13b-v1.5-16k-q4_0` (इस मामले में [Vicuna के विभिन्न टैग्स](https://ollama.ai/library/vicuna/tags) देखें)
* सभी पुल किए गए मॉडल्स को देखने के लिए, `ollama list` का उपयोग करें
* कमांड लाइन से सीधे किसी मॉडल से चैट करने के लिए, `ollama run <name-of-model>` का उपयोग करें
* अधिक आदेशों के लिए [Ollama दस्तावेज़ीकरण](https://github.com/jmorganca/ollama) देखें। उपलब्ध आदेशों को देखने के लिए टर्मिनल में `ollama help` चलाएँ।

## उपयोग

आप समर्थित पैरामीटर की पूरी सूची [API रेफरेंस पेज](https://api.python.langchain.com/en/latest/llms/langchain.llms.ollama.Ollama.html) पर देख सकते हैं।

यदि आप LLaMA `chat` मॉडल का उपयोग कर रहे हैं (उदाहरण के लिए, `ollama pull llama3`) तो आप `ChatOllama` इंटरफ़ेस का उपयोग कर सकते हैं।

इसमें सिस्टम संदेश और उपयोगकर्ता इनपुट के लिए [विशेष टोकन्स](https://huggingface.co/blog/llama2#how-to-prompt-llama-2) शामिल हैं।

## मॉडल्स के साथ इंटरैक्ट करना

यहाँ कुछ तरीके दिए गए हैं जिनसे आप पुल किए गए लोकल मॉडल्स के साथ इंटरैक्ट कर सकते हैं

#### सीधे टर्मिनल में:

* आपके सभी लोकल मॉडल्स स्वचालित रूप से `localhost:11434` पर सर्व किए जाते हैं
* कमांड लाइन के माध्यम से सीधे इंटरैक्ट करने के लिए `ollama run <name-of-model>` चलाएँ

### एक API के माध्यम से

इंटरैक्ट करने के लिए Ollama के API एंडपॉइंट को `application/json` अनुरोध भेजें।
...
सभी एंडपॉइंट्स के लिए Ollama [API दस्तावेज़ीकरण](https://github.com/jmorganca/ollama/blob/main/docs/api.md) देखें।

#### LangChain के माध्यम से

LangChain एप्लिकेशन में `ChatOllama` चैट मॉडल के माध्यम से Ollama का उपयोग करने का एक सामान्य बुनियादी उदाहरण देखें।
...
LCEL chains, बॉक्स से बाहर, अतिरिक्त कार्यात्मकताएँ प्रदान करते हैं, जैसे प्रतिक्रियाओं का स्ट्रीमिंग, और असिंक्रोनस समर्थन
...
स्ट्रीमिंग असिंक्रोनस समर्थन के लिए, यहाँ एक उदाहरण है - सभी संभव एकल चैन से जो ऊपर बनाया गया है।
...
अन्य उपलब्ध इंटरफेस के लिए [LangChain Expressive Language (LCEL) Interface](/docs/expression_language/interface) देखें जब एक चैन बनाया जाता है।

## स्रोत से निर्माण

स्रोत से निर्माण पर अद्यतित निर्देशों के लिए, [स्रोत से निर्माण](https://github.com/jmorganca/ollama?tab=readme-ov-file#building) पर Ollama दस्तावेज़ीकरण देखें

## निष्कर्षण

Ollama के नवीनतम संस्करण का उपयोग करें और [`format`](https://github.com/jmorganca/ollama/blob/main/docs/api.md#json-mode) फ्लैग प्रदान करें। `format` फ्लैग मॉडल को JSON में प्रतिक्रिया उत्पन्न करने के लिए मजबूर करेगा।

> **नोट:** आप सुविधा के लिए प्रयोगात्मक [OllamaFunctions](/docs/integrations/chat/ollama_functions) रैपर भी आज़मा सकते हैं।
...
Ollama में मल्टी-मॉडल LLMs के लिए समर्थन है, जैसे [bakllava](https://ollama.ai/library/bakllava) और [llava](https://ollama.ai/library/llava)।

`tags` के साथ मॉडल्स के लिए संस्करणों का पूरा सेट ब्राउज़ करें, जैसे [Llava](https://ollama.ai/library/llava/tags)।

वांछित LLM को `ollama pull bakllava` के माध्यम से डाउनलोड करें

मल्टी-मॉडल को सपोर्ट करने के लिए सबसे नवीनतम संस्करण के लिए Ollama को अपडेट करना सुनिश्चित करें।

नीचे ChatOllama मल्टी-मॉडल समर्थन का उपयोग करने का एक सामान्य उदाहरण देखें:
