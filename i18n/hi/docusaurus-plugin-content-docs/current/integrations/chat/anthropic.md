...
यह नोटबुक एंथ्रोपिक चैट मॉडल्स का उपयोग शुरू करने के बारे में कवर करता है।

## सेटअप

सेटअप निर्देशों के लिए, कृपया [एंथ्रोपिक प्लेटफ़ॉर्म पृष्ठ](/docs/integrations/platforms/anthropic.mdx) के इंस्टॉलेशन और पर्यावरण सेटअप अनुभागों देखें।
...
प्रदान किया गया कोड मान्य करता है कि आपका ANTHROPIC_API_KEY आपके पर्यावरण चरों में सेट है। यदि आप अपनी API कुंजी को मैन्युअल रूप से निर्दिष्ट करना चाहते हैं और एक अलग मॉडल का भी उपयोग करना चाहते हैं, तो आप निम्नलिखित कोड का उपयोग कर सकते हैं:
...
इन डेमो में, हम Claude 3 Opus मॉडल का उपयोग करेंगे, और आप Sonnet मॉडल के लॉन्च संस्करण का भी उपयोग कर सकते हैं `claude-3-sonnet-20240229` के साथ।

आप मॉडल तुलना दस्तावेज़ [यहाँ](https://docs.anthropic.com/claude/docs/models-overview#model-comparison) देख सकते हैं।
...
## `ChatAnthropic` एसिंक्रोनस और स्ट्रीमिंग कार्यक्षमता का भी समर्थन करता है:
...
एंथ्रोपिक के [टूल-कॉलिंग, या टूल-उपयोग, API](https://docs.anthropic.com/claude/docs/functions-external-tools) के साथ, आप मॉडल को उपकरण परिभाषित कर सकते हैं। यह टूल-उपयोग श्रृंखलाओं और एजेंटों को बनाने के लिए अत्यंत उपयोगी है, साथ ही मॉडल से संरचित आउटपुट प्राप्त करने के लिए भी।

:::note

एंथ्रोपिक का टूल-कॉलिंग कार्यक्षमता अभी भी बीटा में है।

:::

### bind_tools()

`ChatAnthropic.bind_tools` के साथ, हम आसानी से Pydantic वर्गों, डिक्ट स्कीमाओं, LangChain उपकरणों या यहां तक कि कार्यों को भी उपकरणों के रूप में मॉडल को पास कर सकते हैं। इसके तहत ये एंथ्रोपिक उपकरण स्कीमाओं में रूपांतरित किए जाते हैं, जो इस प्रकार दिखते हैं:
...
ध्यान दें कि आउटपुट संदेश सामग्री एक सूची है जिसमें एक पाठ ब्लॉक और फिर एक उपकरण_उपयोग ब्लॉक होता है:
...
महत्वपूर्ण बात यह है कि उपकरण कॉल भी `tool_calls` में निकाले जाते हैं जहां वे एक मानक, मॉडल-निरपेक्ष प्रारूप में होते हैं:
...
ChatAnthropic मॉडल आउटपुट हमेशा एक एकल AI संदेश होता है जिसमें या तो एक अकेला स्ट्रिंग या सामग्री ब्लॉकों की एक सूची होती है। सामग्री ब्लॉक पाठ ब्लॉक या उपकरण-उपयोग ब्लॉक हो सकते हैं। इनमें से एक या अधिक हो सकते हैं और वे एक-दूसरे के बीच होते हैं।

:::

### उपकरण कॉल पार्स करना

`langchain_anthropic.output_parsers.ToolsOutputParser` एंथ्रोपिक AI संदेश से उपकरण कॉल को Pydantic वस्तुओं में पार्स करने में आसान बनाता है यदि हम ऐसा करना चाहते हैं:
...
[BaseChatModel.with_structured_output इंटरफ़ेस](/docs/modules/model_io/chat/structured_output) चैट मॉडल से संरचित आउटपुट प्राप्त करना आसान बनाता है। आप `ChatAnthropic.with_structured_output` का उपयोग कर सकते हैं, जो टूल-कॉलिंग का उपयोग करता है), ताकि मॉडल को एक विशिष्ट प्रारूप में आउटपुट देने के लिए अधिक विश्वसनीय बना सके:
...
के बीच का मुख्य अंतर यह है कि यह केवल पहला GetWeather कॉल लौटाएगा, जबकि दूसरा दृष्टिकोण एक सूची लौटाएगा।

### उपकरण परिणामों को मॉडल में पास करना

हम उचित `tool_call_id` के साथ `ToolMessage` का उपयोग कर सकते हैं ताकि उपकरण परिणामों को मॉडल को वापस पास किया जा सके:
...
एंथ्रोपिक वर्तमान में उपकरण कॉल स्ट्रीमिंग का समर्थन नहीं करता है। स्ट्रीमिंग का प्रयास एक अंतिम संदेश लौटाएगा।
...
एंथ्रोपिक के Claude-3 मॉडल छवि और पाठ दोनों इनपुट के साथ संगत हैं। आप इसका निम्न प्रकार से उपयोग कर सकते हैं:
